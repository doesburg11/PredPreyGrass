...
from predpreygrass.rllib.v2_speed.config_env import config_env
...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        moore_actions = {0: (-1, -1), 1: (-1, 0), 2: (-1, 1), 3: (0, -1), 4: (0, 0), 5: (0, 1), 6: (1, -1), 7: (1, 0), 8: (1, 1)}
        speed2_actions = {i: (dx, dy) for i, (dx, dy) in enumerate([(dx, dy) for dx in range(-2, 3) for dy in range(-2, 3)])}
        ...
        for agent in self.agents[:]:
            if "predator" in agent:
                if self.agent_energies[agent] >= self.predator_creation_energy_threshold:
                    ...
                    if not potential_new_ids:
                        if self.verbose_spawning:
                            print(f"[SPAWNING] No available predator slots at speed {new_speed}")
                        continue
                    ...
                    if self.verbose_spawning:
                        print(f"[SPAWNING] Predator {agent} spawned {new_agent} at {new_position}")
                    else:
                        if self.verbose_spawning:
                            print("[SPAWNING] No new predator slots available for spawning")
                ...
                if self.agent_energies[agent] >= self.prey_creation_energy_threshold:
                    ...
                    if not potential_new_ids:
                        if self.verbose_spawning:
                            print(f"[SPAWNING] No available prey slots at speed {new_speed}")
                        continue
                    ...
                    if self.verbose_spawning:
                        print(f"[SPAWNING] Prey {agent} spawned {new_agent} at {new_position}")
                    else:
                        if self.verbose_spawning:
                            print("[SPAWNING] No new prey slots available for spawning")
