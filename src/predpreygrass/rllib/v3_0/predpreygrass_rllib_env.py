...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _handle_predator_engagement(self, agent, observations, rewards, terminations, truncations):
        ...
        if caught_prey:
            ...
            rewards[agent] = self._get_agent_reward(agent, "reward_predator_catch_prey")
            ...
            rewards[agent] = self._get_agent_reward(agent, "reward_predator_step")
    ...
    def _handle_prey_engagement(self, agent, observations, rewards, terminations, truncations):
        ...
        if caught_grass:
            ...
            rewards[agent] = self._get_agent_reward(agent, "reward_prey_eat_grass")
            self.cumulative_rewards.setdefault(agent, 0)
            self.cumulative_rewards[agent] += rewards[agent]
            ...
            rewards[agent] = self._get_agent_reward(agent, "reward_prey_step")
    ...
    def _handle_predator_reproduction(self, agent, rewards, observations, terminations, truncations):
        ...
        if self.agent_energies[agent] >= self.predator_creation_energy_threshold:
            ...
            if not potential_new_ids:
                ...
                rewards[agent] = self._get_agent_reward(agent, "reproduction")
                self.cumulative_rewards.setdefault(agent, 0)
                self.cumulative_rewards[agent] += rewards[agent]
                self._log(
    ...
    def _get_agent_reward(self, agent: str, key: str) -> float:
        """
        Safely look up a reward value for a given agent type and reward key from self.config.

        Args:
            agent (str): Agent ID, e.g. "speed_2_prey_3"
            key (str): Reward key, e.g. "reward_prey_eat_grass"
