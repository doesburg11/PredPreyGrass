...
if __name__ == "__main__":
    ...
    def extract_and_sanitize_hyperparams(config_obj):
        hyperparams = {}
        for key in HYPERPARAM_KEYS:
            if hasattr(config_obj, key):
                val = getattr(config_obj, key)
                try:
                    json.dumps(val)  # test JSON serializability
                    hyperparams[key] = val
                except TypeError:
                    hyperparams[key] = str(val)  # fallback to string
        return hyperparams

    hyperparams = extract_and_sanitize_hyperparams(ppo_config)

    with open(experiment_path / "ppo_hyperparams_clean.json", "w") as f:
