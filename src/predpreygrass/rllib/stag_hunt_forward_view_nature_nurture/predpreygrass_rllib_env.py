...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        self.team_capture_join_cost = max(0.0, float(config.get("team_capture_join_cost", 0.0)))
        self.team_capture_scavenger_fraction = float(config.get("team_capture_scavenger_fraction", 0.0))
        self.team_capture_scavenger_fraction = min(max(self.team_capture_scavenger_fraction, 0.0), 1.0)
        # Nature + nurture knobs:
        # - nurture: binary join_hunt action
        # - nature: heritable continuous cooperation trait in [0, 1]
        self.coop_trait_enabled = bool(config.get("coop_trait_enabled", True))
        self.coop_trait_init_mean = float(config.get("coop_trait_init_mean", 0.6))
        self.coop_trait_init_std = max(0.0, float(config.get("coop_trait_init_std", 0.12)))
        self.coop_trait_mutation_std = max(0.0, float(config.get("coop_trait_mutation_std", 0.04)))
        self.coop_trait_mutation_rate = float(config.get("coop_trait_mutation_rate", 1.0))
        self.coop_trait_mutation_rate = min(max(self.coop_trait_mutation_rate, 0.0), 1.0)
        # Fraction of a joiner's energy that contributes to hunt power:
        # effort = energy * ((1 - nature_weight) + nature_weight * trait)
        self.team_capture_nature_weight = float(config.get("team_capture_nature_weight", 0.75))
        self.team_capture_nature_weight = min(max(self.team_capture_nature_weight, 0.0), 1.0)
        # Hunt success model:
        # deterministic -> ratio >= 1
        # probabilistic -> Bernoulli(1 - (1-p0)^ratio)
        # hybrid -> guaranteed success above force ratio, else probabilistic
        self.team_capture_success_model = str(config.get("team_capture_success_model", "hybrid")).lower()
        if self.team_capture_success_model not in {"deterministic", "probabilistic", "hybrid"}:
            self.team_capture_success_model = "hybrid"
        self.team_capture_base_success_p0 = float(config.get("team_capture_base_success_p0", 0.6))
        self.team_capture_base_success_p0 = min(max(self.team_capture_base_success_p0, 1e-6), 1.0 - 1e-6)
        self.team_capture_force_success_ratio = float(config.get("team_capture_force_success_ratio", 1.05))
        self.team_capture_force_success_ratio = max(self.team_capture_force_success_ratio, 0.0)
        self.team_capture_min_success_prob = float(config.get("team_capture_min_success_prob", 0.0))
        self.team_capture_min_success_prob = min(max(self.team_capture_min_success_prob, 0.0), 1.0)
        ...
        self.predator_cooperation_trait = {}
        self.used_agent_ids = set()
        ...
        self.team_capture_failed_events = []
        self.team_capture_last_success_prob = 0.0
        self.team_capture_last_effort_ratio = 0.0
        self.team_capture_success_prob_sum = 0.0
        self.team_capture_attempts = 0
    ...
    def step(self, action_dict):
        ...
        global_info["team_capture_last_success_prob"] = self.team_capture_last_success_prob
        global_info["team_capture_last_effort_ratio"] = self.team_capture_last_effort_ratio
        global_info["team_capture_attempts"] = self.team_capture_attempts
        if self.team_capture_attempts > 0:
            global_info["team_capture_avg_success_prob"] = self.team_capture_success_prob_sum / self.team_capture_attempts
        else:
            global_info["team_capture_avg_success_prob"] = 0.0
        if self.predator_cooperation_trait:
            trait_values = list(self.predator_cooperation_trait.values())
            global_info["predator_mean_coop_trait"] = float(np.mean(trait_values))
            global_info["predator_trait_variance"] = float(np.var(trait_values))
        else:
            global_info["predator_mean_coop_trait"] = 0.0
            global_info["predator_trait_variance"] = 0.0
        for agent in self.agents:
            ...
            if "predator" in agent:
                ...
                info["coop_trait"] = self.predator_cooperation_trait.get(agent, 0.0)
        ...
        for agent in self.agents:
            ...
            if "predator" in agent:
                ...
                step_data[agent]["coop_trait"] = self.predator_cooperation_trait.get(agent, 0.0)
    ...
    def _split_action(self, agent, action):
        ...
        if "predator" in agent:
            if isinstance(action, dict):
                ...
                join_hunt = action.get("join_hunt", action.get("join", 1))
            elif isinstance(action, (tuple, list)) or (
                isinstance(action, np.ndarray) and action.shape != ()
            ):
                if len(action) >= 2:
                    move, join_hunt = action[0], action[1]
                elif len(action) == 1:
                    move, join_hunt = action[0], 1
                else:
                    move, join_hunt = 0, 1
            else:
    ...
    def _sample_initial_predator_trait(self) -> float:
        if not self.coop_trait_enabled:
            return 1.0
        trait = float(self.rng.normal(self.coop_trait_init_mean, self.coop_trait_init_std))
        return self._clip_trait(trait)

    def _inherit_predator_trait(self, parent_id: str) -> float:
        if not self.coop_trait_enabled:
            return 1.0
        parent_trait = float(self.predator_cooperation_trait.get(parent_id, self.coop_trait_init_mean))
        child_trait = parent_trait
        if self.coop_trait_mutation_std > 0.0 and self.rng.random() < self.coop_trait_mutation_rate:
            child_trait += float(self.rng.normal(0.0, self.coop_trait_mutation_std))
        return self._clip_trait(child_trait)

    def _get_predator_trait(self, predator_id: str) -> float:
        if not self.coop_trait_enabled:
            return 1.0
        return float(self.predator_cooperation_trait.get(predator_id, self.coop_trait_init_mean))

    def _sync_predator_trait_record(self, predator_id: str) -> None:
        if "predator" not in predator_id:
            return
        trait = self.predator_cooperation_trait.get(predator_id)
        if trait is None:
            return
        record = self.agent_stats_live.get(predator_id)
        if record is None:
            return
        if record.get("coop_trait_at_birth") is None:
            record["coop_trait_at_birth"] = float(trait)
        record["coop_trait_last"] = float(trait)

    def _compute_team_capture_success(self, prey_id: str, joiners: list[str], prey_energy: float):
        prey_difficulty = max(prey_energy + float(self.team_capture_margin), 1e-8)
        if not joiners:
            return False, 0.0, 0.0, 0.0

        if not self.coop_trait_enabled:
            total_effort = sum(float(self.agent_energies.get(pid, 0.0)) for pid in joiners)
            effort_ratio = total_effort / prey_difficulty
            success = effort_ratio > 1.0
            return success, 1.0 if success else 0.0, effort_ratio, total_effort

        nature_weight = self.team_capture_nature_weight
        total_effort = 0.0
        for pid in joiners:
            energy = float(self.agent_energies.get(pid, 0.0))
            trait = self._get_predator_trait(pid)
            trait_factor = (1.0 - nature_weight) + nature_weight * trait
            total_effort += energy * trait_factor

        effort_ratio = total_effort / prey_difficulty
        base_prob = 1.0 - (1.0 - self.team_capture_base_success_p0) ** max(effort_ratio, 0.0)
        success_prob = min(max(base_prob, self.team_capture_min_success_prob), 1.0)
        force_success = effort_ratio >= self.team_capture_force_success_ratio

        model = self.team_capture_success_model
        if model == "deterministic":
            ...
            success = force_success or bool(self.rng.random() < success_prob)

        return success, float(success_prob), float(effort_ratio), float(total_effort)
    ...
    def _handle_team_capture(self, prey_id):
        ...
        joiners = [h for h in nearby_predators if self.predator_join_intent.get(h, True)]
        free_riders = [h for h in nearby_predators if h not in joiners]
        if not joiners:
            return False
        ...
        success, success_prob, effort_ratio, total_effort = self._compute_team_capture_success(
            prey_id, joiners, prey_energy
        )
        self.team_capture_last_success_prob = success_prob
        self.team_capture_last_effort_ratio = effort_ratio
        self.team_capture_attempts += 1
        self.team_capture_success_prob_sum += success_prob
        helper_traits = [self._get_predator_trait(pid) for pid in joiners]
        mean_helper_trait = float(np.mean(helper_traits)) if helper_traits else 0.0
        if not success:
            ...
            for pid in joiners:
                    ...
                    evt.setdefault("failed_eating_events", []).append(
                        {
                            ...
                            "effort_ratio": float(effort_ratio),
                            "total_effort": float(total_effort),
                            "coop_trait": float(self._get_predator_trait(pid)),
                        }
                    )
                ...
                info["team_capture_success_prob"] = float(success_prob)
                info["team_capture_effort_ratio"] = float(effort_ratio)
                info["team_capture_total_effort"] = float(total_effort)
                info["team_capture_mean_helper_trait"] = mean_helper_trait
                info["team_capture_joined"] = True
                info["coop_trait"] = float(self._get_predator_trait(pid))
            for pid in free_riders:
                info = self._pending_infos.setdefault(pid, {})
                info["team_capture_helpers"] = helper_count
                info["team_capture_free_riders"] = len(free_riders)
                info["team_capture_success_prob"] = float(success_prob)
                ...
                info["team_capture_total_effort"] = float(total_effort)
                info["team_capture_mean_helper_trait"] = mean_helper_trait
                info["team_capture_joined"] = False
                info["coop_trait"] = float(self._get_predator_trait(pid))
            ...
            global_info["team_capture_last_free_riders"] = len(free_riders)
            global_info["team_capture_last_success_prob"] = float(success_prob)
            global_info["team_capture_last_effort_ratio"] = float(effort_ratio)
            global_info["team_capture_last_total_effort"] = float(total_effort)
            global_info["team_capture_last_mean_helper_trait"] = mean_helper_trait
            return False
        ...
        scavenger_share = scavenger_energy_total / len(free_riders) if free_riders else 0.0
        if scavenger_share:
            for pid in free_riders:
                self.agents_just_ate.add(pid)
                self._per_agent_step_deltas.setdefault(
                ...
                self.agent_energies[pid] += scavenger_share
                self._per_agent_step_deltas[pid]["eat"] += scavenger_share
                self.grid_world_state[self.predator_channel, *self.agent_positions[pid]] = self.agent_energies[pid]
                self.rewards[pid] = self.rewards.get(pid, 0.0)
                pred_record = self.agent_stats_live.get(pid)
                if pred_record is not None:
                    pred_record["times_ate"] += 1
                    pred_record["energy_gained"] += scavenger_share
                info = self._pending_infos.setdefault(pid, {})
                info["team_capture_helpers"] = helper_count
                info["team_capture_free_riders"] = len(free_riders)
                info["team_capture_energy_gain"] = 0.0
                info["team_capture_scavenger_gain"] = scavenger_share
                info["team_capture_join_cost"] = 0.0
                info["team_capture_joined"] = False
                info["team_capture_success_prob"] = float(success_prob)
                info["team_capture_effort_ratio"] = float(effort_ratio)
                info["team_capture_total_effort"] = float(total_effort)
                    ...
                    evt.setdefault("eating_events", []).append(
                        {
                            ...
                            "position_resource": tuple(prey_pos),
                            "position_consumer": tuple(self.agent_positions[pid]),
                            "bite_size": float(scavenger_share),
                            "energy_after": float(self.agent_energies[pid]),
                            "team_capture": helper_count > 1,
                            "predator_list": joiners,
                            "free_riders": free_riders,
    ...
    def _handle_predator_reproduction(self, agent):
        ...
        if self.agent_energies[agent] >= self.predator_creation_energy_threshold:
            ...
            self.predator_cooperation_trait[new_agent] = self._inherit_predator_trait(agent)
            self._register_new_agent(new_agent, parent_agent_id=agent)
            self._sync_predator_trait_record(new_agent)
    ...
    def get_state_snapshot(self):
        return {
            ...
            "predator_cooperation_trait": self.predator_cooperation_trait.copy(),
    ...
    # cumulative_rewards is now stored directly in agent_stats_live/agent_stats_completed
        ...
        self.predator_cooperation_trait = snapshot.get("predator_cooperation_trait", {}).copy()
        for agent_id in self.agent_stats_live:
            if "predator" in agent_id and agent_id in self.predator_cooperation_trait:
                trait = float(self.predator_cooperation_trait[agent_id])
                self.agent_stats_live[agent_id].setdefault("coop_trait_at_birth", trait)
                self.agent_stats_live[agent_id]["coop_trait_last"] = trait
    ...
    def _build_action_space(self, agent_id):
        ...
        if "predator" in agent_id:
            return gymnasium.spaces.MultiDiscrete([size, 2])
    ...
    def _register_new_agent(self, agent_id: str, parent_agent_id: Optional[str] = None, *, is_founder: bool = False):
        ...
        self.agent_stats_live[agent_id] = {
            ...
            "death_cause": None,
            "avg_energy": 0.0,
            "coop_trait_at_birth": self.predator_cooperation_trait.get(agent_id) if "predator" in agent_id else None,
            "coop_trait_last": self.predator_cooperation_trait.get(agent_id) if "predator" in agent_id else None,
        }
    ...
    def _finalize_agent_record(self, agent_id: str, cause: Optional[str] = None):
        ...
        if "predator" in agent_id:
            record["coop_trait_last"] = self.predator_cooperation_trait.get(
                agent_id, record.get("coop_trait_last")
            )
    ...
    def _place_predators(self, predator_list, predator_positions):
        ...
        for i, agent in enumerate(predator_list):
            ...
            self.predator_cooperation_trait[agent] = self._sample_initial_predator_trait()
            self._sync_predator_trait_record(agent)
            self.agent_energies[agent] = self.initial_energy_predator
    ...
    def _remove_agent_from_state(self, agent_id: str):
        ...
        if "predator" in agent_id:
            ...
            self.predator_cooperation_trait.pop(agent_id, None)
        elif "prey" in agent_id:
