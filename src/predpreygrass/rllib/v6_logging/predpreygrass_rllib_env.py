    ...
    def step(self, action_dict):
        ...
        for agent in self.agents:
            ...
            if self.agent_energies[agent] <= 0:
                self._log(
                    self.verbose_decay, f"[DECAY] {agent} at {self.agent_positions[agent]} ran out of energy and is removed.", "red"
                )
                ...
                if terminations.get(agent) is None or not terminations[agent]:
                    ...
                    if caught_grass:
                        self._log(
                            self.verbose_engagement, f"[ENGAGE] {agent} caught grass at {tuple(map(int, prey_position))}", "white"
                        )
                ...
                if self.agent_energies[agent] >= self.predator_creation_energy_threshold:
                    ...
                    if not potential_new_ids:
                        ...
                        self._log(
                            ...
                            f"[REPRODUCTION] No available predator slots at speed {new_speed} for spawning" "red",
                        )
                        continue
