...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _sample_agent_and_grass_positions(self):
        ...
        """
        # Simplified sampling: ignore walls and draw from all grid cells
        num_grid_cells = self.grid_size * self.grid_size
        num_agents_and_grass = len(self.agents) + len(self.grass_agents)
        all_indices = np.arange(num_grid_cells)
        chosen_grid_indices = self.rng.choice(all_indices, size=num_agents_and_grass, replace=False)
        chosen_positions = [(int(i) // self.grid_size, int(i) % self.grid_size) for i in chosen_grid_indices]
