...
"""
"""Random policy viewer with wall visualization (kin_selection variant).

This script intentionally imports the local kin_selection environment & renderer
so that the `walls` parameter on `PyGameRenderer.update` is available. If you
accidentally import the older kin_selection renderer, the call with `walls=`
will raise a TypeError (unexpected keyword). Ensure the imports below stay
pointing at `kin_selection` when using walls.
"""
