...
"""
"""Random policy viewer with wall visualization (selfish_gene variant).

This script intentionally imports the local selfish_gene environment & renderer
so that the `walls` parameter on `PyGameRenderer.update` is available. If you
accidentally import the older selfish_gene renderer, the call with `walls=`
will raise a TypeError (unexpected keyword). Ensure the imports below stay
pointing at `selfish_gene` when using walls.
"""

from predpreygrass.rllib.selfish_gene.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.rllib.selfish_gene.config.config_env_perimeter_four_gaps_walls import config_env as base_config_env
from predpreygrass.rllib.selfish_gene.utils.pygame_grid_renderer_rllib import PyGameRenderer
