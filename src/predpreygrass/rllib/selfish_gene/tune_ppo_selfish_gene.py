...
from predpreygrass.rllib.selfish_gene.utils.episode_return_callback import EpisodeReturn
from predpreygrass.rllib.selfish_gene.utils.networks import build_multi_module_spec
...
def get_config_ppo():
    ...
    if num_cpus == 32:
        from predpreygrass.rllib.selfish_gene.config.config_ppo_gpu_default import config_ppo
    elif num_cpus == 8:
        from predpreygrass.rllib.selfish_gene.config.config_ppo_cpu import config_ppo
    else:
...
if __name__ == "__main__":
    ...
    experiment_name = f"PPO_SELFISH_GENE_{timestamp}"
