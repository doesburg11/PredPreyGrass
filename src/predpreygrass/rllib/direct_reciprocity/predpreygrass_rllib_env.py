...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        # Capture is handled through the historical shared-prey hook, but the direct reciprocity
        # variant uses solo capture plus optional post-capture sharing.
        ...
        self.trust_neutral = float(config.get("trust_neutral", 0.5))
        self.trust_positive_delta = float(config.get("trust_positive_delta", 0.15))
        self.trust_negative_delta = float(config.get("trust_negative_delta", 0.10))
        self.trust_decay = float(config.get("trust_decay", 0.01))
        self.trust_visibility_range = int(config.get("trust_visibility_range", self.predator_obs_range))

        self.trust_channel = None
        self.trust_summary_channel = None
        next_obs_channel = self.num_obs_channels
        if self.include_trust_channel:
            self.trust_channel = next_obs_channel
            next_obs_channel += 1
        if self.include_trust_summary:
            self.trust_summary_channel = next_obs_channel
            next_obs_channel += 1
        ...
        self.predator_share_intent = {}
        self.predator_trust = {}
        self.share_events_total = 0
        self.share_opportunities_total = 0
        self.share_refusals_total = 0

    def _observation_channel_count(self):
        return (
            self.num_obs_channels
            + (1 if self.include_trust_channel else 0)
            + (1 if self.include_trust_summary else 0)
            + (1 if self.include_visibility_channel else 0)
        )
    ...
    def _init_reset_variables(self, seed):
        ...
        self.predator_share_intent = {agent_id: False for agent_id in self.agents if "predator" in agent_id}
        self.predator_trust = {}
        self._initialize_predator_trust_state()
    ...
    def step(self, action_dict):
        ...
        global_info["share_events_total"] = self.share_events_total
        global_info["share_opportunities_total"] = self.share_opportunities_total
        global_info["share_refusals_total"] = self.share_refusals_total
        for agent in self.agents:
            ...
            if "predator" in agent:
                info["share_food"] = bool(self.predator_share_intent.get(agent, False))
                info["share_events_total"] = self.share_events_total
        ...
        if ended_missing:
            ...
            for aid in ended_missing:
                if "predator" in aid:
                    ...
                    continue
                channels = self._observation_channel_count()
    ...
    def _split_action(self, agent, action):
        """Return (move_action, share_food) with share_food=None for prey agents."""
        if "predator" in agent:
            if isinstance(action, dict):
                move = action.get("move", action.get("action", 0))
                share_food = action.get("share_food", action.get("share", 0))
            elif isinstance(action, (tuple, list)) or (
                isinstance(action, np.ndarray) and action.shape != ()
            ):
                if len(action) >= 2:
                    move, share_food = action[0], action[1]
                elif len(action) == 1:
                    move, share_food = action[0], 0
                else:
                    move, share_food = 0, 0
            else:
                move, share_food = action, 0
            share_food = 1 if int(share_food) != 0 else 0
            return int(move), share_food
    ...
    def _process_agent_movements(self, action_dict):
        ...
        for agent in action_dict.keys():
            ...
            move_action, share_food = self._split_action(agent, action)
            if share_food is not None:
                self.predator_share_intent[agent] = bool(share_food)
    ...
    def _get_observation(self, agent):
        ...
        if "predator" in agent and self.include_trust_channel and self.trust_channel is not None:
            for other_id, other_pos in self.predator_positions.items():
                if other_id == agent:
                    continue
                ox, oy = other_pos
                ...
                obs_x = xolo + (ox - xlo)
                obs_y = yolo + (oy - ylo)
                obs[self.trust_channel, obs_x, obs_y] = self._get_trust_value(agent, other_id)

        if "predator" in agent and self.include_trust_summary and self.trust_summary_channel is not None:
            obs[self.trust_summary_channel, :, :] = self._compute_trust_summary(agent)
        ...
        if self.include_visibility_channel or self.mask_observation_with_visibility:
            ...
            if self.mask_observation_with_visibility:
                ...
                upper = self.num_obs_channels + (1 if self.include_trust_channel else 0) + (1 if self.include_trust_summary else 0)
    ...
    def _initialize_predator_trust_state(self):
        predator_ids = [pid for pid in self.agents if "predator" in pid]
        for pid in predator_ids:
            self.predator_trust.setdefault(pid, {})
        for pid in predator_ids:
            for other in predator_ids:
                if pid == other:
                    continue
                self.predator_trust[pid].setdefault(other, self.trust_neutral)

    def _ensure_predator_trust_entry(self, predator_id: str):
        self.predator_trust.setdefault(predator_id, {})
        for other in list(self.predator_trust.keys()):
            if other == predator_id:
                continue
            self.predator_trust[predator_id].setdefault(other, self.trust_neutral)
            self.predator_trust.setdefault(other, {})
            self.predator_trust[other].setdefault(predator_id, self.trust_neutral)

    def _get_trust_value(self, observer_id: str, target_id: str) -> float:
        if not self.trust_enabled or observer_id == target_id:
            return self.trust_neutral
        return float(self.predator_trust.get(observer_id, {}).get(target_id, self.trust_neutral))

    def _adjust_trust(self, observer_id: str, target_id: str, delta: float):
        if not self.trust_enabled or observer_id == target_id:
            return
        self._ensure_predator_trust_entry(observer_id)
        self._ensure_predator_trust_entry(target_id)
        current = self.predator_trust[observer_id].get(target_id, self.trust_neutral)
        self.predator_trust[observer_id][target_id] = float(np.clip(current + delta, 0.0, 1.0))

    def _decay_predator_trust(self):
        if not self.trust_enabled or self.trust_decay <= 0.0:
            return
        keep = 1.0 - self.trust_decay
        for observer_id, targets in self.predator_trust.items():
            for target_id, value in targets.items():
                if observer_id == target_id:
                    continue
                targets[target_id] = float(np.clip(self.trust_neutral + (value - self.trust_neutral) * keep, 0.0, 1.0))

    def _predators_near_predator(self, predator_id: str, include_self: bool = False):
        center = self.agent_positions.get(predator_id)
        if center is None:
            return []
        nearby = self._predators_in_moore_neighborhood(center)
        if include_self:
            return nearby
        return [pid for pid in nearby if pid != predator_id]

    def _select_share_recipient(self, predator_id: str, candidates):
        if not candidates:
            return None
        return max(candidates, key=lambda pid: (self._get_trust_value(predator_id, pid), pid))

    def _compute_trust_summary(self, predator_id: str) -> float:
        center = self.agent_positions.get(predator_id)
        if center is None:
            return self.trust_neutral
        cx, cy = center
        values = []
        for other_id, pos in self.predator_positions.items():
            if other_id == predator_id:
                continue
            if max(abs(pos[0] - cx), abs(pos[1] - cy)) <= self.trust_visibility_range:
                values.append(self._get_trust_value(predator_id, other_id))
        if not values:
            return self.trust_neutral
    ...
    def _handle_team_capture(self, prey_id):
        ...
        nearby_predators = [h for h in self._predators_in_moore_neighborhood(prey_pos) if h not in self.agents_just_ate]
        if not nearby_predators:
            return False

        prey_energy = float(self.agent_energies[prey_id])
        eligible_predators = [
            pid for pid in nearby_predators if self.agent_energies.get(pid, 0.0) > prey_energy + self.team_capture_margin
        ]
        if not eligible_predators:
            return False

        captor = max(eligible_predators, key=lambda pid: (self.agent_energies[pid], pid))
        helper_count = 1
        self.team_capture_successes += 1
        self.team_capture_helper_total += helper_count

        self.agents_just_ate.add(captor)
        self._per_agent_step_deltas.setdefault(captor, {"decay": 0.0, "move": 0.0, "eat": 0.0, "repro": 0.0})

        potential_recipients = [pid for pid in nearby_predators if pid != captor]
        share_opportunity = self.direct_reciprocity_enabled and bool(potential_recipients)
        share_amount = 0.0
        recipient = None
        ...
        self._per_agent_step_deltas[captor]["eat"] += captor_gain
        self.grid_world_state[1, *self.agent_positions[captor]] = self.agent_energies[captor]
        self.rewards[captor] = self.rewards.get(captor, 0.0)

        captor_record = self.agent_stats_live.get(captor)
        if captor_record is not None:
            captor_record["times_ate"] += 1
            captor_record["energy_gained"] += captor_gain

        captor_info = self._pending_infos.setdefault(captor, {})
        captor_info["team_capture_helpers"] = helper_count
        captor_info["team_capture_energy_gain"] = captor_gain
        ...
        if captor_evt is not None:
            captor_evt.setdefault("eating_events", []).append(
                {
                    "t": int(self.current_step),
                    "id_resource": prey_id,
                    "energy_resource": prey_energy,
                    "position_resource": tuple(prey_pos),
                    "position_consumer": tuple(self.agent_positions[captor]),
                    "bite_size": float(captor_gain),
        ...
        if recipient is not None and share_amount > 0.0:
            self.share_events_total += 1
            self._per_agent_step_deltas.setdefault(recipient, {"decay": 0.0, "move": 0.0, "eat": 0.0, "repro": 0.0})
            self.agent_energies[recipient] += share_amount
            self._per_agent_step_deltas[recipient]["eat"] += share_amount
            self.grid_world_state[1, *self.agent_positions[recipient]] = self.agent_energies[recipient]
            self.rewards[recipient] = self.rewards.get(recipient, 0.0)
            self._adjust_trust(recipient, captor, self.trust_positive_delta)

            recipient_record = self.agent_stats_live.get(recipient)
            if recipient_record is not None:
                recipient_record["energy_gained"] += share_amount

            recipient_info = self._pending_infos.setdefault(recipient, {})
            recipient_info["received_shared_energy"] = float(share_amount)
            recipient_info["shared_by"] = captor

            recipient_evt = self.agent_event_log.get(recipient)
            if recipient_evt is not None:
                recipient_evt.setdefault("diet_events", []).append(
                    {
                        "t": int(self.current_step),
                        "kind": "received_share",
                        "from_agent": captor,
                        "energy_gained": float(share_amount),
                        "resource_id": prey_id,
                    }
                )
        ...
        global_info["team_capture_coop_successes"] = self.team_capture_coop_successes
        global_info["team_capture_coop_failures"] = self.team_capture_coop_failures
        global_info["last_share_amount"] = float(share_amount)
        global_info["last_share_recipient"] = recipient
    ...
    def get_state_snapshot(self):
        return {
            ...
            "predator_share_intent": self.predator_share_intent.copy(),
            "predator_trust": {
                observer: targets.copy() for observer, targets in self.predator_trust.items()
            },
            "share_events_total": self.share_events_total,
            "share_opportunities_total": self.share_opportunities_total,
            "share_refusals_total": self.share_refusals_total,
        }
    ...
    # cumulative_rewards is now stored directly in agent_stats_live/agent_stats_completed
        ...
        self.predator_share_intent = snapshot.get("predator_share_intent", {}).copy()
        self.predator_trust = {
            observer: targets.copy() for observer, targets in snapshot.get("predator_trust", {}).items()
        }
        self.share_events_total = snapshot.get("share_events_total", 0)
        self.share_opportunities_total = snapshot.get("share_opportunities_total", 0)
        self.share_refusals_total = snapshot.get("share_refusals_total", 0)
    ...
    def _build_observation_space(self, agent_id):
        ...
        """
        extra = self._observation_channel_count() - self.num_obs_channels
    ...
    def _register_new_agent(self, agent_id: str, parent_agent_id: Optional[str] = None, *, is_founder: bool = False):
        ...
        if "predator" in agent_id:
            self.predator_share_intent[agent_id] = False
            self._ensure_predator_trust_entry(agent_id)
    ...
    def _remove_agent_from_state(self, agent_id: str):
        ...
        if "predator" in agent_id:
            ...
            self.predator_share_intent.pop(agent_id, None)
        elif "prey" in agent_id:
