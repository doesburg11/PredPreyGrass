...
"""
"""Random policy viewer with wall visualization (malthusian_rl variant).

This script intentionally imports the local malthusian_rl environment & renderer
so that the `walls` parameter on `PyGameRenderer.update` is available. If you
accidentally import the older malthusian_rl renderer, the call with `walls=`
