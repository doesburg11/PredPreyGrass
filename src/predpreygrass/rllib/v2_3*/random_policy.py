...
from predpreygrass.rllib.v2_3.config.config_env_random import config_env
from predpreygrass.utils.pygame_renderer_rllib import PyGameRenderer
...
if __name__ == "__main__":
    ...
    while not terminated and not truncated:
        ...
        # --- Update visualizer ---
        agent_positions = {aid: data["position"] for aid, data in env.active_agents.items()}

        agent_energies = {aid: data["energy"] for aid, data in env.active_agents.items()}
        grass_positions = {
            obj_id: obj["position"]
            for obj_id, obj in env.active_objects.items()
            if "grass" in obj_id
        }
