...
class EpisodeReturn(RLlibCallback):
    ...
    def on_episode_step(self, *, episode, **kwargs):
        ...
        # Get human-readable episode number (if not yet assigned, assign -1 as fallback)
        ep_num = self.episode_id_to_number.get(eid, -1)
        # Write step number with a unique marker to a dedicated log file
        try:
            with open("logs/step_numbers.log", "a") as f:
                f.write(f"[RL_STEP_NUM] STEP {step_num} (Episode {eid}, ep_num={ep_num})\n")
        except Exception as e:
            # Fallback to print if file write fails
            print(f"[RL_STEP_NUM] STEP {step_num} (Episode {eid}, ep_num={ep_num}) [file write error: {e}]", flush=True)
            ...
            if terminateds and isinstance(terminateds, dict):
                for agent_id, term_val in terminateds.items():
                    ...
                    if term_val is True or term_val == 1:
                        try:
                            with open("logs/agent_terminated_events.log", "a") as tf:
                                tf.write(f"[AGENT_TERMINATED] agent_id={agent_id} episode={eid} ep_num={ep_num} step={step_num}\n")
                        except Exception as te:
                            print(f"[AGENT_TERMINATED] agent_id={agent_id} episode={eid} ep_num={ep_num} step={step_num} [file write error: {te}]", flush=True)
        except Exception as e:
