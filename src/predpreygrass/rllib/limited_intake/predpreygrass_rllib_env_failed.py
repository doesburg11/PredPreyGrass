...
class PredPreyGrass(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        self._apply_energy_decay_and_age_update_per_step()
        ...
        # Step 6: (DEFERRED) Handle agent removals -- see after output dicts
        ...
        # Consolidate agent sets for RLlib protocol-compliant outputs
        agents_end = set(self.agents)
        deactivated_this_step = [a for a in agents_at_start if a not in agents_end]


        # 8a) Observations: only for active agents at end of step
        # RLlib protocol: output dicts must include all agents present at start of step (including those just terminated), plus any new agents spawned this step
        agents_end = set(self.agents)
        agents_start = set(agents_at_start)
        new_agents_this_step = [a for a in self.agents if a not in agents_start]
        deactivated_this_step = [a for a in agents_at_start if a not in agents_end]
        agents_considered = list(dict.fromkeys(agents_at_start + new_agents_this_step))  # preserve order

        # 8a) Observations: only for agents still active at end of step
        observations = {agent: observations[agent] for agent in self.agents if agent in observations}

        # 8b) Rewards: for all agents under consideration; fill missing with 0.0
        rewards = {agent: rewards.get(agent, 0.0) for agent in agents_considered}

        # 8c) Terminations/Truncations: for all agents under consideration
        terminations = {
            agent: (terminations.get(agent, False) or (agent in deactivated_this_step))
            for agent in agents_considered
        }
        truncations = {agent: truncations.get(agent, False) for agent in agents_considered}
        # Global flags
        truncations["__all__"] = False  # Early truncation handled at beginning if needed
        terminations["__all__"] = self.active_num_prey <= 0 or self.active_num_predators <= 0

        # 8d) Infos: same set as rewards/terminations
        infos = {agent: self._pending_infos.get(agent, {}) for agent in agents_considered}
        ...
        for agent in self.agents:
            # Only include agents present in all required dicts
            if (agent not in self.agent_positions or agent not in self.agent_energies or
                agent not in self.agent_ages or agent not in self.agent_offspring_counts or
                agent not in self.agent_live_offspring_ids):
                continue
        ...
        # Now, after output dicts are constructed, remove deactivated agents
        for agent in deactivated_this_step:
            if agent in self.agents:
                self.agents.remove(agent)
            # Remove from all agent state dicts to prevent KeyError
            for d in [self.agent_energies, self.agent_positions, self.agent_ages, self.agent_parents,
                      self.agent_offspring_counts, self.agent_live_offspring_ids]:
                if agent in d:
                    del d[agent]
            if agent in self.unique_agents:
                uid = self.unique_agents[agent]
                self.death_agents_stats[uid] = {
                    **self.unique_agent_stats[uid],
                    "lifetime": self.agent_ages.get(agent, None),
                    "parent": self.agent_parents.get(agent, None),
                }
    ...
    def _apply_energy_decay_and_age_update_per_step(self):
        """
        Apply fixed per-step energy decay + age update to all active agents based on type.
        """
        for agent in self.agents:
            if "predator" in agent:
    ...
    def _handle_predator_reproduction(self, agent, rewards, observations, terminations, truncations):
        ...
        if self.agent_energies[agent] >= self.predator_creation_energy_threshold:
            ...
            # Suppose self.potential_new_ids is a list of all available agent IDs (predators and prey)
            potential_new_ids = [aid for aid in self.potential_new_ids if aid.startswith(f"type_{new_type}_predator_")]
            ...
            self.potential_new_ids.remove(new_agent)
    ...
    def _handle_prey_reproduction(self, agent, rewards, observations, terminations, truncations):
        ...
        if self.agent_energies[agent] >= self.prey_creation_energy_threshold:
            ...
            # Find available new agent ID
            potential_new_ids = [aid for aid in self.potential_new_ids if aid.startswith(f"type_{new_type}_prey_")]
            if not potential_new_ids:
                # Always grant reproduction reward, even if no slot available
