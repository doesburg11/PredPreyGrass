...
from  predpreygrass.rllib.v7_move.config.config_env import config_env
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        trunc_reached, observations, rewards, terminations, truncations = self._check_truncation()
        if trunc_reached:
            return observations, rewards, terminations, truncations, {}

        # Step 1: Graas regeneration
        self.grass_energies = self._compute_new_grass_energies(...)

        # Fast in-place grid update
        for g, pos in self.grass_positions.items():
            self.grid_world_state[3, pos[0], pos[1]] = self.grass_energies[g]
        ...
        # Step 1: Process movement, total energy decay (fixed + movement), and age update
        for agent, action in action_dict.items():
            ...
            # Total energy cost includes base decay
            ...
            if self.verbose_movement:
                self._log(
                    ...
                    f"Cost: {move_cost:.2f} | Energy: {self.agent_energies[agent]:.2f}",
                    "cyan"
                )
            ...
            if self.agent_energies[agent] <= 0:
                ...
                if terminations.get(agent) is None or not terminations[agent]:
                    ...
                    if caught_grass:
                        self._log(
                            self.verbose_engagement,
                            f"[ENGAGE] {agent} caught grass at {tuple(map(int, prey_position))} Prey Reward: {self.reward_prey_eat_grass}",
                            "black"
                        )
            ...
            if "predator" in agent:
                if self.agent_energies[agent] >= self.predator_creation_energy_threshold:
                    ...
                    if not potential_new_ids:
                        self._log(
                            ...
                            f"No available predator slots at speed {new_speed} for spawning"
                            "red"
                        )
                        continue # TODO neverheless agent needs reproduction reward
                ...
                if self.agent_energies[agent] >= self.prey_creation_energy_threshold:
                    ...
                    if not potential_new_ids:
                        self._log(
                            self.verbose_reproduction,
                            f"No available prey slots at speed {new_speed} for spawning",
                            "red"
                        )
                        continue # TODO neverheless agent needs reproduction reward
    ...
    def _get_movement_energy_cost(self, agent, old_position, new_position):
        """
        Calculate the total energy cost:
        - Fixed decay per step (based on agent type)
        - Movement energy cost (distance-based)
        """
        distance_factor = self.config.get("move_energy_cost_factor", 0.1)
        base_decay = self.energy_loss_per_step_predator if "predator" in agent else self.energy_loss_per_step_prey
        ...
        return base_decay + (distance * distance_factor * current_energy)
    ...
    def __log(self, verbose: bool, message: str, color: str = None):
        """
        Formatted logging with optional color and RLlib-style border.
        
        Args:
            message (str): The message (single or multi-line) to print.
            color (str, optional): One of: red, green, yellow, cyan, blue, magenta.
        """
        if not getattr(self, "verbose_mode_global", True):
            return
    ...
    def _check_truncation(self) -> Tuple[bool, dict, dict, dict, dict]:
        """
        Check for episode truncation due to reaching max steps.

        Returns:
            Tuple:
                - truncation_reached (bool)
                - observations (dict)
                - rewards (dict)
                - terminations (dict)
                - truncations (dict)
        ...
        return True, observations, rewards, terminations, truncations

    def _compute_new_grass_energies(self, grass_energies, energy_gain, max_energy):
        return {
            g: min(e + energy_gain, max_energy)
            for g, e in grass_energies.items()
        }
