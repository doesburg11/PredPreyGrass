...
Additional features:
-limited intake of predators and prey per time step
-if intake smallerethan resource, prey and grass stay alive
"""
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _handle_predator_engagement(self, agent):
        ...
        if caught_prey:
            ...
            intake_cap = float(self.config["max_energy_gain_per_prey"])  # acts as pred_max_intake
            bite = min(prey_energy, intake_cap)
            # Energy transfer (keep efficiency hook for future use)
            efficiency = float(self.config.get("energy_transfer_efficiency", 1.0))
            energy_gain = bite * efficiency
            # Update predator energy (respect absolute cap)
            self.agent_energies[agent] += energy_gain
            max_pred = float(self.config.get("max_energy_predator", float("inf")))
            if not np.isfinite(max_pred):
                max_pred = float("inf")
            self.agent_energies[agent] = min(float(self.agent_energies[agent]), max_pred)
            ...
            # attribution prey: limited-intake logic
            remaining_prey_energy = prey_energy - bite
            if remaining_prey_energy > 0:
                # Prey survives with reduced energy
                self.agent_energies[caught_prey] = remaining_prey_energy
                max_prey = float(self.config.get("max_energy_prey", float("inf")))
                if not np.isfinite(max_prey):
                    max_prey = float("inf")
                self.agent_energies[caught_prey] = min(float(self.agent_energies[caught_prey]), max_prey)
                self.grid_world_state[2, *self.agent_positions[caught_prey]] = self.agent_energies[caught_prey]
            else:
                # Prey fully eaten and removed
    ...
    def _handle_prey_engagement(self, agent):
        ...
        if caught_grass:
            ...
            intake_cap = float(self.config["max_energy_gain_per_grass"])  # acts as prey_max_intake
            ...
            if not np.isfinite(max_prey):
                max_prey = float("inf")
            self.agent_energies[agent] = min(float(self.agent_energies[agent]), max_prey)
            self._per_agent_step_deltas[agent]["eat"] = energy_gain
            self.grid_world_state[2, *prey_position] = self.agent_energies[agent]

            remaining_grass = grass_energy - bite
            if remaining_grass > 0:
                # Grass remains with reduced energy; will regrow in _regenerate_grass_energy
                self.grass_energies[caught_grass] = remaining_grass
                self.grid_world_state[3, *prey_position] = remaining_grass
            else:
                # Grass fully consumed now; will regrow from zero
