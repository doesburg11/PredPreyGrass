...
"""
"""Random policy viewer with wall visualization (walls_occlusion_vectorization_1 variant).

This script intentionally imports the local walls_occlusion_vectorization_1 environment & renderer
so that the `walls` parameter on `PyGameRenderer.update` is available. If you
accidentally import the older walls_occlusion_vectorization_1 renderer, the call with `walls=`
will raise a TypeError (unexpected keyword). Ensure the imports below stay
pointing at `walls_occlusion_vectorization_1` when using walls.
"""
...
from predpreygrass.rllib.walls_occlusion_vectorization_1.utils.pygame_grid_renderer_rllib import PyGameRenderer
