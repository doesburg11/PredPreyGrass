...
def build_module_spec(obs_space, act_space, policy_name: str = None):
    ...
    if hasattr(act_space, "n"):
        ...
        num_actions = int(sum(act_space.nvec))
        action_info = f"nvec={list(act_space.nvec)} (sum={num_actions})"
    else:
        num_actions = None
        action_info = None
    ...
    if policy_name is not None:
        ...
        print(
            f"[MODEL] {policy_name} → conv_filters={conv_filters}, "
            f"fcnet_hiddens={fcnet_hiddens}, fcnet_activation=relu"
        )
