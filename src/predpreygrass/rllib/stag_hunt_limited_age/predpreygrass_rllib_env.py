...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        # Age-related mortality (Gompertz–Makeham)
        self.age_mortality_enabled = bool(config.get("age_mortality_enabled", False))
        self.age_steps_per_year = float(config.get("age_steps_per_year", 1.0))
        self.age_mortality_A = config.get("age_mortality_A", {})
        self.age_mortality_B = config.get("age_mortality_B", {})
        self.age_mortality_C = config.get("age_mortality_C", {})
    ...
    def _apply_time_step_update(self):
        ...
        for agent in list(self.agents):
            ...
            # Age-related mortality applies only if the agent is still alive and has energy.
            if self.age_mortality_enabled and self.agent_energies.get(agent, 0.0) > 0.0:
                p_death = self._get_age_mortality_probability(agent)
                if p_death > 0.0 and self.rng.random() < p_death:
                    self._handle_energy_starvation(agent, cause="aged")
                    continue

    def _get_age_mortality_param(self, source, agent_id: str, default: float = 0.0) -> float:
    ...
    def _get_age_mortality_probability(self, agent_id: str) -> float:
        if not self.age_mortality_enabled or self.age_steps_per_year <= 0:
            return 0.0
        age_steps = self.agent_ages.get(agent_id)
        if age_steps is None:
            return 0.0
        age_years = age_steps / self.age_steps_per_year

        A = self._get_age_mortality_param(self.age_mortality_A, agent_id, 0.0)
        B = self._get_age_mortality_param(self.age_mortality_B, agent_id, 0.0)
        C = self._get_age_mortality_param(self.age_mortality_C, agent_id, 0.0)
        if A <= 0.0 and B <= 0.0:
            return 0.0

        try:
            exp_term = math.exp(C * age_years)
        except OverflowError:
            exp_term = float("inf")

        h = A + (B * exp_term)
        dt = 1.0 / self.age_steps_per_year
        p = 1.0 - math.exp(-h * dt)
        if p < 0.0:
            return 0.0
        if p > 1.0:
