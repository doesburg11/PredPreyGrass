...
## 4) How to turn it on (implemented)

- Toggle `reputation_enabled: True` plus either a rolling `reputation_window` (default 50) or EMA (`reputation_ema_alpha`, use `reputation_window <= 0`).
- Expose the signal with `include_reputation_channel: True` (spatial map) and/or `include_reputation_summary: True` (per-agent summary channel).
- Default visibility range for summaries matches `predator_obs_range`; override with `reputation_visibility_range`.
- Keep it lightweight: by default the feature is off and reputations start neutral (0.5) until `reputation_min_samples` (default 5) join/defect opportunities are seen.
