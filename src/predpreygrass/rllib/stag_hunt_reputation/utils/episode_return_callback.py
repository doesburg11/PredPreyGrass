...
class EpisodeReturn(RLlibCallback):
    ...
    def on_episode_end(self, *, episode, metrics_logger: MetricsLogger, **kwargs):
        ...
        # ---- Reputation-conditioned metrics ----
        rep_join_high = rep_join_high_total = rep_join_low = rep_join_low_total = 0
        capture_rep_stats = None
        env = self._resolve_env(episode, **kwargs)
        if env is not None:
            step_data_list = getattr(env, "per_step_agent_data", None)
            if isinstance(step_data_list, list):
                for step_data in step_data_list:
                    if not isinstance(step_data, dict):
                        continue
                    for aid, data in step_data.items():
                        if "predator" not in aid or not isinstance(data, dict):
                            continue
                        rep_val = data.get("reputation_mean_in_range", data.get("reputation"))
                        if rep_val is None:
                            continue
                        joined = data.get("join_hunt")
                        if joined is None:
                            continue
                        if rep_val >= 0.6:
                            rep_join_high_total += 1
                            if joined:
                                rep_join_high += 1
                        else:
                            rep_join_low_total += 1
                            if joined:
                                rep_join_low += 1

            # Capture success/free-rider exposure vs partner reputation
            event_log = getattr(env, "agent_event_log", None)
            rep_scores = getattr(env, "predator_reputation_scores", {}) or {}
            if isinstance(event_log, dict):
                captures = {}
                for pid, record in event_log.items():
                    if "predator" not in pid or not isinstance(record, dict):
                        continue
                    for evt in record.get("eating_events", []) or []:
                        ...
                        if evt.get("join_hunt", True):
                            entry["joiners"].add(pid)
                        else:
                            entry["free_riders"].add(pid)
                capture_rep_stats = {"threshold": 0.6, "high": {"captures": 0, "free_rider_exposures": 0}, "low": {"captures": 0, "free_rider_exposures": 0}}
                for entry in captures.values():
                    if not entry["joiners"]:
                        continue
                    mean_rep = sum(float(rep_scores.get(j, 0.5)) for j in entry["joiners"]) / len(entry["joiners"])
                    bucket = "high" if mean_rep >= 0.6 else "low"
                    capture_rep_stats[bucket]["captures"] += 1
                    if entry["free_riders"]:
                        capture_rep_stats[bucket]["free_rider_exposures"] += 1
