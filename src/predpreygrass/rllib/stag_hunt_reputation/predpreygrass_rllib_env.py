...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        # Reputation config (predators only)
        self.reputation_enabled = bool(config.get("reputation_enabled", False))
        self.reputation_window = max(0, int(config.get("reputation_window", 50)))
        self.reputation_ema_alpha = float(config.get("reputation_ema_alpha", 0.1))
        self.reputation_use_ema = self.reputation_window <= 0
        self.reputation_opportunity_only = bool(config.get("reputation_opportunity_only", True))
        self.reputation_min_samples = int(config.get("reputation_min_samples", 5))
        self.reputation_noise_std = float(config.get("reputation_noise_std", 0.0))
        self.include_reputation_channel = bool(config.get("include_reputation_channel", False))
        self.include_reputation_summary = bool(config.get("include_reputation_summary", False))
        self.reputation_visibility_range = config.get("reputation_visibility_range", None)
        self.reputation_channel = None
        self.reputation_summary_channel = None
        ...
        # Add optional reputation channels after ensuring base channels exist
        if self.include_reputation_channel:
            self.reputation_channel = self.num_obs_channels
            self.num_obs_channels += 1
        if self.include_reputation_summary:
            self.reputation_summary_channel = self.num_obs_channels
            self.num_obs_channels += 1
        # Default visibility range for reputation summaries matches predator obs range if not provided.
        self.reputation_visibility_range = (
            self.predator_obs_range if self.reputation_visibility_range is None else int(self.reputation_visibility_range)
        )
    ...
    def _init_reset_variables(self, seed):
        ...
        self.active_num_predators = 0
        self.active_num_prey = 0
        # Reputation tracking (predators)
        self.predator_reputation_scores = {}
        self.predator_reputation_counts = {}
        ...
        for agent in self.agents:
            ...
            if "predator" in agent:
                ...
                if self.reputation_enabled or self.include_reputation_channel or self.include_reputation_summary:
                    info["reputation"] = self._get_reputation_value(agent)
                    if self.include_reputation_summary:
                        info["reputation_mean_in_range"] = self._compute_reputation_summary(agent)
        ...
        for agent in self.agents:
            ...
            if "predator" in agent:
                ...
                if self.reputation_enabled or self.include_reputation_channel or self.include_reputation_summary:
                    ...
                    if self.include_reputation_summary:
                        step_data[agent]["reputation_mean_in_range"] = self._compute_reputation_summary(agent)
    ...
    def _record_reputation(self, predator_id: str, joined: bool):
        """Update reputation score for a predator based on join/defect choice."""
        if predator_id not in self.predator_reputation_scores:
            self.predator_reputation_scores[predator_id] = 0.5
            self.predator_reputation_counts[predator_id] = 0
            if not self.reputation_use_ema:
                self.predator_reputation_history[predator_id] = deque(
                    maxlen=max(1, self.reputation_window or 1)
                )

        if self.reputation_use_ema:
            prev = self.predator_reputation_scores.get(predator_id, 0.5)
            alpha = self.reputation_ema_alpha
            new_rep = (1 - alpha) * prev + alpha * float(joined)
            self.predator_reputation_scores[predator_id] = new_rep
            self.predator_reputation_counts[predator_id] = self.predator_reputation_counts.get(predator_id, 0) + 1
        else:
            hist = self.predator_reputation_history.setdefault(
                predator_id, deque(maxlen=max(1, self.reputation_window or 1))
            )
            hist.append(float(joined))
            self.predator_reputation_counts[predator_id] = len(hist)
            if hist:
                self.predator_reputation_scores[predator_id] = float(sum(hist) / len(hist))

    def _get_reputation_value(self, predator_id: str) -> float:
        rep = float(self.predator_reputation_scores.get(predator_id, 0.5))
        count = int(self.predator_reputation_counts.get(predator_id, 0))
        if self.reputation_enabled and count < self.reputation_min_samples:
            return 0.5
        return float(np.clip(rep, 0.0, 1.0))

    def _refresh_reputation_channel(self):
        if not self.include_reputation_channel:
            return
        ch = self.reputation_channel
        self.grid_world_state[ch, :, :] = 0.0
        for pid, pos in self.predator_positions.items():
            if self.terminations.get(pid):
                continue
            rep_val = self._get_reputation_value(pid)
            if self.reputation_noise_std > 0.0:
                rep_val = float(
                    np.clip(rep_val + np.random.normal(0.0, self.reputation_noise_std), 0.0, 1.0)
                )
            self.grid_world_state[ch, pos[0], pos[1]] = rep_val

    def _compute_reputation_summary(self, agent_id: str) -> float:
        """Return mean reputation of predators within visibility range (including self)."""
        ...
        for pid, pos in self.predator_positions.items():
            if self.terminations.get(pid):
                continue
            if max(abs(pos[0] - cx), abs(pos[1] - cy)) <= self.reputation_visibility_range:
                reps.append(self._get_reputation_value(pid))
        if not reps:
            return 0.0
        return float(sum(reps) / len(reps))

    def _update_reputation_after_actions(self):
        """Update per-predator reputation using join decisions for this step."""
        if not (self.reputation_enabled or self.include_reputation_channel or self.include_reputation_summary):
            return
        for pid, pos in self.predator_positions.items():
            if self.terminations.get(pid):
                continue
            if pid not in self.predator_join_intent:
                continue  # newly spawned or inactive this step
            if self.reputation_opportunity_only and not self._has_prey_neighbor(pos):
                continue
            self._record_reputation(pid, bool(self.predator_join_intent.get(pid, True)))
    ...
    def _register_new_agent(self, agent_id: str, parent_agent_id: Optional[str] = None, *, is_founder: bool = False):
        ...
        if "predator" in agent_id:
            self.predator_reputation_scores.setdefault(agent_id, 0.5)
            self.predator_reputation_counts.setdefault(agent_id, 0)
            if not self.reputation_use_ema:
                self.predator_reputation_history.setdefault(
                    agent_id, deque(maxlen=max(1, self.reputation_window or 1))
                )
