...
This directory `stag_hunt_reputation` mirrors `stag_hunt`. Key files:

- `predpreygrass_rllib_env.py`: defection-enabled environment.
- `config/config_env_stag_hunt_reputation.py`: default env config with defection knobs.
...
All imports inside this module point to `predpreygrass.rllib.stag_hunt_reputation.*`
so you can run it without touching the original module. Default `ray_results`
paths also point at `.../stag_hunt_reputation/ray_results/`.
...
| Defection metrics | Not defined | Join/defect rates and free-rider exposure tracked | `utils/reputation_metrics.py`, `EpisodeReturn` |
...
Defined in `config/config_env_stag_hunt_reputation.py`:
...
Use the helper script in `utils/reputation_metrics.py` to summarize metrics from a
short rollout (defaults come from `config_env_stag_hunt_reputation.py`):

```bash
PYTHONPATH=src python -m predpreygrass.rllib.stag_hunt_reputation.utils.reputation_metrics
```
...
- Train: `python src/predpreygrass/rllib/stag_hunt_reputation/tune_ppo.py`
- Random rollouts: `python src/predpreygrass/rllib/stag_hunt_reputation/random_policy.py`
...
```bash
# Train
PYTHONPATH=src python src/predpreygrass/rllib/stag_hunt_reputation/tune_ppo.py

# Random rollout viewer
PYTHONPATH=src python src/predpreygrass/rllib/stag_hunt_reputation/random_policy.py

# Evaluate a checkpoint (debug viewer)
PYTHONPATH=src python src/predpreygrass/rllib/stag_hunt_reputation/evaluate_ppo_from_checkpoint_debug.py

# Evaluate multiple runs (batch)
PYTHONPATH=src python src/predpreygrass/rllib/stag_hunt_reputation/evaluate_ppo_from_checkpoint_multi_runs.py
```
