...
try:
...
except ImportError:  # Lightweight fallback so we can benchmark without Ray installed
    ...
    class MultiAgentEnv:  # type: ignore
        pass
    # Provide a simple alias compatible with usage below
    Tuple = _TypingTuple
import numpy as np
import math
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _initialize_from_config(self):
        ...
        # Micro-optimization flag: avoid sorting agents each step unless explicitly desired for deterministic debug output
        self.sort_agents_each_step = config.get("sort_agents_each_step", False)
        # Step 2 experimental array core flag (initial: only energy decay vectorized)
        self.use_array_core = config.get("use_array_core", False)
        # If reproduction occurs while array core active (not yet supported), we fall back to dict mode
        self._array_core_active = False
    ...
    def _init_reset_variables(self, seed):
        ...
        # Position indices (position -> agent_id) to avoid repeated O(n) scans during engagements
        self.predator_position_index = {}
        self.prey_position_index = {}
        # Grass index is static in terms of positions (grass not removed, only energy changes)
        self.grass_position_index = {}
        ...
        # Potentially initialize array core structures (after agents list built)
        if self.use_array_core:
            self._init_array_core_structures()

    def reset(self, *, seed=None, options=None):
        """
        Reset the environment to its initial state.
        """
        # Ray MultiAgentEnv has reset; fallback stub may not.
        if hasattr(super(), 'reset'):
            try:
                super().reset(seed=seed)
            except TypeError:
                # Older ray versions may not accept seed kw
                super().reset()
        ...
        for i, agent in enumerate(predator_list):
            ...
            self.predator_position_index[pos] = agent
        ...
        for i, agent in enumerate(prey_list):
            ...
            self.cumulative_rewards[agent] = 0
            self.prey_position_index[pos] = agent
        ...
        for i, grass in enumerate(self.grass_agents):
            ...
            self.grass_position_index[pos] = grass
        ...
        # Build/refresh array core state post-reset
        if self.use_array_core:
            self._array_sync_from_dicts()
            self._array_core_active = True
        return observations, {}
    ...
    def step(self, action_dict):
        ...
        # Step 1: If not truncated; process energy depletion due to time steps
        if self.use_array_core and self._array_core_active:
            # If new agents appeared (reproduction), disable array core for remainder of episode (not yet supported)
            if len(self.agents) != self._array_core_capacity_active:
                self._array_core_active = False
                self._apply_energy_decay_per_step_dict(action_dict)
            else:
                self._array_apply_energy_decay(action_dict)
        else:
            self._apply_energy_decay_per_step_dict(action_dict)
        ...
        if self.use_array_core and self._array_core_active:
            self._array_apply_movements(action_dict)
        else:
            self._process_agent_movements(action_dict)

        # Rebuild fast lookup indices after movement (before engagements)
        self.predator_position_index = {pos: a for a, pos in self.predator_positions.items()}
        self.prey_position_index = {pos: a for a, pos in self.prey_positions.items()}

        # Step 5: Handle agent engagements
        if self.use_array_core and self._array_core_active:
            self._array_handle_engagements(observations, rewards, terminations, truncations)
        else:
        ...
        # Optional sorting (debug determinism only)
        if self.sort_agents_each_step:
            self.agents.sort()
    ...
    def _apply_energy_decay_per_step(self, action_dict):
        return self._apply_energy_decay_per_step_dict(action_dict)

    def _apply_energy_decay_per_step_dict(self, action_dict):
        """Original dict-based decay path (kept for fallback)."""
    ...
    # -------------------- Array Core (Step 2) scaffolding --------------------
    def _init_array_core_structures(self):
        """Allocate arrays sized by number of possible agents.
        For Step 2 phase 1 we only use this for energy decay; movement & reproduction remain dict-based.
        """
        n = len(self.possible_agents)
        self._array_core_capacity_total = n
        # Active slice length (agents alive at reset; reproduction not yet supported in array mode)
        self._array_core_capacity_active = 0
        self._ac_index_of_agent = {agent_id: idx for idx, agent_id in enumerate(self.possible_agents)}
        self._ac_agent_of_index = list(self.possible_agents)
        # Arrays
        self._ac_x = np.full(n, -1, dtype=np.int16)
        self._ac_y = np.full(n, -1, dtype=np.int16)
        self._ac_energy = np.zeros(n, dtype=np.float32)
        # Type code: 1 predator, 2 prey, 0 unused
        self._ac_type = np.zeros(n, dtype=np.uint8)
        self._ac_alive = np.zeros(n, dtype=np.bool_)

    def _array_sync_from_dicts(self):
        """Populate arrays from current dict state (only at reset for now)."""
        count = 0
        for agent in self.agents:
            idx = self._ac_index_of_agent[agent]
            if agent not in self.agent_positions:
                continue
            x, y = self.agent_positions[agent]
            self._ac_x[idx] = x
            self._ac_y[idx] = y
            self._ac_energy[idx] = self.agent_energies[agent]
            self._ac_type[idx] = 1 if 'predator' in agent else 2
            self._ac_alive[idx] = True
            count += 1
        self._array_core_capacity_active = count

    def _array_apply_energy_decay(self, action_dict):
        """Vectorized energy decay for agents that acted this step.
        Only processes agents in action_dict that were part of initial active set.
        If a reproduction added new agents, array core is disabled before this is called.
        """
        if self._array_core_capacity_active == 0:
            return
        # Build mask of indices to update
        indices = []
        predator_mask = []
        for agent in action_dict:
            if agent not in self.agent_positions:
                continue
            idx = self._ac_index_of_agent.get(agent)
            if idx is None or not self._ac_alive[idx]:
                continue
            indices.append(idx)
            predator_mask.append(1 if 'predator' in agent else 0)
        if not indices:
            return
        idx_arr = np.array(indices, dtype=np.int32)
        pred_mask = np.array(predator_mask, dtype=np.bool_)
        decay_values = np.where(pred_mask, self.energy_loss_per_step_predator, self.energy_loss_per_step_prey).astype(np.float32)
        # Apply decay
        self._ac_energy[idx_arr] -= decay_values
        # Sync back to dicts and per-agent deltas
        for local_i, global_idx in enumerate(idx_arr):
            agent = self._ac_agent_of_index[global_idx]
            new_energy = float(self._ac_energy[global_idx])
            self.agent_energies[agent] = new_energy
            layer = 1 if pred_mask[local_i] else 2
            self.grid_world_state[layer, self._ac_x[global_idx], self._ac_y[global_idx]] = new_energy
            self._per_agent_step_deltas[agent] = {"decay": -float(decay_values[local_i]), "move": 0.0, "eat": 0.0, "repro": 0.0}
            if self.verbose_decay:
                self._log(
                    self.verbose_decay,
                    f"[DECAY:A] {agent} energy -> {new_energy:.2f}",
                    "red",
                )

    def _array_handle_engagements(self, observations, rewards, terminations, truncations):
        """Array-mode engagements (predator-prey & prey-grass) using position hashing.
        Still executes Python loops but avoids per-agent method dispatch & repeated lookups.
        """
        # 1. Starvation removals first
        dead_agents = [a for a in self.agents if a in self.agent_positions and self.agent_energies[a] <= 0]
        for agent in dead_agents:
            self._handle_energy_decay(agent, observations, rewards, terminations, truncations)

        # Build quick mapping from prey positions -> prey agent id (after starvation removal)
        prey_pos_map = {pos: aid for aid, pos in self.prey_positions.items() if aid in self.agent_positions}

        # 2. Predator-prey engagements
        for predator, pos in list(self.predator_positions.items()):
            if predator not in self.agent_positions:
                continue
            prey = prey_pos_map.get(pos)
            if not prey:
                # step reward only
                rewards[predator] = self._get_type_specific("reward_predator_step", predator)
                self.cumulative_rewards.setdefault(predator, 0.0)
                self.cumulative_rewards[predator] += rewards[predator]
                observations[predator] = self._get_observation(predator)
                terminations[predator] = False
                truncations[predator] = False
                continue

            # Predator catches prey
            self.agents_just_ate.add(predator)
            rewards[predator] = self._get_type_specific("reward_predator_catch_prey", predator)
            self.cumulative_rewards.setdefault(predator, 0.0)
            self.cumulative_rewards[predator] += rewards[predator]

            raw_gain = min(self.agent_energies.get(prey, 0.0), self.config.get("max_energy_gain_per_prey", float("inf")))
            efficiency = self.config.get("energy_transfer_efficiency", 1.0)
            gain = raw_gain * efficiency
            self.agent_energies[predator] += gain
            self._per_agent_step_deltas[predator]["eat"] = gain
            max_energy = self.config.get("max_energy_predator", float("inf"))
            self.agent_energies[predator] = min(self.agent_energies[predator], max_energy)

            uid = self.unique_agents[predator]
            self.unique_agent_stats[uid]["times_ate"] += 1
            self.unique_agent_stats[uid]["energy_gained"] += raw_gain
            self.unique_agent_stats[uid]["cumulative_reward"] += rewards[predator]
            self.grid_world_state[1, *pos] = self.agent_energies[predator]

            # Prey termination
            rewards[prey] = self._get_type_specific("penalty_prey_caught", prey)
            self.cumulative_rewards.setdefault(prey, 0.0)
            self.cumulative_rewards[prey] += rewards[prey]
            terminations[prey] = True
            truncations[prey] = False
            self.active_num_prey -= 1
            self.grid_world_state[2, *self.agent_positions[prey]] = 0

            uid_prey = self.unique_agents[prey]
            stat = self.unique_agent_stats[uid_prey]
            stat["death_step"] = self.current_step
            stat["death_cause"] = "eaten"
            stat["final_energy"] = self.agent_energies[predator]
            steps = max(stat["avg_energy_steps"], 1)
            stat["avg_energy"] = stat["avg_energy_sum"] / steps
            stat["cumulative_reward"] = self.cumulative_rewards.get(prey, 0.0)
            self.death_agents_stats[uid_prey] = stat

            # Array core bookkeeping (mark prey dead)
            prey_idx = self._ac_index_of_agent.get(prey)
            if prey_idx is not None:
                self._ac_alive[prey_idx] = False
                # energy left as is; optional zero-out could be done
            # Capture prey observation BEFORE deleting state to mirror original logic
            observations[prey] = self._get_observation(prey)

            # Delete prey data structures
            del self.agent_positions[prey]
            del self.prey_positions[prey]
            del self.agent_energies[prey]

            # Predator observation after engagement
            observations[predator] = self._get_observation(predator)
            terminations[predator] = False
            truncations[predator] = False

        # 3. Prey-grass engagements (skip prey already terminated)
        # Build grass position index once (already exists) – reuse self.grass_position_index
        for prey, pos in list(self.prey_positions.items()):
            if prey not in self.agent_positions:
                continue
            grass_id = self.grass_position_index.get(pos)
            if grass_id and self.grass_energies.get(grass_id, 0) > 0:
                self.agents_just_ate.add(prey)
                rewards[prey] = self._get_type_specific("reward_prey_eat_grass", prey)
                self.cumulative_rewards.setdefault(prey, 0.0)
                self.cumulative_rewards[prey] += rewards[prey]
                raw_gain = min(self.grass_energies[grass_id], self.config.get("max_energy_gain_per_grass", float("inf")))
                efficiency = self.config.get("energy_transfer_efficiency", 1.0)
                gain = raw_gain * efficiency
                self.agent_energies[prey] += gain
                self._per_agent_step_deltas[prey]["eat"] = gain
                max_energy_prey = self.config.get("max_energy_prey", float("inf"))
                self.agent_energies[prey] = min(self.agent_energies[prey], max_energy_prey)
                uid = self.unique_agents[prey]
                self.unique_agent_stats[uid]["times_ate"] += 1
                self.unique_agent_stats[uid]["energy_gained"] += self.grass_energies[grass_id]
                self.unique_agent_stats[uid]["cumulative_reward"] += rewards[prey]
                self.grid_world_state[2, *pos] = self.agent_energies[prey]
                self.grid_world_state[3, *pos] = 0
                self.grass_energies[grass_id] = 0
                terminations[prey] = False
                truncations[prey] = False
            else:
                # step reward only
                rewards[prey] = self._get_type_specific("reward_prey_step", prey)
                self.cumulative_rewards.setdefault(prey, 0.0)
                self.cumulative_rewards[prey] += rewards[prey]
                uid = self.unique_agents[prey]
                self.unique_agent_stats[uid]["cumulative_reward"] += rewards[prey]
        ...
        # Ensure any predators without prey encounter still have observation if not already set
        for predator in self.predator_positions:
            if predator in self.agent_positions and predator not in observations:
                observations[predator] = self._get_observation(predator)
                if predator not in rewards:
                    rewards[predator] = self._get_type_specific("reward_predator_step", predator)
                    self.cumulative_rewards.setdefault(predator, 0.0)
                    self.cumulative_rewards[predator] += rewards[predator]
                terminations[predator] = False
    ...
    def _array_apply_movements(self, action_dict):
        """Array-core movement path (Step 3): still sequential for collision fidelity.
        We iterate agents with actions, compute candidate moves, apply collision rule, update arrays & dicts.
        """
        move_cost_factor = self.config.get("move_energy_cost_factor", 0.01)
        G = self.grid_size
        # Pre-calc movement maps as arrays (lazily) for faster indexing
        if not hasattr(self, "_ac_move_dx_type1"):
            # Build from existing dicts (ordered by action index)
            max_a1 = max(self.action_to_move_tuple_type_1_agents.keys())
            max_a2 = max(self.action_to_move_tuple_type_2_agents.keys())
            self._ac_move_dx_type1 = np.array([self.action_to_move_tuple_type_1_agents[i][0] for i in range(max_a1 + 1)], dtype=np.int8)
            self._ac_move_dy_type1 = np.array([self.action_to_move_tuple_type_1_agents[i][1] for i in range(max_a1 + 1)], dtype=np.int8)
            self._ac_move_dx_type2 = np.array([self.action_to_move_tuple_type_2_agents[i][0] for i in range(max_a2 + 1)], dtype=np.int8)
            self._ac_move_dy_type2 = np.array([self.action_to_move_tuple_type_2_agents[i][1] for i in range(max_a2 + 1)], dtype=np.int8)

        for agent, action in action_dict.items():
            idx = self._ac_index_of_agent.get(agent)
            if idx is None:
                continue
            if not self._ac_alive[idx]:
                continue
            # Current position
            x = int(self._ac_x[idx])
            y = int(self._ac_y[idx])
            agent_type_code = int(self._ac_type[idx])  # 1 predator, 2 prey

            # Get movement delta
            if agent_type_code == 1:  # predator uses type1 action space mapping (by design) or type-specific? We mirror original logic (type_1 vs type_2 embedded in id)
                # Actually differentiate by id: if 'type_2_' then use type2 mapping
                if 'type_2_' in agent:
                    dx = int(self._ac_move_dx_type2[action])
                    dy = int(self._ac_move_dy_type2[action])
                else:
                    dx = int(self._ac_move_dx_type1[action])
                    dy = int(self._ac_move_dy_type1[action])
            else:  # prey
                if 'type_2_' in agent:
                    dx = int(self._ac_move_dx_type2[action])
                    dy = int(self._ac_move_dy_type2[action])
            ...
            new_x = min(max(x + dx, 0), G - 1)
            new_y = min(max(y + dy, 0), G - 1)

            # Collision check: if target already has same type occupant energy>0 keep old position
            layer = 1 if agent_type_code == 1 else 2
            if self.grid_world_state[layer, new_x, new_y] > 0 and (new_x != x or new_y != y):
                new_x, new_y = x, y
                distance = 0.0
            else:
            ...
            current_energy = self._ac_energy[idx]
            move_cost = distance * move_cost_factor * current_energy
            if move_cost:
                self._ac_energy[idx] -= move_cost
                self.agent_energies[agent] = float(self._ac_energy[idx])
            # Update positions (arrays & dict proxies)
            if new_x != x or new_y != y:
                # Clear old grid cell
                self.grid_world_state[layer, x, y] = 0
                self._ac_x[idx] = new_x
                self._ac_y[idx] = new_y
                self.agent_positions[agent] = (new_x, new_y)
                if agent_type_code == 1:
                    self.predator_positions[agent] = (new_x, new_y)
                else:
                    self.prey_positions[agent] = (new_x, new_y)
            # Write new energy into grid cell (even if didn't move, refresh value)
            self.grid_world_state[layer, int(self._ac_x[idx]), int(self._ac_y[idx])] = self._ac_energy[idx]
            # Track deltas
            self._per_agent_step_deltas[agent]["move"] = -float(move_cost)
            if self.verbose_movement and distance > 0:
                self._log(
                    self.verbose_movement,
                    f"[MOVE:A] {agent} ({x},{y})->({new_x},{new_y}) d={distance:.2f} cost={move_cost:.3f} E={self._ac_energy[idx]:.2f}",
                    "blue",
                )
    ...
    def _handle_predator_engagement(self, agent, observations, rewards, terminations, truncations):
        ...
        caught_prey = self.prey_position_index.get(predator_position)
        ...
        if caught_prey:
            ...
            # Remove from fast index
            self.prey_position_index.pop(predator_position, None)
        else:
    ...
    def _handle_prey_engagement(self, agent, observations, rewards, terminations, truncations):
        ...
        # O(1) lookup using static position index (grass remains at fixed coordinates)
        caught_grass = self.grass_position_index.get(prey_position)

        if caught_grass and self.grass_energies.get(caught_grass, 0) > 0:
