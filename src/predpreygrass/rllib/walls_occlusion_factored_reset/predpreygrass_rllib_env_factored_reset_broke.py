...
Additional features:
- Improved engagement handling for catching prey and eating grass.
- Improved agent removal process by deleting all associated data in a single step.
- Cleaned up predator engagement handling for catching prey, removing unnecessary timing code.
- Cleaned up engagement handling for prey eating grass, removing unnecessary timing code.
"""
# external libraries
import gymnasium
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        for agent in self.agents[:]:
            if terminations.get(agent, False):
                print(f"\033[91m[TERMINATED] Agent {agent} removed from self.agents at step {self.current_step}!\033[0m")
    ...
    def _handle_energy_starvation(self, agent, observations, rewards, terminations, truncations):
        ...
        stat["death_cause"] = "starved"  # alternative is "eaten"
    ...
    def _handle_predator_engagement(self, agent, observations, rewards, terminations, truncations):
        ...
        if caught_prey:
            ...
            self.death_agents_stats[uid] = stat
            print(f"\033[92m[ENGAGE] {agent} caught {caught_prey} at step {self.current_step} at position {predator_position}!\033[0m")
        else:
