...
class PredPreyGrass:
    def __init__(
        ...
        self.show_agent_id_nrs: bool = False  # show agent id numbers in visualization
        # visualization
    ...
    def reset(self):
        ...
        # create agents of all types excluding "wall"-agents and grass agents
        for agent_type_nr in range(1, len(self.agent_type_name_list)-1):
        ...
        x = np.linspace(0, self.x_grid_size - 1, self.x_grid_size)
        y = np.linspace(0, self.y_grid_size - 1, self.y_grid_size)
        x, y = np.meshgrid(x, y)


        # create grass agents in every cell of the grid
        # create the center of the Gaussian distribution that represents the grass distribution
        mean =(random.randint(0, self.x_grid_size-1), random.randint(0, self.y_grid_size-1))  # mean = center point 
        variance = float(random.randint(1, abs(self.x_grid_size)))
        print("variance ", variance)


        # std_dev =(random.randint(1, abs(self.x_grid_size)**0.5), random.randint(1, abs(self.y_grid_size)**0.5))   
        # Generate the 2D Gaussian distribution for the grass agents
        gaussian_distribution = np.exp(-((x - mean[0])**2 / (2 * variance) + (y - mean[1])**2 / (2 * variance)))

        #print("gaussian_distribution 1", gaussian_distribution)

        # Normalize the distribution so that it represents probabilities
        gaussian_distribution /= np.sum(gaussian_distribution)
        #print("gaussian_distribution 2", gaussian_distribution)
        ...
        for i in range(self.x_grid_size):
            for j in range(self.y_grid_size):
                agent_id_nr = self.agent_id_counter
                agent_name = "grass" + "_" + str(agent_id_nr)
                self.agent_id_counter += 1
                agent_instance = DiscreteAgent(
                    self.grass_type_nr,
                    agent_id_nr,
                    ...
                    initial_energy=self.initial_energy_list[self.grass_type_nr],
                    energy_gain_per_step=self.energy_gain_per_step_list[self.grass_type_nr],
                )

                print("[i,j] = [",i,",",j,"]")
                agent_instance.position = (i, j)
                if random.random() < factor * gaussian_distribution[i, j]:
                    agent_instance.is_active = True
                else:
                    agent_instance.is_active = False
                agent_instance.energy = self.initial_energy_list[self.grass_type_nr]
                self.model_state[self.grass_type_nr, i, j] = agent_instance.energy
                agent_type_instance_list[self.grass_type_nr].append(agent_instance)
                self.agent_instance_in_grid_location[
                    self.grass_type_nr, i, j
                ] = agent_instance
    ...
    def predator_catches_prey_records(self, predator_instance, x_new, y_new):
        ...
        self.agent_energy_from_eating_dict[predator_instance.agent_name] = prey_instance_removed.energy
    ...
    def render(self):
        ...
        def draw_predator_instances():
            for predator_instance in self.active_predator_instance_list:
                ...
                size_predator = self.cell_scale / 5 * abs(predator_instance.energy) ** 0.5

                pygame.draw.circle(self.screen, col, center, int(size_predator))  # type: ignore
        ...
        def draw_prey_instances():
            for prey_instance in self.active_prey_instance_list:
                ...
                size_prey = self.cell_scale / 5 * abs(prey_instance.energy) ** 0.5

                #pygame.draw.circle(self.screen, col, center, int(self.cell_scale / 2.3))  # type: ignore
                pygame.draw.circle(self.screen, col, center, int(size_prey))  # type: ignore
        ...
        def draw_grass_instances():
            for grass_instance in self.active_grass_instance_list:
                ...
                col = (0, 128, 0)  # green
                size_grass = self.cell_scale / 5 * abs(grass_instance.energy) ** 0.5

                pygame.draw.circle(self.screen, col, center, int(size_grass))  # type: ignore
