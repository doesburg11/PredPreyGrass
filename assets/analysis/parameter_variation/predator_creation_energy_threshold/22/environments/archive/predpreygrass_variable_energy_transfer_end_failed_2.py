...
TODO: add another agent "SUN" which shines on the grass and increases the energy of the grass
"SUN" can be a resource agent for the grass and be manually controlled by the user or randomly
...
class PredPreyGrass:
    def __init__(
        ...
        self.possible_learning_agent_name_list: List[AgentID] = []
        ...
        self.step_reward_agent_type: List[int] = [
            0,
        ...
        self.consume_reward_resource_list: List[int] = [
            0,
        ...
        self.possible_learning_agent_name_list = self.possible_predator_name_list + self.possible_prey_name_list
        ...
        self.get_energy_from_consuming_resource = dict(
        ...
        self.resource_to_be_removed_by_consumer_dict: Dict[AgentID, bool] = dict(
        ...
        self.consumer_who_removes_resource_dict: Dict[AgentID, bool] = dict(
            zip(self.possible_learning_agent_name_list, [False for _ in self.possible_learning_agent_name_list])
        )
        #print("self.consumer_who_removes_resource_dict", self.consumer_who_removes_resource_dict)
    ...
    def reset(self):
        ...
        self.active_prey_instance_list = agent_type_instance_list[self.prey_type_nr]
        self.active_grass_instance_list = agent_type_instance_list[self.grass_type_nr]
        ...
        self.possible_learning_agent = self.create_possible_agent_name_list_from_instance_list(
            self.active_grass_instance_list + self.create_possible_agent_name_list_from_instance_list(
                self.active_prey_instance_list)    
        )
        ...
        self.consumer_who_removes_resource_dict = dict(
            zip(self.possible_learning_agent_name_list, [False for _ in self.possible_learning_agent_name_list])
        )
        print("self.consumer_who_removes_resource_dict", self.consumer_who_removes_resource_dict)
            ...
            if agent_type_nr == self.predator_type_nr:
                if agent_instance.energy > 0:
                    ...
                    if is_prey_in_new_cell:
                        ...
                        #self.predator_catches_prey_records(agent_instance, x_new, y_new)
                        self.agent_consumes_resource(agent_instance)
                else:
            ...
            # If the agent is a prey and it's alive
                ...
                if agent_instance.energy > 0:
                    ...
                    if is_grass_in_new_cell:
                        ...
                        self.agent_consumes_resource(agent_instance)
                        #self.prey_eats_grass_records(agent_instance, x_new, y_new)
                else:
    ...
    def agent_consumes_resource(self, agent_instance):
        agent_instance_removed = self.agent_instance_in_grid_location[
            agent_instance.agent_type_nr+1, # resource agent has type_nr consumer + 1
            agent_instance.position[0],
            agent_instance.position[1],
        ]
        self.get_energy_from_consuming_resource[agent_instance.agent_name] = agent_instance_removed.energy
        self.consumer_who_removes_resource_dict[agent_instance.agent_name] = True
        self.resource_to_be_removed_by_consumer_dict[agent_instance_removed.agent_name] = True
 

    def predator_catches_prey_records(self, predator_instance, x_new, y_new):
        prey_instance_removed = self.agent_instance_in_grid_location[
            self.prey_type_nr][
            (x_new, y_new)
        ]
        # book keeping for last step of the cycle actions
        # TODO: change to: agent_who_eats_dict and agent_who_gets_eaten_dict? To generalize function for both predator and prey
        ...
        self.get_energy_from_consuming_resource[predator_instance.agent_name] = prey_instance_removed.energy

    def prey_eats_grass_records(self, prey_instance, x_new, y_new):
        grass_instance_removed = self.agent_instance_in_grid_location[self.grass_type_nr][
            (x_new, y_new)
        ]
        # book keeping for last step of the cycle actions
        ...
        self.get_energy_from_consuming_resource[prey_instance.agent_name] = grass_instance_removed.energy
    ...
    def create_new_predator(self, parent_predator):
        ...
        if non_active_predator_names:
            ...
            x_new, y_new = self.find_new_position(self.predator_type_nr)
            new_predator_instance.position = (x_new, y_new)
            self.agent_instance_in_grid_location[
                self.predator_type_nr, x_new, y_new
    ...
    def reward_agent(self, agent_instance):
        ...
        agent_type_nr = agent_instance.agent_type_nr
        self.agent_reward_dict[agent_name] += self.step_reward_agent_type[agent_type_nr]
        agent_instance.energy += self.energy_gain_per_step_list[agent_type_nr]
        print("self.comsumer_who_removes_resource_dict",self.consumer_who_removes_resource_dict)    
        self.agent_reward_dict[agent_name] += (
            self.consume_reward_resource_list[agent_type_nr] * self.consumer_who_removes_resource_dict[agent_name]
        )
        rew=self.agent_reward_dict[agent_name]
        print("reward 2",rew)
        self.model_state[
            agent_instance.agent_type_nr,
            agent_instance.position[0],
    ...
    def reward_predator(self, predator_instance):
        ...
        rew=self.agent_reward_dict[predator_name]
        print("reward 1",rew)
        predator_instance.energy += self.energy_gain_per_step_predator
        predator_instance.energy += (
            self.get_energy_from_consuming_resource[predator_name]
        )
        self.model_state[
            self.predator_type_nr,
            predator_instance.position[0],
    ...
    def reward_prey(self, prey_instance):
        ...
        prey_instance.energy += (
            self.get_energy_from_consuming_resource[prey_name]
        )
        self.model_state[
            self.prey_type_nr,
            prey_instance.position[0],
    ...
    def reset_removal_records(self):
        ...
        self.resource_to_be_removed_by_consumer_dict = dict(
            zip(
                self.possible_learning_agent_name_list,
                [False for _ in self.possible_learning_agent_name_list],
            )
        )
