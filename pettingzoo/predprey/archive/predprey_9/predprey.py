...
class DiscreteAgent():
    def __init__(
        ...
        self.terminal = False
        self.observation_range = obs_range
        self.observation_shape = (n_channels * obs_range**2 + 1,) if flatten else (obs_range, obs_range, n_channels)
...
class PredPrey:
    def __init__(
        ...
        # lists of agents
        self.n_agents = self.n_prey + self.n_grasses # + self.n_predators
    ...
    def reset(self):
        ...
        """
        self.prey_instance_list = self.create_agents(
            4, self.prey_type_nr, 7, self.np_random, moore_neighborhood=True
        )
        self.prey_instance_list = self.create_agents(
            4, self.prey_type_nr, 5, self.np_random, moore_neighborhood=False
        )
        """
    ...
    def safely_observe(self, agent_id):
        ...
        for index in range(n_agents):
            if agent_id == index:
                ...
                observation = np.zeros((self.nr_observation_channels, self.obs_range, self.obs_range), dtype=np.float32)
...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #self.agents = self.create_agent_name_list()
