...
class PredPrey:
    ...
    def reset(self):
        ...
        # initialization
        self.n_predator: int = self.n_initial_predator
        self.n_prey: int = self.n_initial_prey  
        self.n_grass: int = self.n_initial_grass
        ...
        agent_type_name = self.agent_type_names[self.predator_type_nr]
        for _ in range(self.n_predator): 
            agent_id_nr = self.agent_id_counter           
            agent_name = agent_type_name + "_" + str(agent_id_nr)
            self.agent_id_counter+=1
            ...
            #  updates lists en records
            self.model_state[self.predator_type_nr, xinit, yinit] += 1
            self.agent_name_to_instance_dict[agent_name] = agent_instance
            agent_instance.position = (xinit, yinit)  
            self.predator_instance_list.append(agent_instance)


        agent_type_name = self.agent_type_names[self.prey_type_nr]
        for _ in range(self.n_prey): 
            ...
            # not implemented yet
            self.model_state[self.prey_type_nr, xinit, yinit] += 1
 
            self.agent_name_to_instance_dict[agent_name] = agent_instance
            agent_instance.position = (xinit, yinit)  
            self.prey_instance_list.append(agent_instance)


        agent_type_name = self.agent_type_names[self.grass_type_nr]
        for _ in range(self.n_grass): 
            ...
            agent_instance = DiscreteAgent(
                ...
                catch_grass_reward=self.catch_grass_reward,
                catch_prey_reward=self.catch_prey_reward,
                energy_loss_per_step=0
            )
            #  updates lists en records
 
            # not implemented yet
            self.model_state[self.grass_type_nr, xinit, yinit] += 1
 
            self.agent_name_to_instance_dict[agent_name] = agent_instance
            agent_instance.position = (xinit, yinit)  
            self.grass_instance_list.append(agent_instance)
