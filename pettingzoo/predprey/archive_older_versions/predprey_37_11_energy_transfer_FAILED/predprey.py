"""
[v37_11]
-Moore neighborhood out of input parameters
-cleanup alive arrays.
-rebuild wall agentm because it is part of the obeservation space. Could theoretically
be removed from the observation space in case of Torus environment
-remove self.motion_range builder loops



TODO Later
-change agent_observation with number of prey in observation range into energy of 
prey in observation range [use 38_energytransfer]
...
class PredPrey:
    def __init__(
        ...
        self.action_range = 3
        self.moore_neighborhood_actions = False
        ...
        # observations
        max_energy_in_channel_cell = 100
    ...
    def print_prey_in_location(self):
        for i in range(self.x_grid_size):
            for j in range(self.y_grid_size):
                if self.agent_instance_in_grid_location_dict['prey'][(i,j)]:
                    print("prey in location ", (i,j), " = ", self.create_agent_name_list_from_instance_list(self.agent_instance_in_grid_location_dict['prey'][(i,j)]))
    ...
    def reset(self):
        ...
        # lists for creating agents and looping over the agent types
        ...
        self.initial_energy_list = [0, self.initial_energy_predator, self.initial_energy_prey, 3]
        ...
        for agent_type_nr in range(1, len(self.agent_type_name_list)):
            ...
            for _ in range(self.n_agent_instance_list[agent_type_nr]): 
                ...
                xinit, yinit = (self.np_random.integers(0, self.x_grid_size), self.np_random.integers(0, self.y_grid_size))      
    ...
    def step(self, action, agent_instance, is_last):
        ...
        if agent_instance.is_alive:
            ...
            #print("aec cycle ", self.n_aec_cycles)
            #self.print_prey_in_location()            # If the agent is a predator 
            #print()
            if agent_type_nr == self.predator_type_nr: 
                if agent_energy > 0: # If predator has energy
                    ...
                    if prey_energy_at_new_spot > 0:
                        ...
                        print()  
                        print("prey_name_list_in_cell_predator ", prey_name_list_in_cell_predator)      
                        print()        
            ...
            # If the agent is a prey 
                ...
                if agent_energy > 0:  # If prey has energy
                    ...
                    print("STEP")
                    print("BEFORE: self.model_state[self.prey_type_nr] = ")
                    print(np.transpose(self.model_state[self.prey_type_nr]))
                    self.model_state[agent_type_nr, agent_instance.position[0], agent_instance.position[1]] += agent_instance.energy_loss_per_step
                    # agent leaves old position
                    self.model_state[agent_type_nr, old_position[0], old_position[1]] -= agent_instance.energy
                    ...
                    print("AFTER: self.model_state[self.prey_type_nr] = ")
                    print(np.transpose(self.model_state[self.prey_type_nr]))
                    ...
                    #self.remove_agent_instance_from_position_dict(agent_instance)  
                    print("self.prey_to_be_removed_by_starvation_dict", self.prey_to_be_removed_by_starvation_dict)
                    print(agent_name, " starves to death")
        ...
        if is_last: # removes agents and reap rewards at the end of the cycle
            ...
            for prey_name in self.prey_name_list:
                ...
                if prey_instance.is_alive:
                    ...
                    if self.prey_to_be_removed_by_predator_dict[prey_name]:
                        ...
                        self.model_state[self.prey_type_nr,prey_instance.position[0],prey_instance.position[1]] -= agent_instance.energy
                        self.model_state[self.predator_type_nr,prey_instance.position[0],prey_instance.position[1]] += agent_instance.energy
                        prey_instance.is_alive = False
                        prey_instance.energy = 0.0
                        self.remove_agent_instance_from_position_dict(prey_instance)
                    elif self.prey_to_be_removed_by_starvation_dict[prey_name]: 
                        self.prey_instance_list.remove(prey_instance)
                        self.n_active_prey -= 1
                        self.model_state[self.prey_type_nr,predator_instance.position[0],predator_instance.position[1]] -= 0
                        prey_instance.is_alive = False
                        prey_instance.energy = 0.0
                        # not yet implemented
                        self.remove_agent_instance_from_position_dict(prey_instance)
            ...
            for grass_name in self.grass_name_list:
                ...
                if self.grass_to_be_removed_by_prey_dict[grass_name]:
                    ...
                    self.model_state[self.grass_type_nr,grass_instance.position[0],grass_instance.position[1]] -= agent_instance.energy
    ...
    def add_agent_instance_to_position_dict(self, agent_instance):
        ...
        print("add ", agent_instance.agent_name, " to position ", agent_instance.position)
        position_dict[position].append(agent_instance)
        print("position_dict[",position,"] ", self.create_agent_name_list_from_instance_list(position_dict[position]))
        print()

    def remove_agent_instance_from_position_dict(self, agent_instance):
        # Determine the correct dictionary based on agent type
        agent_type = self.agent_type_name_list[agent_instance.agent_type_nr]
        if agent_type == 'grass':
            position_dict = self.agent_instance_in_grid_location_dict['grass']
        ...
        print()
        print("position_dict[",position,"] ", self.create_agent_name_list_from_instance_list(position_dict[position]))
        print("remove ", agent_instance.agent_name, " from position ", agent_instance.position)
...
class raw_env(AECEnv, EzPickle):
    ...
    def observe(self, _agent_name):
        ...
        if not _agent_instance.is_alive:
