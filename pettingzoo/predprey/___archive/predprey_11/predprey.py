...
class PredPrey:
    ...
    def reset(self):
        ...
        self.grass_instance_list = self.create_agents(
            self.n_grasses, self.grass_type_nr, self.max_observation_range, self.np_random, moore_neighborhood=self.moore_neighborhood_grasses
        ) 
    ...
    def step(self, action, agent_id, is_last):
        ...
        if is_last:
            ...
            #self.latest_rewards_state is set to zero
            #so latest_reward_state should be replaced by "rewards"???
    ...
    def safely_observe_agent_name(self, agent_id_name):
        ...
        observation_range = agent_instance.observation_range
        mask = int((self.max_observation_range - observation_range)/2)
        #obs_range = 7, 5, 3, 1 ; mask = 0, 1, 2, 3
        if mask == 0: # observation_range agent is smaller than default max_observation_range
            return observation
        elif mask == 1:
            observation[0][0,0:7] = 0
            observation[0][6,0:7] = 0
            observation[0][0:7,0] = 0
            observation[0][0:7,6] = 0
            observation[1][0,0:7] = 0
            observation[1][6,0:7] = 0
            observation[1][0:7,0] = 0
            observation[1][0:7,6] = 0
            observation[2][0,0:7] = 0
            observation[2][6,0:7] = 0
            observation[2][0:7,0] = 0
            observation[2][0:7,6] = 0
            return observation
        elif mask == 2:
            observation[0][0,0:7] = 0
            observation[0][6,0:7] = 0
            ...
            observation[0][1,0:7] = 0
            observation[0][5,0:7] = 0
            observation[0][0:7,1] = 0
            observation[0][0:7,5] = 0
            observation[1][1,0:7] = 0
            observation[1][5,0:7] = 0
            observation[1][0:7,1] = 0
            observation[1][0:7,5] = 0
            observation[2][1,0:7] = 0
            observation[2][5,0:7] = 0
            observation[2][0:7,1] = 0
            observation[2][0:7,5] = 0
            return observation
       
            """
            print("AGENT",agent_id_name)
            print("obs0")
            print(np.transpose(observation[0]))
            print("obs1")
            print(np.transpose(observation[1]))
            print("obs2")
            print(np.transpose(observation[2]))
            """
...
class raw_env(AECEnv, EzPickle):
    ...
    def step(self, action):
        ...
        for k in self.agents:
            #latest_reward_state is cumulative rewards of an agent
    ...
    def observe(self, agent_id_name):
        obs = self.env.safely_observe_agent_name(agent_id_name)
    ...
    def get_agent_instance_by_name(self, agent_id_name):
        return self.env.agent_name_to_instance_mapping[agent_id_name]
