...
environment_name = "predpreygrass_rllib"

import environments.predpreygrass_rllib as predpreygrass_rllib
from environments.predpreygrass_rllib import PredPreyGrassEnv
from config.parameters_rllib import training_steps_string
from config.parameters_rllib import env_kwargs as env_config
...
def env_creator(env_config):
    return PredPreyGrassEnv(env_config)  # return an env instance

register_env("PredPreyGrassEnv", env_creator)
algo = ppo.PPO(env="PredPreyGrassEnv")



def train(env_fn, steps: int = 10_000, seed: int | None = 0, **env_kwargs):

    parallel_env = parallel_wrapper_fn(env_fn.PredPreyGrassEnv)
...
def eval(env_fn, num_games: int = 100, render_mode: str | None = None, **env_kwargs):
    ...
    PredPreyGrassEnv = env_fn.PredPreyGrassEnv(render_mode=render_mode, **env_kwargs)
    model = PPO.load(loaded_policy)
    if not save_model:
        shutil.rmtree(output_project)
    cumulative_rewards = {agent: 0 for agent in PredPreyGrassEnv.possible_agents}
    ...
    agent_selector = agent_selector(agent_order=PredPreyGrassEnv.agents)
    ...
    for i in range(num_games):
        n_aec_cycles = 0
        PredPreyGrassEnv.reset()
        PredPreyGrassEnv._agent_selector.reset()
        predator_name_list = PredPreyGrassEnv.pred_prey_env.predator_name_list
        prey_name_list = PredPreyGrassEnv.pred_prey_env.prey_name_list
        agent_name_list = PredPreyGrassEnv.pred_prey_env.agent_name_list
        ...
        for agent in PredPreyGrassEnv.agent_iter():
            observation, reward, termination, truncation, info = PredPreyGrassEnv.last()
        ...
        avg_cycles[i]= PredPreyGrassEnv.pred_prey_env.n_aec_cycles
        ...
        print(f"Cycles = {PredPreyGrassEnv.pred_prey_env.n_aec_cycles}", f"Avg = {round(avg_rewards[i],1)}", 
