...
class DiscreteAgent:
    def __init__(
        ...
        self.x_grid_dim: int = self.model_state_agent[0].shape[0]  # first zero is the wall type
        self.y_grid_dim: int = self.model_state_agent[0].shape[1]
    ...
    def step(self, action: int) -> np.ndarray:
        ...
        # if intended to move to occupied cell of other agent of the same type: dont move
        elif self.model_state_agent[self.agent_type_nr][next_position[0], next_position[1]] > 0:
            return (
                self.position
            )  # if intended to moved to occupied cell of same agent type: dont move
        # if intended to move to occupied cell of WALL type: dont move
        elif self.model_state_agent[0][next_position[0], next_position[1]] > 0:
            return (
                self.position
            )  # if intended to moved to occupied cell of same agent type: dont move
