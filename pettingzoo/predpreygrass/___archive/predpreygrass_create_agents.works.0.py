...
class PredPrey:
    def __init__(
        ...
        self.n_initial_active_grass = n_initial_active_grass
        ...
        # initialization
        self.n_active_predator: int = self.n_initial_active_predator
        self.n_active_prey: int = self.n_initial_active_prey  
        self.n_active_grass: int = self.n_initial_active_grass
        ...
        # start indices of agent types in agent_name_list
        self.start_index_predator = 0
        self.start_index_prey = self.n_possible_predator
        self.start_index_grass = self.n_possible_predator + self.n_possible_prey
        self.end_index_grass = self.n_possible_predator + self.n_possible_prey + self.n_possible_grass


        # creation agent type lists
        self.predator_name_list =  ["predator" + "_" + str(a) for a in range(self.start_index_predator, self.start_index_prey)]
        self.prey_name_list =  ["prey" + "_" + str(a) for a in range(self.start_index_prey, self.start_index_grass)]
        self.agent_name_list = self.predator_name_list + self.prey_name_list

        self.grass_name_list =  ["grass" + "_" + str(a) for a in range(self.start_index_grass, self.end_index_grass)]
    ...
    def reset(self):
        ...
        for prey_instance in self.prey_instance_list:
            if prey_instance.agent_id_nr >= self.start_index_prey + self.n_initial_active_prey: # number of initial active predators
    ...
    def render(self):
        ...
        def draw_bar_chart_energy(self):
            ...
            predator_legend_x = 1100
            predator_legend_y = 600
            predator_legend_color = (255, 0, 0)  # red
            font = pygame.font.Font(None, 30)
            predator_legend_text = font.render(predator_legend_title, True, predator_legend_color)
            ...
            font = pygame.font.Font(None, 30)
            prey_legend_text = font.render(prey_legend_title, True, prey_legend_color)
            ...
            x_screenposition_prey_bars = 850   
            ...
            for i, value in enumerate(data_prey):
                bar_height = (value / max_energy_value_chart) * height
                bar_x = x_screenposition_prey_bars + x_screenposition + (self.width_energy_chart - (bar_width * len(data_prey))) // 2 + i * (bar_width+offset_bars)
                bar_y = y_screenposition + height - bar_height
