...
from pettingzoo.predpreygrass.predprey_3.predprey_base import PredPrey as _env
...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(self, *args, **kwargs):
        ...
        pygame.init()
        #agents
        self.agents = ["pursuer_" + str(p) for p in range(self.env.n_pursuers)]
        """
        self.agents = ["pursuer_" + str(p) for p in range(self.env.n_pursuers)] + \
                    ["predator_" + str(p) for p in range(self.env.n_predators)]
        """
        ...
        #print("self.env.act_dims[0] ",self.env.act_dims[0])
    ...
    def step(self, action):
        ...
        """
        print("self.env.latest_reward_state ",self.env.latest_reward_state)
        print()
        print("self.agents ", self.agents)
        print()
        print("self.agent_name_mapping ", self.agent_name_mapping)
        print()
        """
