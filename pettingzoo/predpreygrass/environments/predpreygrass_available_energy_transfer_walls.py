...
from agents.discrete_agent_walls import DiscreteAgent
...
class PredPreyGrass:
    def __init__(
        ...
        # within the spawning area of the agent, the agent can be placed on any random cell
        # walls are not included in the spawning area because they are not ranmdomly placed
        ...
        self.active_wall_instance_list: List[DiscreteAgent] = (
            []
        ...
        self.possible_wall_name_list: List[AgentID] = []
        ...
        self.n_actions_agent: int = len(self.motion_range) + 4
    ...
    def reset(self):
        ...
        self.active_wall_instance_list = []
        ...
        self.n_agent_type_list: List[int] = [
            self.x_grid_size*self.y_grid_size,  # wall agents
        ...
        for agent_type_nr in range(1, len(self.agent_type_name_list)):
            ...
            for _ in range(self.n_agent_type_list[agent_type_nr]):
                ...
                agent_instance = DiscreteAgent(
                    ...
                    # and to detect if a cell is already occupied by a WALL agent
                    self.model_state,  
                ...
                xinit, yinit = self.position_new_agent_on_gridworld(
        ...
        for _ in range(self.n_agent_type_list[WALL_TYPE_NR]):
            agent_id_nr = self.agent_id_counter
            agent_name = "wall_" + str(agent_id_nr)
            self.agent_id_counter += 1
            agent_instance = DiscreteAgent(
                WALL_TYPE_NR,
                agent_id_nr,
            ...
            agent_type_instance_list[WALL_TYPE_NR].append(agent_instance)



        self.active_predator_instance_list = agent_type_instance_list[PREDATOR_TYPE_NR]
        ...
        self.active_wall_instance_list = [] # no walls at start after reset


        self.possible_predator_name_list = (
            self.create_possible_agent_name_list_from_instance_list(
                self.active_predator_instance_list
            )
        )
        self.possible_prey_name_list = (
        ...
        self.possible_wall_name_list = (
            self.create_possible_agent_name_list_from_instance_list(
                agent_type_instance_list[WALL_TYPE_NR]
            )
        )
        ...
        if agent_instance.is_active:
            ...
            if agent_type_nr == PREDATOR_TYPE_NR:
                if agent_instance.energy > 0:
                    ...
                    # move in Von Neumann neighborhood or build wall in Moore neighborhood
                    self.step_agent(agent_instance, action)
                    x_new, y_new = agent_instance.position
                    is_prey_in_new_cell = (
                        self.agent_instance_in_grid_location[PREY_TYPE_NR][
                            (x_new, y_new)
                        ]
                        is not None
                    )
                    ...
                    self.predator_to_be_removed_by_starvation_dict[
            ...
            # If the agent is a prey and it's alive
                ...
                if agent_instance.energy > 0:
                    ...
                    self.step_agent(agent_instance, action)
                    x_new, y_new = agent_instance.position
                    is_grass_in_new_cell = (
                        self.model_state[GRASS_TYPE_NR, x_new, y_new] > 0
                    )
                    ...
                    self.prey_to_be_removed_by_starvation_dict[
                        agent_instance.agent_name
                    ] = True
    ...
    def place_wall_in_model_state(self, x, y):
        # place wall in model state only if cell is empty and is within the grid boundaries
        if (0 <= x < self.x_grid_size and 0 <= y < self.y_grid_size and
            self.model_state[WALL_TYPE_NR, x, y] == 0.0 and 
            self.model_state[PREDATOR_TYPE_NR,  x, y] == 0.0 and
            self.model_state[PREY_TYPE_NR, x, y] == 0.0 and
            self.model_state[GRASS_TYPE_NR, x, y] == 0.0):
            non_active_wall_names = [
                                        name
                                        for name in self.possible_wall_name_list
                                        if not self.agent_name_to_instance_dict[name].is_active
                                    ]
            # check ik there are inactive wall instances available
            if non_active_wall_names:
                # if so, choose the first inactive wall instance from the list
                new_wall_name = non_active_wall_names[0]
                new_wall_instance = self.agent_name_to_instance_dict[new_wall_name]
                self.model_state[WALL_TYPE_NR, x, y] = 1.0
                position_wall = (x, y)
                new_wall_instance.position = position_wall
    ...
    def earmarking_predator_catches_prey(self, predator_instance, x_new, y_new):
        ...
        if is_only_earmarked_if_prey_is_unaccompanied:
            ...
            if not is_accompanied_prey:
                ...
                self.prey_to_be_removed_by_predator_dict[
                    prey_instance_removed.agent_name
                ] = True
                self.agent_energy_from_eating_dict[predator_instance.agent_name] = (
                    prey_instance_removed.energy
                )
        else:
            # allways earmarking option
            prey_instance_removed = self.agent_instance_in_grid_location[PREY_TYPE_NR][
                (x_new, y_new)
            ]
            self.predator_who_remove_prey_dict[predator_instance.agent_name] = True
            self.prey_to_be_removed_by_predator_dict[
    ...
    def earmarking_prey_eats_grass(self, prey_instance, x_new, y_new):
        ...
        self.agent_energy_from_eating_dict[prey_instance.agent_name] = (
    ...
    def create_new_predator(self, parent_predator):
        ...
        if non_active_predator_names:
            ...
            x_new, y_new = self.position_new_agent_on_gridworld(
                new_predator_instance, self.spawning_area, self.model_state
            )

            new_predator_instance.position = (x_new, y_new)
            self.agent_instance_in_grid_location[PREDATOR_TYPE_NR, x_new, y_new] = (
    ...
    def create_possible_agent_name_list_from_instance_list(
        self, _active_agent_instance_list
    ):
    ...
    def render(self):
        ...
        def draw_wall_instances():
            for wall_instance in self.active_wall_instance_list:
                ...
                # Assuming center is a tuple (x, y) representing the center of the rectangle
