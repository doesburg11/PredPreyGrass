...
from predpreygrass.hyper_parameter_tuning.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.hyper_parameter_tuning.config.config_env_train_v1_0 import config_env
from predpreygrass.hyper_parameter_tuning.utils.networks import build_multi_module_spec
...
def get_config_ppo():
    ...
    if smoke and str(smoke).lower() in ("1", "true", "yes", "y"):
        from predpreygrass.hyper_parameter_tuning.config.config_ppo_cpu_pbt_smoke import config_ppo
        return config_ppo
    ...
    if num_cpus == 32:
        ...
        from predpreygrass.hyper_parameter_tuning.config.config_ppo_gpu_pbt import config_ppo
        return config_ppo
    if num_cpus == 8:
        from predpreygrass.hyper_parameter_tuning.config.config_ppo_cpu_pbt_smoke import config_ppo
        return config_ppo
