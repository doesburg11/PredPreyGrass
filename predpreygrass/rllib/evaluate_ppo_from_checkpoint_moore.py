...
from predpreygrass.rllib.predpreygrass_rllib_env_moore import PredPreyGrass  # Import the custom environment
...
checkpoint_path = "/home/doesburg/ray_results/benchmarks/moore/PPO_2025-03-21_21-48-35/PPO_PredPreyGrass_dbb97_00000_0_2025-03-21_21-48-35/checkpoint_000019"  # Update as needed
...
while not done:
    ...
    for agent_id, r in rewards.items():
        step_total_reward += r
        if "predator" in agent_id:
            step_predator_reward += r
        elif "prey" in agent_id:
            step_prey_reward += r

    total_reward += step_total_reward
    total_predator_reward = total_predator_reward + step_predator_reward if 'total_predator_reward' in locals() else step_predator_reward
    total_prey_reward = total_prey_reward + step_prey_reward if 'total_prey_reward' in locals() else step_prey_reward
...
# Use all possible agents to include terminated/spawned ones
all_agent_ids = set(env.possible_agents)

for agent_id in sorted(all_agent_ids):  # optional: sorted for pretty print
    reward = env.cumulative_rewards.get(agent_id, 0.0)
...
# Use values directly from the step-by-step loop
total_reward_all = total_predator_reward + total_prey_reward


print("\n--- Aggregated Rewards (Step-Based) ---")
