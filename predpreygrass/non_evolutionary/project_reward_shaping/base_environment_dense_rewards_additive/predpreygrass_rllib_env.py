"""
Additive dense+reproduction-bonus reward variant of the PredPreyGrass base
environment. Two types of agents: predators and prey. Independently learning
policies for each type. Reward is the dense, per-step net energy delta
(decay + move + eat + reproduction cost), same as
base_environment_dense_rewards, PLUS a flat reproduction bonus
(+reproduction_reward_predator/_prey) added to the parent's reward on the
step it reproduces -- unlike base_environment_dense_rewards, which relies
solely on the energy delta (a net cost to the parent) with no compensating
signal for the act of reproducing itself.

Exists to isolate whether base_environment_dense_rewards' lower reproduction
rate/less balanced populations (vs. base_environment_sparse_rewards, see
that module's results) is caused by reward density itself or specifically
by removing the reproduction incentive under "pure replacement" dense
...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        # same as base_environment_dense_rewards, PLUS a flat reproduction bonus
        # added to the parent on the step it reproduces (see Step 5 / Step 5b in
        # step()) -- the one hand-designed shaping constant this variant restores.
    ...
    def step(self, action_dict):
        ...
        # Flat reproduction bonus for a parent that reproduces this step, added
        # on top of its dense energy-delta reward in Step 5b (not a replacement
        # for it -- the energy cost of reproducing is still captured by the
        # delta itself, same as base_environment_dense_rewards).
        reproduction_bonus: Dict[AgentID, float] = {}
            ...
            if "predator" in agent:
                if self.agent_energies[agent] >= self.predator_creation_energy_threshold:
                    ...
                    if self._next_predator_idx < self.n_possible_predators:
                        ...
                        # energy delta; the flat bonus below is added on top of that in
                        # Step 5b. The newborn had no energy_before this step.
                        reproduction_bonus[agent] = self.reproduction_reward_predator
                        rewards[new_agent] = 0
                ...
                if self.agent_energies[agent] >= self.prey_creation_energy_threshold:
                    ...
                    if self._next_prey_idx < self.n_possible_prey:
                        ...
                        # energy delta; the flat bonus below is added on top of that in
                        # Step 5b. The newborn had no energy_before this step.
                        reproduction_bonus[agent] = self.reproduction_reward_prey
                        rewards[new_agent] = 0
                        self.cumulative_rewards[new_agent] = 0
                        observations[new_agent] = self._get_observation(new_agent)
                        terminations[new_agent] = False
        ...
        # eat + reproduction cost) PLUS a flat reproduction bonus for any agent
        # that reproduced this step, for every agent that was alive at the start
        # of this step and is still alive now. Agents that died this step
        # (starvation or being caught) already had their reward set at the point
        # of removal; newborns already had their reward set to 0 at spawn time.
        for agent, before_energy in energy_before.items():
            if agent in self.agent_energies:
                rewards[agent] = (self.agent_energies[agent] - before_energy) + reproduction_bonus.get(agent, 0.0)
                self.cumulative_rewards[agent] += rewards[agent]
