...
and runs them in the PredPreyGrass environment (base_environment) for interactive debugging.
...
from predpreygrass.non_evolutionary.project_reward_shaping.base_environment_dense_rewards.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.non_evolutionary.project_reward_shaping.base_environment_dense_rewards.utils.pygame_grid_renderer_rllib import (
    PyGameRenderer,
    ViewerControlHelper,
    LoopControlHelper,
)
...
if __name__ == "__main__":
    ...
    checkpoint_path = os.path.join(
        os.path.expanduser("~"),
        "ray_results",
        "PPO_BASE_ENVIRONMENT_DENSE_REWARDS_2026-07-29_23-47-05",
        "PPO_PredPreyGrass_0a3bd_00000_0_2026-07-29_23-47-05",
        "checkpoint_000099",
    )
    ...
    while not loop_helper.simulation_terminated:
        ...
        if control.step_backward:
            if len(snapshots) > 1:
                ...
                # Use agent_positions (not env.agents) as the source of truth for who
                # is actually alive right now: env.agents may still list an agent that
                # died on this exact step (removal is deferred to the next step()
                # call), which no longer has a position to observe from.
                observations = {agent: env._get_observation(agent) for agent in env.agent_positions}
        ...
        if loop_helper.should_step(control):
            # Build actions from per-policy RLModules. Use agent_positions (not
            # env.agents) as the source of truth for who needs an action: env.agents
            # may still list an agent that died last step (removal deferred to the
            # start of the next step()), which has no observation to act on.
            action_dict = {
                agent_id: policy_pi(
                ...
                for agent_id in env.agent_positions
            }
            ...
            # Track stats
            num_predators = sum(1 for agent in env.agent_positions if "predator" in str(agent))
            num_prey = sum(1 for agent in env.agent_positions if "prey" in str(agent))
