...
def load_predpreygrass_modules() -> None:
    ...
    from predpreygrass.non_evolutionary.project_cooperation.stag_hunt_forward_view.predpreygrass_rllib_env import PredPreyGrass as _PredPreyGrass
    from predpreygrass.non_evolutionary.project_cooperation.stag_hunt_forward_view.config.config_env_stag_hunt_forward_view import config_env as _config_env
    from predpreygrass.non_evolutionary.project_cooperation.stag_hunt_forward_view.utils.matplot_renderer import (
        CombinedEvolutionVisualizer as _CombinedEvolutionVisualizer,
    )
    from predpreygrass.non_evolutionary.project_cooperation.stag_hunt_forward_view.utils.defection_metrics import (
...
def setup_modules():
    if TRAINED_EXAMPLE_DIR:
        ...
        # STAG_HUNT_FORWARD_VIEW_JOIN_COST_0.02_SCAVENGER_0.1_2026-01-25_14-20-20/PPO_PredPreyGrass_99161_00000_0_2026-01-25_14-20-20/checkpoint_000099
        ray_results_dir = "/home/doesburg/Projects/PredPreyGrass/predpreygrass/stag_hunt_forward_view/ray_results/"
        checkpoint_root = "STAG_HUNT_FORWARD_VIEW_JOIN_COST_0_02_SCAVENGER_0_1_2026-01-25_14-20-20/PPO_PredPreyGrass_99161_00000_0_2026-01-25_14-20-20/checkpoint_000099"
...
def compute_defection_metrics(env):
    ...
    join_cost_stats = _compute_join_cost_stats(env.agent_event_log)
    successes = int(getattr(env, "team_capture_successes", 0))
    failures = int(getattr(env, "team_capture_failures", 0))
    coop_successes = int(getattr(env, "team_capture_coop_successes", 0))
    coop_failures = int(getattr(env, "team_capture_coop_failures", 0))
    mammoth_successes = int(getattr(env, "team_capture_mammoth_successes", 0))
    mammoth_failures = int(getattr(env, "team_capture_mammoth_failures", 0))
    rabbit_successes = int(getattr(env, "team_capture_rabbit_successes", 0))
    rabbit_failures = int(getattr(env, "team_capture_rabbit_failures", 0))

    solo_successes = max(successes - coop_successes, 0)
    solo_failures = max(failures - coop_failures, 0)

    capture_attempts = successes + failures
    coop_attempts = coop_successes + coop_failures
    solo_attempts = solo_successes + solo_failures
    mammoth_attempts = mammoth_successes + mammoth_failures
    rabbit_attempts = rabbit_successes + rabbit_failures

    failure_stats = {
        "team_capture_successes": successes,
        "team_capture_failures": failures,
        "team_capture_attempts": capture_attempts,
        "team_capture_failure_rate": _safe_div(failures, capture_attempts),
        "team_capture_coop_successes": coop_successes,
        "team_capture_coop_failures": coop_failures,
        "team_capture_coop_attempts": coop_attempts,
        "team_capture_coop_failure_rate": _safe_div(coop_failures, coop_attempts),
        "team_capture_solo_successes": solo_successes,
        "team_capture_solo_failures": solo_failures,
        "team_capture_solo_attempts": solo_attempts,
        "team_capture_solo_failure_rate": _safe_div(solo_failures, solo_attempts),
        "team_capture_mammoth_successes": mammoth_successes,
        "team_capture_mammoth_failures": mammoth_failures,
        "team_capture_mammoth_attempts": mammoth_attempts,
        "team_capture_mammoth_failure_rate": _safe_div(mammoth_failures, mammoth_attempts),
        "team_capture_rabbit_successes": rabbit_successes,
        "team_capture_rabbit_failures": rabbit_failures,
        "team_capture_rabbit_attempts": rabbit_attempts,
        "team_capture_rabbit_failure_rate": _safe_div(rabbit_failures, rabbit_attempts),
    }
    metrics = {
        "steps": env.current_step,
        "join_defect": join_stats,
        "capture_outcomes": capture_stats,
        "capture_failures": failure_stats,
        "join_costs": join_cost_stats,
    }
    return metrics
...
if __name__ == "__main__":
        ...
        per_run_metrics.append(
            {
                ...
                **defection_metrics.get("capture_failures", {}),
                **defection_metrics.get("join_costs", {}),
            }
        )
        if SAVE_EVAL_RESULTS:
            visualizer.plot()
    ...
    if filtered_runs:
        def _aggregate_runs(runs):
            ...
            team_capture_successes = sum(m.get("team_capture_successes", 0) for m in runs)
            team_capture_failures = sum(m.get("team_capture_failures", 0) for m in runs)
            team_capture_coop_successes = sum(m.get("team_capture_coop_successes", 0) for m in runs)
            team_capture_coop_failures = sum(m.get("team_capture_coop_failures", 0) for m in runs)
            team_capture_solo_successes = sum(m.get("team_capture_solo_successes", 0) for m in runs)
            team_capture_solo_failures = sum(m.get("team_capture_solo_failures", 0) for m in runs)
            team_capture_mammoth_successes = sum(m.get("team_capture_mammoth_successes", 0) for m in runs)
            team_capture_mammoth_failures = sum(m.get("team_capture_mammoth_failures", 0) for m in runs)
            team_capture_rabbit_successes = sum(m.get("team_capture_rabbit_successes", 0) for m in runs)
            team_capture_rabbit_failures = sum(m.get("team_capture_rabbit_failures", 0) for m in runs)
            ...
            team_capture_attempts = team_capture_successes + team_capture_failures
            team_capture_coop_attempts = team_capture_coop_successes + team_capture_coop_failures
            team_capture_solo_attempts = team_capture_solo_successes + team_capture_solo_failures
            team_capture_mammoth_attempts = team_capture_mammoth_successes + team_capture_mammoth_failures
            team_capture_rabbit_attempts = team_capture_rabbit_successes + team_capture_rabbit_failures
            join_cost_per_event = join_cost_total / join_cost_events if join_cost_events else 0.0
            join_cost_per_predator = (
                join_cost_total / predators_with_join_cost if predators_with_join_cost else 0.0
            )
            ...
            return {
                ...
                "capture_failures": {
                    ...
                    "team_capture_failure_rate": team_capture_failures / team_capture_attempts if team_capture_attempts else 0.0,
                    ...
                    "team_capture_coop_failure_rate": (
                        team_capture_coop_failures / team_capture_coop_attempts if team_capture_coop_attempts else 0.0
                    ),
                    ...
                    "team_capture_solo_failure_rate": (
                        team_capture_solo_failures / team_capture_solo_attempts if team_capture_solo_attempts else 0.0
                    ),
                    ...
                    "team_capture_mammoth_failure_rate": (
                        team_capture_mammoth_failures / team_capture_mammoth_attempts if team_capture_mammoth_attempts else 0.0
                    ),
                    ...
                    "team_capture_rabbit_failure_rate": (
                        team_capture_rabbit_failures / team_capture_rabbit_attempts if team_capture_rabbit_attempts else 0.0
                    ),
                },
                "join_costs": {
                    "join_cost_total": join_cost_total,
        ...
        if SAVE_EVAL_RESULTS:
            ...
            try:
                from predpreygrass.non_evolutionary.project_cooperation.stag_hunt_forward_view.utils.summarize_capture_failures import (
                    build_capture_failure_summary,
                )

                failure_summary, failure_text = build_capture_failure_summary(aggregate_path)
                (summary_dir / "capture_failures_summary.json").write_text(
                    json.dumps(failure_summary, indent=2)
                )
                (summary_dir / "capture_failures_summary.txt").write_text(failure_text)
                print(f"CAPTURE_FAILURES -> {summary_dir / 'capture_failures_summary.txt'}")
            except Exception as exc:
                print(f"Failed to write capture failure summary: {exc}")
    else:
        print("\n=== Aggregate defection metrics across runs ===")
