...
"""
from predpreygrass.non_evolutionary.project_cooperation.direct_reciprocity.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.non_evolutionary.project_cooperation.direct_reciprocity.config.config_env_direct_reciprocity import config_env
from predpreygrass.non_evolutionary.project_cooperation.direct_reciprocity.utils.episode_return_callback import EpisodeReturn
from predpreygrass.non_evolutionary.project_cooperation.direct_reciprocity.utils.networks import build_multi_module_spec
...
def get_config_ppo():
    ...
    if num_cpus == 32:
        from predpreygrass.non_evolutionary.project_cooperation.direct_reciprocity.config.config_ppo_gpu_direct_reciprocity import config_ppo
    elif num_cpus == 8:
        from predpreygrass.non_evolutionary.project_cooperation.direct_reciprocity.config.config_ppo_cpu_direct_reciprocity import config_ppo
    else:
...
if __name__ == "__main__":
    ...
    ray_results_dir = "/home/doesburg/Projects/PredPreyGrass/predpreygrass/direct_reciprocity/ray_results/"
    ray_results_path = Path(ray_results_dir).expanduser()
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    version = "DIRECT_RECIPROCITY_GRID_30_PRED_OBS_RANGE_9_SHARE_25"
