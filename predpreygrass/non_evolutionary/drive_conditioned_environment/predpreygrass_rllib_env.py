...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        self.num_world_channels = config.get("num_obs_channels", 4)
        self.enable_drive_channels = config.get("enable_drive_channels", True)
        self.predator_drive_channels = config.get(
        ...
        self.prey_drive_channels = config.get(
            "prey_drive_channels",
            [
                "hunger_pressure",
                "reproductive_readiness",
        ...
        self.predator_num_obs_channels = self.num_world_channels + (
            len(self.predator_drive_channels) if self.enable_drive_channels else 0
        )
        self.prey_num_obs_channels = self.num_world_channels + (
            len(self.prey_drive_channels) if self.enable_drive_channels else 0
        )
        self.predator_hunger_safe_energy = config.get("predator_hunger_safe_energy", self.initial_energy_predator)
        self.prey_hunger_safe_energy = config.get("prey_hunger_safe_energy", self.initial_energy_prey)
        self.prey_opportunity_normalizer = config.get("prey_opportunity_normalizer", self.initial_energy_prey * 3)
        self.predator_danger_normalizer = config.get(
            "predator_danger_normalizer",
            self.initial_energy_predator * 2,
        )
        self.grass_opportunity_normalizer = config.get(
            "grass_opportunity_normalizer",
            config.get("initial_energy_grass", 2.0) * 5,
        )
        ...
        predator_obs_shape = (self.predator_num_obs_channels, self.predator_obs_range, self.predator_obs_range)
        prey_obs_shape = (self.prey_num_obs_channels, self.prey_obs_range, self.prey_obs_range)
    ...
    def _get_observation(self, agent):
        ...
        observation[
            1:self.num_world_channels,
            xolo:xohi,
            yolo:yohi,
        ] = self.grid_world_state[1:, xlo:xhi, ylo:yhi]

        if self.enable_drive_channels:
            drive_features = self._get_drive_features(agent, observation)
            for channel_offset, value in enumerate(drive_features):
                observation[self.num_world_channels + channel_offset].fill(value)

        return observation

    def _get_observation_channel_count(self, agent):
        if "predator" in agent:
            return self.predator_num_obs_channels
        return self.prey_num_obs_channels

    def _get_drive_features(self, agent, observation):
        if "predator" in agent:
            feature_names = self.predator_drive_channels
        else:
            feature_names = self.prey_drive_channels

        return np.array(
            [self._get_drive_feature(agent, observation, name) for name in feature_names],
            dtype=np.float64,
        )
    ...
    def _get_drive_feature(self, agent, observation, feature_name):
        ...
        if feature_name == "hunger_pressure":
            safe_energy = self.predator_hunger_safe_energy if "predator" in agent else self.prey_hunger_safe_energy
            return self._safe_clip01(1.0 - energy / safe_energy)

        if feature_name == "reproductive_readiness":
            threshold = (
                self.predator_creation_energy_threshold if "predator" in agent else self.prey_creation_energy_threshold
            )
            return self._safe_clip01(energy / threshold)

        if feature_name == "prey_opportunity":
            return self._safe_clip01(np.sum(observation[2]) / self.prey_opportunity_normalizer)

        if feature_name == "predator_danger_pressure":
            return self._safe_clip01(np.sum(observation[1]) / self.predator_danger_normalizer)

        if feature_name == "grass_opportunity":
            return self._safe_clip01(np.sum(observation[3]) / self.grass_opportunity_normalizer)

        raise ValueError(f"Unknown drive feature: {feature_name!r}")

    def _safe_clip01(self, value):
