...
**Setup**: `base_environment` and `drive_conditioned_environment --drive-set full`, both seed 42, both 1000 PPO training iterations, identical `config_env.py` settings and PPO hyperparameters (verified like-for-like beforehand — the only difference is the 3-4 extra drive channels drive-conditioned adds to the observation). Runs: `PPO_BASE_ENVIRONMENT_SEED42_2026-09-05_18-55-45` and `PPO_DRIVE_CONDITIONED_FULL_SEED42_2026-09-06_07-26-38` under `~/simulation_results/ray_results/base_v_drive_2026-09-06/`. Both completed cleanly (1000/1000 iterations, no crashes, no extinctions to speak of — `extinct_predator`/`extinct_prey` ≈ 0 throughout).
...
**To pick this back up**: `python -m predpreygrass.non_evolutionary.drive_conditioned_environment.tune_ppo_drive_conditioned_environment --seed 42 --drive-set energy_only --max-iters 1000`, then compare against the `full` run above the same way (episode-count-weighted pooling, not per-iteration means — see "Results" above for why). If `energy_only` captures most of the +6% gap, that confirms the two-group prediction; if it captures little of it, the density-based drives (or their interaction with the energy-based ones) are doing more of the work than predicted.

### TODO: additional seeds for the comparison (not yet run)

Everything in "Results" above is a single seed (42) per arm. Per the caveat already stated there, a consistent-looking pattern in one seed is not evidence of a real effect on its own — this project's own history (see the cultural-plasticity and nuptial-gift experiments elsewhere in this repo) has produced single-seed patterns that turned out to be noise once more seeds were run. The `+6.0%` full-run gap and its windowed shape should be treated as a documented observation from this one run, not a validated result, until replicated.

**To validate**: run `base_environment` and `drive_conditioned_environment --drive-set full` again with at least 2-3 more seeds each (e.g. `--seed 43`, `--seed 44`), using the same `--max-iters 1000`. If the gap holds up in the same direction and rough magnitude across seeds, that's real evidence for the effect. If it varies wildly in sign or size, this run's `+6.0%` was this seed's particular luck. Same episode-count-weighted pooling method as above applies when comparing.
