...
base_environment) PLUS a flat eating-event bonus (+1 predator / +0.1 prey,
asymmetric -- see config_env.py for why), still no continuous energy-delta
signal. Same RLlib-compliance fixes as the other base_environment_* modules
(see predpreygrass_rllib_env.py / README). Compare training dynamics against
base_environment_sparse_rewards/tune_ppo_base_environment_sparse_rewards.py
(reproduction-only) and
base_environment_dense_rewards_additive/tune_ppo_base_environment_dense_rewards_additive.py
(dense delta + reproduction bonus), with everything else held equal.
"""
from predpreygrass.non_evolutionary.base_environment_sparse_rewards_plus_eating.predpreygrass_rllib_env import PredPreyGrass
...
if __name__ == "__main__":
    ...
    experiment_name = f"PPO_BASE_ENVIRONMENT_SPARSE_REWARDS_PLUS_EATING_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
