...
```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/tune_ppo_malthusian_rl.py
```

Strict exact APPO run:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/tune_appo_malthusian_exact.py
```

Strict exact APPO multi-seed run:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/scripts/run_exact_reproduction_seeds.py --seeds 0 1 2
```

Article-task Allelopathy reconstruction:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/scripts/run_article_reproduction_seeds.py --task allelopathy --variant biased --condition allelopathy_biased_heterogeneous_dynamic --seeds 0 1 2
```

Article-task Clamity reconstruction:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/scripts/run_article_reproduction_seeds.py --task clamity --condition clamity_dynamic_population --seeds 0 1 2
```

Live article-task viewers:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/view_article_task.py --task allelopathy --variant biased --total-individuals 64 --num-islands 4
```

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/view_article_task.py --task clamity --total-individuals 64 --num-islands 4
```

These viewers open a Pygame window and run the article-task reconstruction with random actions. They are for inspecting environment mechanics, not for training or checkpoint evaluation. The Allelopathy viewer shows the shrub/resource grid, type-A/type-B shrubs, species-colored agents, species counts, mean returns, and shrub counts for the selected island. The Clamity viewer shows nutrient patches, agents, settled agents, shell radius, species counts, and mean shell size.

Viewer controls:

- `space`: pause or resume.
- `right arrow`: step once while paused.
- `tab`: next island.
- `left arrow`: previous island.
- `esc`: quit.

Full article-condition matrix reconstruction:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/scripts/run_article_condition_matrix.py --seeds 0 1 2
```

Paper-like evaluation for exact runs:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/evaluate_exact_reproduction.py
```

Random rollout viewer:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/random_policy.py
```

Checkpoint evaluation:

```bash
PYTHONPATH=src python predpreygrass/malthusian_rl/evaluate_ppo_from_checkpoint_debug.py
```
