...
**Replicated across six seeds (300-iteration runs, `RESULTS.md` §19):** it is sustainable in most seeds but not all. Compared with `base_environment` (~19 predators and 0% predator extinction in every seed), the defaults give fewer predators in all six seeds (mean ~10.6, range 3.8-14.1) and more prey (mean ~32 vs ~19). Predator extinction is 0-4% in five seeds, but in seed 45 the predator population never established (~4 predators and 34-48% extinction across all 300 iterations). Its predator policy never sharpened (entropy stayed near random, `RESULTS.md` §20), so it fails when acting by sampling although its greedy version survives; why it fell behind early is unknown. So expect roughly one failed run in six until this is understood.

**Open**: why seed 45 fell behind early, and whether a slightly easier predator cost, an entropy bonus, longer training or more seeds change the failure rate; how long the stability of the other seeds holds beyond 500 iterations; what causes the extra predator clustering (`RESULTS.md` §16-18); whether the clustering also holds when actions are sampled from the policy (all clustering measurements used greedy actions, and seed 45's greedy policy is under-trained, §20); and whether the extra clustering opens room for cooperation mechanisms (an idea only, not investigated).
