...
parity" result in this iteration was specific to the reproduction-blocking cap, not a general consequence of holding
population near a ceiling. The sentence above, "consistent with population growth or crowding having contributed to
the uncapped reversal," survives only for the *size* of the reversal; the cap's own selection effect, which this
iteration's caveats already named as unseparated, is the more likely reason the sign disappeared here. See Iteration
15 for the corrected reading.
...
Males approach prey somewhat more when their mate is nearby than when she is dead (all three positive, two clearly
excluding zero). This is the same kind of marginal observational association as the near-away result, with the same
...
3. **Whether the split overshoots into a female lead in realized prey-energy share depends on the design.** It does
   ...
   uncapped; it does not under the cap. The density-target result resembles the uncapped one in sign, and the
   reproduction-blocking cap is the odd one out, which fits the confound named in Iteration 13's caveats. This is a
...
So the cap's "close to parity" was specific to the reproduction-blocking cap, and the reversal in direction is not simply a population-size
artifact, though its size changes with how population is controlled. The fruit-approach difference stays in the CONTROL direction (attenuated)
...
so the overshoot is real in direction but not a stable size.** Iteration 13's reading (that population growth caused the reversal) is superseded: the
cap's own selection effect was the more likely reason the sign disappeared there. Each population-control mechanism is itself an intervention, so
...
1. ~~Fix the replenishment pool exhaustion in `FixedPreyDensityEnv`, then run the full odds factorial inside it~~ **Done
   (Iteration 12).** ~~Control for the population-boom confound~~ **Done (Iterations 13 and 15):** first with a
