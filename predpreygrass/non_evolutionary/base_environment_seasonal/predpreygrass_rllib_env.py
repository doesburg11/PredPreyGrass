...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        # Seasonal grass-regrowth cycle (see _current_season_multiplier)
        self.season_length_steps = config.get("season_length_steps", 40)
        self.season_high_multiplier = config.get("season_high_multiplier", 1.5)
        self.season_low_multiplier = config.get("season_low_multiplier", 0.5)
    ...
    def _current_season_multiplier(self) -> float:
        """
        Square-wave seasonal cycle on grass regrowth: season_high_multiplier for
        season_length_steps steps, then season_low_multiplier for the next
        season_length_steps steps, repeating. Phase is purely a function of the
        per-episode step counter, so it resets with every reset() like the rest
        of episode state.
        """
        phase = (self.current_step // self.season_length_steps) % 2
        return self.season_high_multiplier if phase == 0 else self.season_low_multiplier
    ...
    def step(self, action_dict):
        ...
        seasonal_energy_gain_per_step_grass = self.energy_gain_per_step_grass * self._current_season_multiplier()
        for grass, grass_position in self.grass_positions.items():
            self.grass_energies[grass] = min(
                self.grass_energies[grass] + seasonal_energy_gain_per_step_grass, self.initial_energy_grass
            )
