"""
Sparse, event-based reward variant of the PredPreyGrass base environment --
same clean, discrete-event reward style as base_environment_sparse_rewards
(no continuous energy-delta signal), but with an added flat reward for the
eating event itself (catching prey / eating grass), not just reproduction.
Magnitudes are asymmetric by design (+1 predator / +0.1 prey) -- see
config_env.py and the module README for the measured eats-per-reproduction
data behind that choice. Also includes the same RLlib-compliance fixes as
the other three base_environment_* modules (see README): deferred
self.agents removal timing so terminal transitions actually reach RLlib,
and never-reused monotonic agent IDs so a recycled ID doesn't collide with
...
"""
from predpreygrass.non_evolutionary.base_environment_sparse_rewards_plus_eating.config_env import config_env
