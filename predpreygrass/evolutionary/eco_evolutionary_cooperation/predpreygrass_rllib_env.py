...
from predpreygrass.evolutionary.eco_evolutionary_cooperation.utils.genome import (
    Genome,
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _init_reset_variables(self, seed):
        ...
        # Cooperative-donation episode totals, by species, for aggregate metrics.
        self.total_energy_donated = {"predator": 0.0, "prey": 0.0}
        self.total_energy_received = {"predator": 0.0, "prey": 0.0}
        self.total_kin_donation = {"predator": 0.0, "prey": 0.0}
    ...
    def _build_live_genome_metrics(self) -> dict[str, float]:
        ...
        for agent_id in self.agents:
            ...
            if genome is not None:
                ...
                grouped[key].append(float(genome.cooperation_rate))
        metrics: dict[str, float] = {}
        for species, values in grouped.items():
            if values:
                arr = np.array(values)
                p25, p50, p75 = np.percentile(arr, [25, 50, 75])
                metrics[f"{species}_cooperation_rate_mean"] = float(np.mean(arr))
                metrics[f"{species}_cooperation_rate_std"] = float(np.std(arr))
                metrics[f"{species}_cooperation_rate_p25"] = float(p25)
                metrics[f"{species}_cooperation_rate_p50"] = float(p50)
                metrics[f"{species}_cooperation_rate_p75"] = float(p75)
                metrics[f"{species}_count"] = float(len(values))
            else:
                metrics[f"{species}_cooperation_rate_mean"] = 0.0
                metrics[f"{species}_cooperation_rate_std"] = 0.0
                metrics[f"{species}_cooperation_rate_p25"] = 0.0
                metrics[f"{species}_cooperation_rate_p50"] = 0.0
                metrics[f"{species}_cooperation_rate_p75"] = 0.0
    ...
    def _apply_cooperative_donation(self, agent: str, energy_gain: float) -> float:
        """Share a slice of this step's positive food/hunting gain with nearby same-species agents.

        Meal-sharing model: donation is a fraction of the gain just captured, not of
        standing energy stock, so agents that don't eat this step donate nothing.
        Returns the net gain retained by the donor after any donation.
        """
        if not self.genome_enabled or energy_gain <= 0.0:
            return energy_gain
        genome = self.agent_genomes.get(agent)
        cooperation_rate = float(genome.cooperation_rate) if genome is not None else 0.0
        ...
        positions_map = self.predator_positions if is_predator else self.prey_positions

        neighbors = [
            other
            for other, pos in positions_map.items()
            if other != agent
        ...
        donation_total = cooperation_rate * energy_gain
        share = donation_total / len(neighbors)
        kin_donation = 0.0
        for neighbor in neighbors:
            self.agent_energies[neighbor] += share
            self.grid_world_state[layer, *self.agent_positions[neighbor]] = self.agent_energies[neighbor]
        ...
        self.total_energy_donated[species] += donation_total
        self.total_energy_received[species] += donation_total
        self.total_kin_donation[species] += kin_donation
    ...
    def _apply_time_step_update(self):
        ...
        for raw_agent in list(self.agents):
            ...
            energy_decay = self.basal_energy_cost_predator if is_predator else self.basal_energy_cost_prey
    ...
    def _handle_predator_engagement(self, agent):
        ...
        if caught_prey:
            ...
            energy_gain = self._apply_cooperative_donation(agent, prey_energy)
    ...
    def _handle_prey_engagement(self, agent):
        ...
        if caught_grass:
            ...
            energy_gain = self._apply_cooperative_donation(agent, grass_energy)
    ...
    def _build_episode_training_metrics(self) -> dict[str, float]:
        ...
        for species in ("predator", "prey"):
            donated = self.total_energy_donated[species]
            metrics[f"{species}_energy_donated_total"] = float(donated)
            metrics[f"{species}_energy_received_total"] = float(self.total_energy_received[species])
            metrics[f"{species}_local_relatedness_proxy"] = (
                float(self.total_kin_donation[species] / donated) if donated > 0.0 else 0.0
            )
        ...
        # Reverse-leg metric: does cooperation_rate predict individual reproductive success?
        # Spearman(coop, reproduced) > 0 → cooperating pays off (inclusive fitness dominant)
        # Spearman(coop, reproduced) < 0 → cooperation is a net cost (defection favored)
        # Sign flip as policy improves is direct evidence that RL learning (e.g. dispersal
        # behavior, which sets local relatedness) altered the genome fitness landscape.
        ...
        for species in ("prey", "predator"):
            pairs = [
                (float(rec["genome"]["cooperation_rate"]), float(rec.get("offspring_count", 0) > 0))
                for aid, rec in self._iter_all_agent_records()
                if aid.startswith(species)
                and isinstance(rec.get("genome"), dict)
                and "cooperation_rate" in rec["genome"]
            ]
            if len(pairs) < 4:
                continue
            coop_arr = np.array([p[0] for p in pairs])
            repro_arr = np.array([p[1] for p in pairs])
            metrics[f"{species}_coop_repro_spearman"] = _spearman_corr(coop_arr, repro_arr)
            q25, q50, q75 = np.percentile(coop_arr, [25, 50, 75])
            bins = [
                coop_arr <= q25,
                (coop_arr > q25) & (coop_arr <= q50),
                (coop_arr > q50) & (coop_arr <= q75),
                coop_arr > q75,
            ]
            for i, mask in enumerate(bins, 1):
                if mask.sum() > 0:
                    metrics[f"{species}_coop_repro_rate_q{i}"] = float(repro_arr[mask].mean())
