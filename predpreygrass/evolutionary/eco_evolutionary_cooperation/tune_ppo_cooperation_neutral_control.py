"""
Neutral-drift control for the eco_evolutionary_cooperation environment.

Identical to tune_ppo_cooperation.py in every respect except the environment
...
same-species agent, not the actual parent) while leaving population/energy/spatial
dynamics (and thus local relatedness) unchanged. Any cooperation_rate drift observed
in this run is attributable purely to mutation + finite-population sampling noise,
not selection.

Compare this run's live_genome/*_cooperation_rate_mean trajectory directly against
the real pilot run (2026-07-17, see RESULTS.md).
"""

from predpreygrass.evolutionary.eco_evolutionary_cooperation.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.evolutionary.eco_evolutionary_cooperation.config.config_env_eco_evolutionary_neutral_control import config_env
...
if __name__ == "__main__":
    ...
    version = f"ECO_EVOLUTION_COOPERATION_NEUTRAL_CONTROL{seed_tag}"
