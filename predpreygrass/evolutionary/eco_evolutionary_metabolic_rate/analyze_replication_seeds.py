"""
Iteration 5 analysis: aggregate multi-seed replication runs (real satiation-
...
Expects experiment directories under ~/simulation_results/ray_results/ named:
  PPO_ECO_EVOLUTION_METABOLIC_RATE_SEED<seed>_<timestamp>                (real)
  PPO_ECO_EVOLUTION_METABOLIC_RATE_NEUTRAL_CONTROL_SEED<seed>_<timestamp> (control)

as produced by tune_ppo_metabolic_rate.py / tune_ppo_metabolic_rate_neutral_control.py
...
Usage:
    python predpreygrass/eco_evolutionary_metabolic_rate/analyze_replication_seeds.py
"""
...
REAL_PATTERN = re.compile(r"^PPO_ECO_EVOLUTION_METABOLIC_RATE_SEED(\d+)_")
CONTROL_PATTERN = re.compile(r"^PPO_ECO_EVOLUTION_METABOLIC_RATE_NEUTRAL_CONTROL_SEED(\d+)_")
...
def find_seed_runs(pattern: re.Pattern) -> dict[int, Path]:
    ...
    for exp_dir in RAY_RESULTS_DIR.glob("PPO_ECO_EVOLUTION_METABOLIC_RATE*"):
...
def load_mr_series(result_json: Path, key: str) -> list[float]:
    values = []
...
def summarize_run(result_json: Path) -> dict:
    """Q1/Q5 mean and max |deviation from founder| for predator and prey live MR."""
    out = {}
    for species in ("predator", "prey"):
        series = load_mr_series(result_json, f"live_genome/{species}_metabolic_rate_mean")
        if not series:
            out[species] = None
            continue
        n = len(series)
...
def main():
    ...
    report_hunt_fits(real_runs, control_runs, trait="metabolic_rate")
