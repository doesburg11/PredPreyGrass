"""
Predator-Prey Grass RLlib Environment — Nuptial Gift variant.

Predators are split into two sexes with separate policies: predator_male and
predator_female. Only females reproduce; females graze grass (capped, so
grazing alone cannot sustain reaching the reproduction threshold) while males
exclusively hunt prey. A heritable, mechanically-executed genome trait
(male_donation_rate) governs what fraction of a male's hunting gain he
donates ("nuptial gift") to predator_female neighbors within cooperation_range.
See README.md for the full argument.
"""
...
from predpreygrass.evolutionary.eco_evolutionary_nuptial_gift.utils.genome import (
    Genome,
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _initialize_from_config(self):
        ...
        self.reproduction_reward_predator_female_config = config["reproduction_reward_predator_female"]
        self.reproduction_reward_prey_config = config["reproduction_reward_prey"]

        # Energy settings
        self.basal_energy_cost_predator_male = config["basal_energy_cost_predator_male"]
        self.basal_energy_cost_predator_female = config["basal_energy_cost_predator_female"]
        ...
        self.predator_female_creation_energy_threshold = config["predator_female_creation_energy_threshold"]
        ...
        # Sized (during smoke-testing) so that grazing income net of
        # basal_energy_cost_predator_female trends <= 0 on average -- the mechanical
        # guarantee that females cannot reach the reproduction threshold from
        # grazing alone and require nuptial gifts from males.
        self.female_grass_energy_gain_cap = config["female_grass_energy_gain_cap"]

        # Learning agents
        self.n_possible_predator_male = config["n_possible_predator_male"]
        self.n_possible_predator_female = config["n_possible_predator_female"]
        self.n_possible_prey = config["n_possible_prey"]

        self.n_initial_active_predator_male = config["n_initial_active_predator_male"]
        self.n_initial_active_predator_female = config["n_initial_active_predator_female"]
        ...
        self.n_initial_active_predator_male_min = config.get(
            "n_initial_active_predator_male_min", max(1, self.n_initial_active_predator_male // 3)
        )
        self.n_initial_active_predator_female_min = config.get(
            "n_initial_active_predator_female_min", max(1, self.n_initial_active_predator_female // 3)
        )
        self.n_initial_active_prey_min = config.get(
            "n_initial_active_prey_min", max(1, self.n_initial_active_prey // 3)
        )

        self.initial_energy_predator_male = config["initial_energy_predator_male"]
        self.initial_energy_predator_female = config["initial_energy_predator_female"]
        ...
        # Chebyshev-distance radius for nuptial-gift eligibility (male donor -> female recipient).
    ...
    def _init_reset_variables(self, seed):
        ...
        self.reproduction_blocked_due_to_capacity_predator_female = 0
        self.reproduction_blocked_due_to_capacity_prey = 0
        # Episode-level spawn counters
        self.spawned_predator_male = 0
        self.spawned_predator_female = 0
        self.spawned_prey = 0
        # Peak simultaneous active counts
        self.peak_active_predator_male = 0
        self.peak_active_predator_female = 0
        self.peak_active_prey = 0

        # Nuptial-gift episode totals (unidirectional: predator_male -> predator_female).
        self.total_gift_donated = 0.0
        self.total_gift_received = 0.0
        self.total_gift_kin = 0.0
        # Reverse-leg signal: gift share of each female's energy intake at the moment
        # she reproduces (lifetime_energy_from_gifts / total intake). One entry per
        # reproduction event this episode.
        self._female_reproduction_gift_shares: list[float] = []
        ...
        # aggregates per step
        self.active_num_predator_male = 0
        self.active_num_predator_female = 0
        self.active_num_prey = 0

        n_pred_male_min = min(self.n_initial_active_predator_male_min, self.n_initial_active_predator_male)
        n_pred_female_min = min(self.n_initial_active_predator_female_min, self.n_initial_active_predator_female)
        n_prey_min = min(self.n_initial_active_prey_min, self.n_initial_active_prey)
        n_initial_predator_male = int(self.rng.integers(n_pred_male_min, self.n_initial_active_predator_male + 1))
        n_initial_predator_female = int(self.rng.integers(n_pred_female_min, self.n_initial_active_predator_female + 1))
        n_initial_prey = int(self.rng.integers(n_prey_min, self.n_initial_active_prey + 1))

        self.agents = []
        for i in range(n_initial_predator_male):
            agent_id = f"predator_male_{i}"
            self.agents.append(agent_id)
            self._register_new_agent(agent_id, is_founder=True)
        for i in range(n_initial_predator_female):
            agent_id = f"predator_female_{i}"
            self.agents.append(agent_id)
            self._register_new_agent(agent_id, is_founder=True)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.active_num_predator_male = sum(1 for a in self.predator_positions if "predator_male" in a)
        self.active_num_predator_female = sum(1 for a in self.predator_positions if "predator_female" in a)
    ...
    def step(self, action_dict):
        ...
        # 4c) Predator engagements: males hunt, females graze (skip terminated)
        predator_snapshot = tuple(self.predator_positions.keys())
        for agent in predator_snapshot:
            if self.terminations.get(agent):
                continue
            if "predator_male" in agent:
                self._handle_male_hunting(agent)
            else:
                self._handle_female_grazing(agent)
        ...
        # Use snapshots to avoid iterating over newly spawned agents in this step.
        # Only predator_female ever reproduces.
        predator_female_snapshot = tuple(a for a in self.predator_positions if "predator_female" in a)
        prey_snapshot = tuple(self.prey_positions.keys())
        energies = self.agent_energies
        pred_female_thr = self.predator_female_creation_energy_threshold
        prey_thr = self.prey_creation_energy_threshold

        for agent in predator_female_snapshot:
            if energies[agent] >= pred_female_thr:
                self._handle_female_reproduction(agent)
        ...
        episode_done = (
            self.active_num_prey <= 0
            or self.active_num_predator_male <= 0
            or self.active_num_predator_female <= 0
        )
        if episode_done:
        ...
        n_pred_male = sum(1 for a in self.agents if "predator_male" in str(a))
        n_pred_female = sum(1 for a in self.agents if "predator_female" in str(a))
        n_prey = sum(1 for a in self.agents if "prey" in str(a))
        if n_pred_male > self.peak_active_predator_male:
            self.peak_active_predator_male = n_pred_male
        if n_pred_female > self.peak_active_predator_female:
            self.peak_active_predator_female = n_pred_female
    ...
    def _build_live_genome_metrics(self) -> dict[str, float]:
        ...
        grouped: dict[str, list[float]] = {"predator_male": [], "predator_female": [], "prey": []}
        for agent_id in self.agents:
            agent_str = str(agent_id)
            genome = self.agent_genomes.get(agent_str)
            if genome is not None:
                key = self._policy_group(agent_str)
                grouped[key].append(float(genome.male_donation_rate))
        metrics: dict[str, float] = {}
        for species, values in grouped.items():
            if values:
                arr = np.array(values)
                p25, p50, p75 = np.percentile(arr, [25, 50, 75])
                metrics[f"{species}_male_donation_rate_mean"] = float(np.mean(arr))
                metrics[f"{species}_male_donation_rate_std"] = float(np.std(arr))
                metrics[f"{species}_male_donation_rate_p25"] = float(p25)
                metrics[f"{species}_male_donation_rate_p50"] = float(p50)
                metrics[f"{species}_male_donation_rate_p75"] = float(p75)
                metrics[f"{species}_count"] = float(len(values))
            else:
                metrics[f"{species}_male_donation_rate_mean"] = 0.0
                metrics[f"{species}_male_donation_rate_std"] = 0.0
                metrics[f"{species}_male_donation_rate_p25"] = 0.0
                metrics[f"{species}_male_donation_rate_p50"] = 0.0
                metrics[f"{species}_male_donation_rate_p75"] = 0.0
    ...
    def _inherit_genome(self, agent_id: str, parent_agent_id: Optional[str], *, is_founder: bool) -> Optional[Genome]:
        ...
        if self.genome_neutral_drift_control:
            # Sever genome from reproductive success: the actual mother still pays
            ...
            # template is a uniformly random currently-alive agent from the
            # *parent's* policy group (always predator_female here, since only
            # females reproduce) instead of the true mother. Deliberately keyed off
            # the parent's group, not the offspring's own (possibly male) group --
            # the thing being severed is "did this mother's genome cause her to
            # reproduce", not "does this child's sex match some unrelated agent".
            parent_group = self._policy_group(parent_agent_id)
            candidates = [
                str(aid) for aid in self.agents
                if str(aid) in self.agent_genomes and self._policy_group(str(aid)) == parent_group
            ]
            if candidates:
    ...
    def _is_kin(self, agent_a: str, agent_b: str) -> bool:
        ...
        parent_a = self.agent_parents.get(agent_a)
        parent_b = self.agent_parents.get(agent_b)
        if agent_b == parent_a or agent_a == parent_b:
            return True
        return parent_a is not None and parent_a == parent_b

    def _apply_nuptial_gift(self, agent: str, energy_gain: float) -> float:
        """Male hunting gain: donate a nuptial gift to nearby predator_female neighbors.

        Meal-sharing model: donation is a fraction of this step's catch only, so a
        ...
        radius = self.cooperation_range
        neighbors = [
            other
            for other, pos in self.predator_positions.items()
            if "predator_female" in other
            and not self.terminations.get(other)
            and max(abs(pos[0] - position[0]), abs(pos[1] - position[1])) <= radius
        ]
        if not neighbors:
            return energy_gain

        donation_total = donation_rate * energy_gain
        share = donation_total / len(neighbors)
        kin_donation = 0.0
        for neighbor in neighbors:
            self.agent_energies[neighbor] += share
            self.grid_world_state[0, *self.agent_positions[neighbor]] = self.agent_energies[neighbor]
            neighbor_record = self.agent_stats_live.get(neighbor)
            if neighbor_record is not None:
                neighbor_record["energy_received"] += share
                neighbor_record["lifetime_energy_from_gifts"] += share
            if self._is_kin(agent, neighbor):
                kin_donation += share

        donor_record = self.agent_stats_live.get(agent)
        if donor_record is not None:
            donor_record["energy_donated"] += donation_total
            donor_record["energy_donated_to_kin"] += kin_donation

        self.total_gift_donated += donation_total
        self.total_gift_received += donation_total
        self.total_gift_kin += kin_donation

        return energy_gain - donation_total
    ...
    def _apply_time_step_update(self):
        ...
        for raw_agent in list(self.agents):
            ...
            if "predator_male" in agent:
                layer = 0
                energy_decay = self.basal_energy_cost_predator_male
            elif "predator_female" in agent:
                layer = 0
                energy_decay = self.basal_energy_cost_predator_female
            else:
                layer = 1
                energy_decay = self.basal_energy_cost_prey
    ...
    def _handle_energy_starvation(self, agent):
        ...
        if "predator_male" in agent:
            self.active_num_predator_male -= 1
        elif "predator_female" in agent:
            self.active_num_predator_female -= 1
        else:
            self.active_num_prey -= 1

    def _handle_male_hunting(self, agent):
        self._per_agent_step_deltas.setdefault(
            agent,
            {
                "decay": 0.0,
        ...
        if caught_prey:
            ...
            energy_gain = self._apply_nuptial_gift(agent, prey_energy)
    ...
    def _handle_female_grazing(self, agent):
        """Predator_female foraging: grazes grass like prey, but energy gain per
        graze is capped by female_grass_energy_gain_cap (a partial-browsing
        model -- unlike prey, which fully consumes the patch, a female only
        takes up to the cap and leaves the remainder for prey). This makes
        predator-females compete with prey for the same grass patches, an
        intended omnivory side effect -- see README.md."""
        if self.terminations.get(agent):
            return
        self._per_agent_step_deltas.setdefault(
            agent,
        ...
        female_position = tuple(self.agent_positions[agent])
        caught_grass = next(
            (g for g, pos in self.grass_positions.items() if "grass" in g and np.array_equal(female_position, pos)), None
        )
        if caught_grass:
            self.agents_just_ate.add(agent)
            self.rewards[agent] = self._get_role_specific("reward_predator_female_graze_grass", agent)
            raw_grass_energy = float(self.grass_energies[caught_grass])
            energy_gain = min(raw_grass_energy, self.female_grass_energy_gain_cap)

            self.agent_energies[agent] += energy_gain
            self._per_agent_step_deltas[agent]["eat"] = energy_gain
            self.grid_world_state[0, *female_position] = self.agent_energies[agent]

            remaining_grass_energy = max(raw_grass_energy - energy_gain, 0.0)
            self.grass_energies[caught_grass] = remaining_grass_energy
            self.grid_world_state[2, *female_position] = remaining_grass_energy

            female_record = self.agent_stats_live.get(agent)
            if female_record is not None:
                female_record["times_ate"] += 1
                female_record["energy_gained"] += energy_gain
                female_record["lifetime_energy_from_grazing"] += energy_gain
                female_record["cumulative_reward"] += self.rewards[agent]
            evt = self.agent_event_log.get(agent)
            if evt is not None:
                evt.setdefault("eating_events", []).append(
                    {
            ...
            self.rewards[agent] = self._get_role_specific("reward_predator_female_step", agent)
            female_record = self.agent_stats_live.get(agent)
            if female_record is not None:
                female_record["cumulative_reward"] += self.rewards[agent]

    def _handle_female_reproduction(self, agent):
        self._per_agent_step_deltas.setdefault(
            agent,
            {"decay": 0.0, "move": 0.0, "eat": 0.0, "repro": 0.0},
        )
        if self.agent_energies[agent] < self.predator_female_creation_energy_threshold:
            return

        # Offspring sex is drawn independently of genome/mutation (an even, unbiased
        # sex ratio); the male_donation_rate genome value is still inherited from the
        # mother regardless of which sex the offspring turns out to be (sex-limited
        # expression -- see utils/genome.py).
        child_sex = "predator_male" if self.rng.random() < 0.5 else "predator_female"
        new_agent = self._alloc_new_id(child_sex)
        if not new_agent:
            self.reproduction_blocked_due_to_capacity_predator_female += 1
            self.truncations["__all__"] = True
            print(
                f"[WARN] {child_sex} ID pool exhausted at step {self.current_step}. "
                f"Raise n_possible_{child_sex} in config."
            )
            return
        ...
        if child_sex == "predator_male":
            self.spawned_predator_male += 1
        else:
            self.spawned_predator_female += 1
        ...
        if new_position is None:
            raise RuntimeError(f"No free spawn position available for offspring of {agent}.")

        self.agent_positions[new_agent] = new_position
        self.predator_positions[new_agent] = new_position

        offspring_energy = (
            self.initial_energy_predator_male if child_sex == "predator_male" else self.initial_energy_predator_female
        )
        ...
        if parent_record is not None:
            ...
            gifts = parent_record.get("lifetime_energy_from_gifts", 0.0)
            grazing = parent_record.get("lifetime_energy_from_grazing", 0.0)
            total_intake = gifts + grazing
            if total_intake > 0.0:
                self._female_reproduction_gift_shares.append(gifts / total_intake)
        ...
        self.rewards[agent] = self._get_role_specific("reproduction_reward_predator_female", agent)
        if parent_record is not None:
    ...
    def get_state_snapshot(self):
        return {
            ...
            "active_num_predator_male": self.active_num_predator_male,
            "active_num_predator_female": self.active_num_predator_female,
    ...
    def restore_state_snapshot(self, snapshot):
        ...
        self.active_num_predator_male = snapshot["active_num_predator_male"]
        self.active_num_predator_female = snapshot["active_num_predator_female"]
        ...
        self.agent_genomes = {
            aid: Genome(**{trait: genome_data.get(trait, GENOME_FIELD_DEFAULTS.get(trait, 1.0)) for trait in active_genome_traits})
    ...
    def _build_possible_agent_ids(self):
        agent_ids = []
        for i in range(self.n_possible_predator_male):
            agent_ids.append(f"predator_male_{i}")
        for i in range(self.n_possible_predator_female):
            agent_ids.append(f"predator_female_{i}")
    ...
    def _register_new_agent(self, agent_id: str, parent_agent_id: Optional[str] = None, *, is_founder: bool = False):
        ...
        self.agent_stats_live[agent_id] = {
            ...
            "energy_gained": 0.0,
            "energy_donated": 0.0,
            "energy_donated_to_kin": 0.0,
            "energy_received": 0.0,
            "lifetime_energy_from_grazing": 0.0,
            "lifetime_energy_from_gifts": 0.0,
            "avg_energy_sum": 0.0,
            "avg_energy_steps": 0,
    ...
    def _finalize_agent_record(self, agent_id: str, cause: Optional[str] = None):
        ...
        if "predator_male" in agent_id:
            info.setdefault("species", "predator_male")
        elif "predator_female" in agent_id:
            info.setdefault("species", "predator_female")
        elif "prey" in agent_id:
    ...
    def _build_episode_training_metrics(self) -> dict[str, float]:
        ...
        metrics = {}
        grouped_records = {"predator_male": [], "predator_female": [], "prey": []}
        for agent_id, record in self._iter_all_agent_records():
            if agent_id.startswith("predator_male"):
                grouped_records["predator_male"].append(record)
            elif agent_id.startswith("predator_female"):
                grouped_records["predator_female"].append(record)
            elif agent_id.startswith("prey"):
                grouped_records["prey"].append(record)

        for species, records in grouped_records.items():
            for trait in ("male_donation_rate",):
                values = [
                    float(record["genome"][trait])
                    for record in records
                    if isinstance(record.get("genome"), dict) and trait in record["genome"]
                ]
                if values:
                    arr = np.array(values)
                    p25, p50, p75 = np.percentile(arr, [25, 50, 75])
                    metrics[f"{species}_{trait}_mean"] = float(np.mean(arr))
                    metrics[f"{species}_{trait}_std"] = float(np.std(arr))
                    metrics[f"{species}_{trait}_p25"] = float(p25)
                    metrics[f"{species}_{trait}_p50"] = float(p50)
                    metrics[f"{species}_{trait}_p75"] = float(p75)
                else:
                    metrics[f"{species}_{trait}_mean"] = 0.0
                    metrics[f"{species}_{trait}_std"] = 0.0
                    metrics[f"{species}_{trait}_p25"] = 0.0
                    metrics[f"{species}_{trait}_p50"] = 0.0
                    metrics[f"{species}_{trait}_p75"] = 0.0
            ...
            if records:
                ...
                metrics[f"{species}_energy_donated_mean"] = float(
                    np.mean([float(record.get("energy_donated", 0.0)) for record in records])
                )
                metrics[f"{species}_energy_received_mean"] = float(
                    np.mean([float(record.get("energy_received", 0.0)) for record in records])
                )
            else:
                ...
                metrics[f"{species}_energy_donated_mean"] = 0.0
                metrics[f"{species}_energy_received_mean"] = 0.0

        # Nuptial-gift flow totals (unidirectional predator_male -> predator_female).
        metrics["gift_energy_donated_total"] = float(self.total_gift_donated)
        metrics["gift_energy_received_total"] = float(self.total_gift_received)
        metrics["gift_local_relatedness_proxy"] = (
            float(self.total_gift_kin / self.total_gift_donated) if self.total_gift_donated > 0.0 else 0.0
        )

        # Reverse-leg signal: for each female reproduction event, what fraction of her
        # lifetime energy intake (up to that point) came from gifts vs. her own grazing.
        # This is the direct mechanistic test of the "obligate gate" claim -- expected
        # to sit near 1.0 if female_grass_energy_gain_cap is tuned correctly. Unlike
        # cooperation_rate's individual-level coop_repro_spearman, a male's own
        # reproductive success is never directly observable (he never reproduces
        # himself), so this female-side share is the primary metric this module can
        # deliver for the reverse leg -- see README.md.
        if self._female_reproduction_gift_shares:
            shares = np.array(self._female_reproduction_gift_shares)
            metrics["female_reproduction_gift_share_mean"] = float(np.mean(shares))
            metrics["female_reproduction_gift_share_p50"] = float(np.percentile(shares, 50))
        else:
            metrics["female_reproduction_gift_share_mean"] = 0.0
            metrics["female_reproduction_gift_share_p50"] = 0.0
        metrics["female_reproduction_events_total"] = float(len(self._female_reproduction_gift_shares))

        metrics["predator_female_reproduction_blocked"] = float(self.reproduction_blocked_due_to_capacity_predator_female)
        metrics["prey_reproduction_blocked"] = float(self.reproduction_blocked_due_to_capacity_prey)
        metrics["predator_male_spawned_total"] = float(self.spawned_predator_male)
        metrics["predator_female_spawned_total"] = float(self.spawned_predator_female)
        metrics["prey_spawned_total"] = float(self.spawned_prey)
        metrics["peak_active_predator_male"] = float(self.peak_active_predator_male)
        metrics["peak_active_predator_female"] = float(self.peak_active_predator_female)
        ...
        metrics["predator_male_unique_ids_used"] = float(sum(1 for a in self.used_agent_ids if "predator_male" in a))
        metrics["predator_female_unique_ids_used"] = float(sum(1 for a in self.used_agent_ids if "predator_female" in a))
    ...
    def get_total_energy_by_type(self):
        ...
        - 'predator_male': total predator_male energy
        - 'predator_female': total predator_female energy
        ...
        for agent_id, energy in self.agent_energies.items():
            if "predator_male" in agent_id:
                energy_totals["predator_male"] += energy
            elif "predator_female" in agent_id:
                energy_totals["predator_female"] += energy
            elif "prey" in agent_id:
    ...
    def get_total_offspring_by_type(self):
        ...
        counts = {"predator_male": 0, "predator_female": 0, "prey": 0}
    ...
    def _place_predators(self, predator_list, predator_positions):
        ...
        for i, agent in enumerate(predator_list):
            ...
            initial_energy = (
                self.initial_energy_predator_male if "predator_male" in agent else self.initial_energy_predator_female
            )
            self.agent_energies[agent] = initial_energy
            self.grid_world_state[0, *pos] = initial_energy
