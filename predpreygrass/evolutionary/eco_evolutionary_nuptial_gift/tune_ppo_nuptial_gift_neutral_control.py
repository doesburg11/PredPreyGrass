"""
Neutral-drift control for the eco_evolutionary_nuptial_gift environment.

Identical to tune_ppo_nuptial_gift.py in every respect except the environment
config: genome_neutral_drift_control=True severs genome inheritance from
reproductive success (offspring genome templates come from a random currently-
alive agent from the mother's policy group -- always predator_female, since only
females reproduce -- rather than the actual mother) while leaving population/
energy/spatial dynamics (and thus local relatedness) unchanged. Any
male_donation_rate drift observed in this run is attributable purely to
mutation + finite-population sampling noise, not selection.

Compare this run's live_genome/*_male_donation_rate_mean trajectory directly
against a real run. See README.md.
"""

from predpreygrass.evolutionary.eco_evolutionary_nuptial_gift.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.evolutionary.eco_evolutionary_nuptial_gift.config.config_env_eco_evolutionary_neutral_control import config_env
...
if __name__ == "__main__":
    ...
    version = f"ECO_EVOLUTION_NUPTIAL_GIFT_NEUTRAL_CONTROL{seed_tag}"
