...
class EpisodeReturn(RLlibCallback):
    ...
    def on_episode_end(
        ...
        **kwargs,
        ...
        Logs the total and average rewards separately for predator_male,
        predator_female, and prey.
        """
        self.num_episodes += 1
        ...
        def _group_of(agent_id: str) -> Optional[str]:
        ...
        group_totals = {"predator_male": 0.0, "predator_female": 0.0, "prey": 0.0}

        for agent_id, agent_ep in episode.agent_episodes.items():
            total = agent_ep.get_return()
            group = _group_of(agent_id)
        ...
        print(f"  - Predator (male):   Total = {group_totals['predator_male']:.2f}")
        print(f"  - Predator (female): Total = {group_totals['predator_female']:.2f}")
        print(f"  - Prey:              Total = {group_totals['prey']:.2f}")
        ...
        for group, totals in group_rewards.items():
            if totals:
                ...
                log_value(f"{group}_episode_return_p25", float(p25))
                log_value(f"{group}_episode_return_p50", float(p50))
                log_value(f"{group}_episode_return_p75", float(p75))

        # Optional: normalize returns by lifetime (if the env exposes lifetime_steps in infos)
        infos = self._episode_last_infos(episode)
        lifetimes = {"predator_male": [], "predator_female": [], "prey": []}
        return_per_life = {"predator_male": [], "predator_female": [], "prey": []}
        for aid, info in infos.items():
            ...
            if group is None:
                continue
            lifetimes[group].append(life)
            return_per_life[group].append(float(final_ret) / float(life))

        for group in ("predator_male", "predator_female", "prey"):
            if lifetimes[group]:
                log_value(f"{group}_lifetime_steps_median", float(np.median(lifetimes[group])))
                log_value(f"{group}_return_per_lifetime_mean", float(np.mean(return_per_life[group])))
    ...
    def on_episode_step(
        ...
        **kwargs,
        ...
        live_metrics = getattr(resolved_env, "_last_live_genome_metrics", None)
        if not isinstance(live_metrics, dict):
            return
        for metric_name, metric_value in live_metrics.items():
            if isinstance(metric_value, (int, float, np.integer, np.floating)):
                metrics_logger.log_value(f"live_genome/{metric_name}", float(metric_value))
