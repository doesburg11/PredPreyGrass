...
def plot(data: dict[str, list[int]], out_path: Path):
    ...
    for i, strategy in enumerate(STRATEGIES):
        ...
        ax.scatter(
            ...
            s=22, color=color, alpha=0.5, edgecolors="#fcfcfb", linewidths=0.5, zorder=3,
        )
    ...
    ax.set_ylabel("survival time (steps, log scale)", color="#52514e")
    ax.set_xticks(range(len(STRATEGIES)))
    n_per = [len(data[s]) for s in STRATEGIES]
    ax.set_xticklabels([f"{s}\n(n={n})" for s, n in zip(STRATEGIES, n_per)])
    ax.set_title(
