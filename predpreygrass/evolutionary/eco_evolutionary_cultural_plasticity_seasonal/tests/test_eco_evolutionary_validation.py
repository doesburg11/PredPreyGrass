...
from predpreygrass.evolutionary.eco_evolutionary_cultural_plasticity_seasonal.config.config_env_eco_evolutionary import config_env
from predpreygrass.evolutionary.eco_evolutionary_cultural_plasticity_seasonal.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.evolutionary.eco_evolutionary_cultural_plasticity_seasonal.utils.episode_return_callback import EpisodeReturn
from predpreygrass.evolutionary.eco_evolutionary_cultural_plasticity_seasonal.utils.genome import (
    Genome,
    founder_genome,
    mutate_genome,
)
...
def test_dialect_match_bonus_uses_target_dialect_not_local_majority():
    # The bonus target is the environment's current_step-driven target dialect,
    # NOT the local majority -- this is the module's core behavior change vs.
    # eco_evolutionary_cultural_plasticity. Construct a case where the agent's
    # dialect mismatches the local majority but matches the target (0 at
    # current_step=0, since no step() has been called): bonus must still be
    # granted.
    bonus = 2.0
    env = _make_test_env(
    ...
    env.reset(seed=888)
    env.rewards = {}
    assert env._current_target_dialect() == 0
    ...
    env.agent_live_dialect[prey] = 0  # matches target (0), mismatches local majority
    env.agent_live_dialect[neighbor] = 1  # sole neighbor -> local majority is dialect 1
    assert env._local_majority_dialect(prey) == 1
...
def test_dialect_matching_majority_but_not_target_grants_no_bonus():
    # Mirror case: agent's dialect matches the local majority but mismatches
    # the current target dialect -- must NOT get the bonus. This is the
    # regression guard for the semantic change (bonus is target-based, not
    # majority-based).
    bonus = 2.0
    env = _make_test_env(
        overrides={"coordination_bonus_multiplier": bonus, "culture_range": 5, "n_initial_active_prey": 2}
    ...
    _place_agent(env, prey, (3, 3))
    _place_agent(env, neighbor, (3, 4))
    env.agent_live_dialect[prey] = 1  # mismatches target (0), matches local majority
    env.agent_live_dialect[neighbor] = 1  # sole neighbor -> local majority is dialect 1
    assert env._local_majority_dialect(prey) == 1
...
def test_current_target_dialect_cycles_through_all_dialects():
    length = 5
    env = _make_test_env(overrides={"n_dialects": 4, "dialect_season_length_steps": length})
    env.reset(seed=42)

    for step in (0, 1, length - 1):
        env.current_step = step
        assert env._current_target_dialect() == 0
    for step in (length, length + 1, 2 * length - 1):
        env.current_step = step
        assert env._current_target_dialect() == 1
    env.current_step = 3 * length
    assert env._current_target_dialect() == 3
    env.current_step = 4 * length  # a full tour through all 4 dialects -> back to 0
    assert env._current_target_dialect() == 0


def test_current_target_dialect_disabled_with_single_dialect():
    # With n_dialects=1, the target is always dialect 0 regardless of
    # current_step/dialect_season_length_steps -- a sanity/regression guard.
    env = _make_test_env(overrides={"n_dialects": 1, "dialect_season_length_steps": 3})
    env.reset(seed=7)
