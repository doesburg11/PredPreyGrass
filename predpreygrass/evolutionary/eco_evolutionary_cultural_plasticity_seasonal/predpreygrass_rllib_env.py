...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _initialize_from_config(self):
        ...
        self.dialect_season_length_steps = int(config.get("dialect_season_length_steps", 25))
    ...
    def _current_target_dialect(self) -> int:
        """
        Square-wave seasonal cycle on which dialect currently earns the
        coordination bonus: rotates through all n_dialects, dialect_season_
        length_steps steps per phase, repeating. Phase is purely a function
        of the per-episode step counter, so it resets with every reset()
        like the rest of episode state (same pattern as
        base_environment_seasonal's _current_season_multiplier).
        """
        return (self.current_step // self.dialect_season_length_steps) % self.n_dialects

    def _dialect_match_bonus(self, agent: str) -> float:
        """Coordination-game multiplier: >1.0 if agent's live dialect matches
        the environment's current target dialect (see _current_target_dialect),
        else 1.0. NOT the local majority -- local-majority copying
        (_apply_cultural_learning) is still how an agent's live dialect
        updates, it's just no longer what determines the bonus, so a
        seasonal flip in the target creates a real, repeated fitness gap
        between fast- and slow-adapting agents. Also updates the
        episode-level match-rate bookkeeping used by the eco-evolution
        metrics."""
        agent = str(agent)
        ...
        if own_dialect is None:
            return 1.0
        target = self._current_target_dialect()
        matched = target == own_dialect
        if is_predator:
