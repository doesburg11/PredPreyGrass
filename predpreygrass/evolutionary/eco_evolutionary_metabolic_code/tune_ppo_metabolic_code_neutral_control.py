"""
Neutral-drift control for the eco_evolutionary_metabolic_code environment.

Identical to tune_ppo_metabolic_code.py in every respect except the environment
...
dynamics unchanged. Any mean-WRONG-loci-count drift observed in this run is
attributable purely to mutation + finite-population sampling noise, not selection.

Compare this run's live_haystack/*_mean_wrong_loci trajectory directly against
...
from predpreygrass.evolutionary.eco_evolutionary_metabolic_code.config.config_env_eco_evolutionary_neutral_control import config_env
from predpreygrass.evolutionary.eco_evolutionary_metabolic_code.utils.episode_return_callback import EpisodeReturn
from predpreygrass.evolutionary.eco_evolutionary_metabolic_code.utils.networks import build_multi_module_spec
...
def get_config_ppo():
    ...
    if num_cpus == 32:
        from predpreygrass.evolutionary.eco_evolutionary_metabolic_code.config.config_ppo_gpu_eco_evolutionary import config_ppo
    elif num_cpus == 8:
        from predpreygrass.evolutionary.eco_evolutionary_metabolic_code.config.config_ppo_cpu_eco_evolutionary import config_ppo
    else:
...
if __name__ == "__main__":
    ...
    version = f"ECO_EVOLUTION_METABOLIC_CODE_NEUTRAL_CONTROL{seed_tag}"
