...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _initialize_from_config(self):
        ...
        # eco_evolutionary_metabolic_rate / eco_evolutionary_investment: regulates
        # predator population growth through each predator's own recent hunting
        # success (a Holling-type handling-time mechanism) rather than a
        # population-level rule. 0 disables the cooldown.
        ...
        # unlike eco_evolutionary_investment, it is not this module's genome trait.
        # One heritable trait per module, matching every prior module's isolation
        # discipline.
        ...
        # eco_evolutionary_metabolic_rate. See RESULTS.md.
        self.genome_neutral_drift_control = bool(config.get("genome_neutral_drift_control", False))
        self.genome_config = {
            "haystack_num_loci": config.get("haystack_num_loci", 10),
            "haystack_founder_probs": config.get("haystack_founder_probs", {}),
            "haystack_mutation": config.get("haystack_mutation", {}),
        }
        # Combinatorial-trait fitness bonus: a solved genome (zero WRONG loci and a
        # successful joint plastic-locus guess this life) multiplies energy gain
        # from that step onward. See utils/genome.py and README.md.
        self.haystack_bonus_multiplier = float(config.get("haystack_bonus_multiplier", 1.5))
    ...
    def _init_reset_variables(self, seed):
        ...
        # Combinatorial-trait per-individual lifetime-search state. None of these
        # are heritable -- they are re-derived from the genome at birth and evolve
        # only within one agent's own lifetime (see _update_haystack_solving).
        self.agent_has_wrong = {}
        self.agent_solved = {}
        self.agent_solved_step = {}
        ...
        self._last_live_haystack_metrics: dict = {}
    ...
    def step(self, action_dict):
        ...
        # Step 1b: Per-individual lifetime search over this step's combinatorial
        # genome trial (see _update_haystack_solving). Runs before this step's
        # engagements so an agent that solves this step already earns the bonus
        # on this step's catch/graze.
        self._update_haystack_solving()
        ...
        self._last_live_haystack_metrics = self._build_live_haystack_metrics()
        return self.observations, self.rewards, self.terminations, self.truncations, self.infos

    def _update_haystack_solving(self):
        """One lifetime-search trial per living, unsolved, viable agent.

        "Viable" means zero WRONG loci -- an agent with any WRONG locus can
        never achieve a full match this lifetime and is skipped entirely (its
        has_wrong flag is fixed at birth). This is the per-individual learning
        channel from Hinton & Nowlan (1987), deliberately independent of the
        shared PPO policy that governs movement/hunting behavior.
        """
        for agent in self.agents:
            if self.agent_solved.get(agent, False) or self.agent_has_wrong.get(agent, False):
                continue
            genome = self.agent_genomes.get(agent)
            if genome is None:
                continue
            if attempt_resolve(genome.loci, self.rng):
                self.agent_solved[agent] = True
                self.agent_solved_step[agent] = self.current_step

    def _build_live_haystack_metrics(self) -> dict[str, float]:
        """Genome/solving-state distributions of the currently alive population."""
        ...
        for species, agent_ids in grouped.items():
            if agent_ids:
                wrong = [self.agent_genomes[a].num_wrong for a in agent_ids]
                correct = [self.agent_genomes[a].num_correct for a in agent_ids]
                plastic = [self.agent_genomes[a].num_plastic for a in agent_ids]
                solved_flags = [self.agent_solved.get(a, False) for a in agent_ids]
                metrics[f"{species}_mean_wrong_loci"] = float(np.mean(wrong))
                metrics[f"{species}_mean_correct_loci"] = float(np.mean(correct))
                metrics[f"{species}_mean_plastic_loci"] = float(np.mean(plastic))
                metrics[f"{species}_fraction_solved"] = float(np.mean(solved_flags))
                steps_to_solve = [
                    self.agent_solved_step[a] - self.agent_stats_live[a].get("birth_step", self.agent_solved_step[a])
                    for a in agent_ids
                    if self.agent_solved.get(a, False) and a in self.agent_stats_live
                ]
                metrics[f"{species}_mean_steps_to_solve"] = float(np.mean(steps_to_solve)) if steps_to_solve else 0.0
                metrics[f"{species}_count"] = float(len(agent_ids))
            else:
                metrics[f"{species}_mean_wrong_loci"] = 0.0
                metrics[f"{species}_mean_correct_loci"] = 0.0
                metrics[f"{species}_mean_plastic_loci"] = 0.0
                metrics[f"{species}_fraction_solved"] = 0.0
                metrics[f"{species}_mean_steps_to_solve"] = 0.0
    ...
    def _handle_predator_engagement(self, agent):
        ...
        if caught_prey:
            ...
            if self.agent_solved.get(agent, False):
                energy_gain *= self.haystack_bonus_multiplier
    ...
    def get_state_snapshot(self):
        return {
            ...
            "agent_has_wrong": self.agent_has_wrong.copy(),
            "agent_solved": self.agent_solved.copy(),
            "agent_solved_step": self.agent_solved_step.copy(),
    ...
    def restore_state_snapshot(self, snapshot):
        ...
        self.agent_has_wrong = snapshot.get("agent_has_wrong", {}).copy()
        self.agent_solved = snapshot.get("agent_solved", {}).copy()
        self.agent_solved_step = snapshot.get("agent_solved_step", {}).copy()
    ...
    def _register_new_agent(self, agent_id: str, parent_agent_id: Optional[str] = None, *, is_founder: bool = False):
        ...
        if genome is not None:
            ...
            self.agent_has_wrong[agent_id] = bool(genome.has_wrong)
        else:
            genome_dict = None
            self.agent_has_wrong[agent_id] = False
        self.agent_solved[agent_id] = False
    ...
    def _build_episode_training_metrics(self) -> dict[str, float]:
        ...
        for species, agent_ids in grouped_ids.items():
            ...
            if genome_ids:
                wrong = [self.agent_genomes[aid].num_wrong for aid in genome_ids]
                correct = [self.agent_genomes[aid].num_correct for aid in genome_ids]
                plastic = [self.agent_genomes[aid].num_plastic for aid in genome_ids]
                solved_flags = [self.agent_solved.get(aid, False) for aid in genome_ids]
                metrics[f"{species}_mean_wrong_loci"] = float(np.mean(wrong))
                metrics[f"{species}_mean_correct_loci"] = float(np.mean(correct))
                metrics[f"{species}_mean_plastic_loci"] = float(np.mean(plastic))
                metrics[f"{species}_fraction_solved"] = float(np.mean(solved_flags))
                ...
                for aid in genome_ids:
                    if not self.agent_solved.get(aid, False):
                        continue
                    record = self.agent_stats_live.get(aid) or self.agent_stats_completed.get(aid)
                    if record is None:
                        continue
                    birth_step = record.get("birth_step")
                    solved_step = self.agent_solved_step.get(aid)
                    if birth_step is not None and solved_step is not None:
                        steps_to_solve.append(solved_step - birth_step)
                metrics[f"{species}_mean_steps_to_solve"] = float(np.mean(steps_to_solve)) if steps_to_solve else 0.0
            else:
                metrics[f"{species}_mean_wrong_loci"] = 0.0
                metrics[f"{species}_mean_correct_loci"] = 0.0
