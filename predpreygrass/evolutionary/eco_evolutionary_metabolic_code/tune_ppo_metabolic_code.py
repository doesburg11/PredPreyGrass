"""
Trains the eco_evolutionary_metabolic_code environment with PPO using the Ray RLlib
new API stack.

The environment is a predator-prey-grass grid world in which agents carry a
heritable combinatorial genome trait: a length-L locus code, each locus
CORRECT/WRONG/PLASTIC relative to an implicit fixed target (Hinton & Nowlan,
1987 needle-in-haystack design). An agent can only achieve a full match if it
carries zero WRONG loci; PLASTIC loci are searched fresh every step within
that agent's own lifetime -- a genuine per-individual lifetime-search "learning"
...
if __name__ == "__main__":
    ...
    version = f"ECO_EVOLUTION_METABOLIC_CODE{seed_tag}"
