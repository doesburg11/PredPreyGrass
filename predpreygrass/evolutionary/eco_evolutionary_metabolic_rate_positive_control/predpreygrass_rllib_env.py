...
from predpreygrass.evolutionary.eco_evolutionary_metabolic_rate_positive_control.utils.genome import (
    Genome,
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _initialize_from_config(self):
        ...
        # Exponent for gain scaling: gain = food * metabolic_rate**alpha. This module
        # sets alpha=3.0 (super-linear) as a positive control -- see config comment.
    ...
    def _build_episode_training_metrics(self) -> dict[str, float]:
        ...
        # These keys are always set below (NaN when not yet computable), even on the
        # very first call, so Ray Tune's CSV logger -- which fixes its column set from
        # whichever keys exist in the first reported iteration and silently drops any
        # key that only starts appearing later -- always has a column for them. A prior
        # version of this method only set these keys inside the `len(pairs) >= 4`
        # branch; with a small starting population, that branch could be skipped on
        # iteration 1, permanently excluding the column from progress.csv even once the
        # population grew large enough for it to be computed every iteration after.
        for species in ("prey", "predator"):
            metrics[f"{species}_mr_repro_spearman"] = float("nan")
            for i in range(1, 5):
                metrics[f"{species}_mr_repro_rate_q{i}"] = float("nan")
