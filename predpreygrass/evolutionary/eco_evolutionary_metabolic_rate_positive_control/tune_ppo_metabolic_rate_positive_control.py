"""
Trains the eco_evolutionary_metabolic_rate_positive_control environment with PPO using the Ray RLlib
new API stack.

The environment is a predator-prey-grass grid world in which agents carry a
heritable metabolic_rate genome trait. Energy gain scales sub-linearly with
metabolic_rate (digestive saturation, exponent alpha<1) while energy cost scales
linearly, creating a policy-dependent interior optimum — the two-way Darwin/Baldwin
...
from predpreygrass.evolutionary.eco_evolutionary_metabolic_rate_positive_control.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.evolutionary.eco_evolutionary_metabolic_rate_positive_control.config.config_env_eco_evolutionary import config_env
from predpreygrass.evolutionary.eco_evolutionary_metabolic_rate_positive_control.utils.episode_return_callback import EpisodeReturn
from predpreygrass.evolutionary.eco_evolutionary_metabolic_rate_positive_control.utils.networks import build_multi_module_spec
...
def get_config_ppo():
    ...
    if torch.cuda.is_available():
        from predpreygrass.evolutionary.eco_evolutionary_metabolic_rate_positive_control.config.config_ppo_gpu_eco_evolutionary import config_ppo
    else:
        from predpreygrass.evolutionary.eco_evolutionary_metabolic_rate_positive_control.config.config_ppo_cpu_eco_evolutionary import config_ppo
    return config_ppo
...
if __name__ == "__main__":
    ...
    version = f"ECO_EVOLUTION_METABOLIC_RATE_POSITIVE_CONTROL{seed_tag}"
