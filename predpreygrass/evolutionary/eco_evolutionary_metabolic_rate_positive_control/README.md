# Metabolic Rate — Positive Control

## What this module is

This is a deliberate **positive control**, not a new biologically-motivated trait design.
It is a clone of `eco_evolutionary_metabolic_rate` (Trial 3) with exactly one change:
`metabolic_rate_alpha` is set to `3.0` (super-linear gain, see config comment) instead of
the parent module's sub-linear `0.4-0.7` range. This flips the trait from "genuine interior
optimum" (this document's original framing below) to "overwhelming, monotonic advantage for
high metabolic_rate, regardless of policy quality."

**Why:** after Trial 3 (null, even after sharpening the gradient) and Trial 8/9 (dual-inheritance
cultural trait, also null/flat so far), the project has never confirmed that this training
pipeline — genome inheritance, mutation, differential reproduction, population-level metric
aggregation — can detect *any* selection signal at all. If a trait with an unmissably large,
unambiguous fitness gradient still comes back flat, that's strong evidence the problem is
structural (population size, reproduction-event count, genetic drift dominance, or the
shared-per-species-policy architecture) rather than any individual trait's design. See
`RESULTS.md` for results and `predpreygrass/evolutionary/RESULTS.md` for how this fits the
project's trial history.

Everything below this point is the parent module's original design rationale (the sub-linear,
interior-optimum trait). It's kept for background on the mechanism and metrics — genome
inheritance, mutation, and the `{species}_mr_repro_spearman` metric all work identically here
— but the specific tradeoff argument (interior optimum, `α ≈ 0.7`) does **not** apply to this
variant. Read `energy_gain = food * metabolic_rate ** 3.0` in place of `** 0.7` throughout.

---

# Metabolic Rate as a Second Genome Trait (parent module's original design rationale)
