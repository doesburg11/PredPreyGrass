"""
Trial 6 (population scaling): trains eco_evolutionary_investment at ~4x the
grid/population scale of R4-R7, to test whether R7's null selection-drift result
was masked by a small-population neutral-drift noise floor rather than reflecting
a genuinely absent selection signal.

Identical to tune_ppo_investment.py except the environment config
(config_env_eco_evolutionary_scaled: grid_size 25->50, initial population and
grass 4x) and the absence of the R4 fixed-investment-fraction sweep flag, which
doesn't apply here. See predpreygrass/evolutionary/RESULTS.md, Trial 6.

First run should be a short single-seed pilot (--max-iters 100, matching the R4
pilot pattern) to confirm sustainability holds at the larger scale before
committing to a full multi-seed real-vs-neutral-control replication.
"""

from predpreygrass.evolutionary.eco_evolutionary_investment.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.evolutionary.eco_evolutionary_investment.config.config_env_eco_evolutionary_scaled import config_env
...
if __name__ == "__main__":
    ...
    version = f"ECO_EVOLUTION_INVESTMENT_SCALED{seed_tag}"
