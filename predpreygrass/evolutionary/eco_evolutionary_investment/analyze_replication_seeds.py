"""
R7 analysis: aggregate multi-seed replication runs (real satiation-throttle
config vs. neutral-drift control) and compare offspring_investment_fraction
drift magnitude between the two groups.

Expects experiment directories under ~/simulation_results/ray_results/ named:
  PPO_ECO_EVOLUTION_INVESTMENT_SEED<seed>_<timestamp>                (real)
  PPO_ECO_EVOLUTION_INVESTMENT_NEUTRAL_CONTROL_SEED<seed>_<timestamp> (control)

as produced by tune_ppo_investment.py / tune_ppo_investment_neutral_control.py
...
Usage:
    python predpreygrass/evolutionary/eco_evolutionary_investment/analyze_replication_seeds.py
"""
...
REAL_PATTERN = re.compile(r"^PPO_ECO_EVOLUTION_INVESTMENT_SEED(\d+)_")
CONTROL_PATTERN = re.compile(r"^PPO_ECO_EVOLUTION_INVESTMENT_NEUTRAL_CONTROL_SEED(\d+)_")
...
def find_seed_runs(pattern: re.Pattern) -> dict[int, Path]:
    ...
    for exp_dir in RAY_RESULTS_DIR.glob("PPO_ECO_EVOLUTION_INVESTMENT*"):
...
def main():
    ...
    report_hunt_fits(real_runs, control_runs, trait="offspring_investment_fraction", prefix="live_investment")
