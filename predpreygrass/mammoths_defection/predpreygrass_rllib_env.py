...
class PredPreyGrass(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        self.pending_captures = {}
        frozen_prey = set(pending_captures.keys())
        ...
        self._process_agent_movements(action_dict, frozen_prey=frozen_prey)
        ...
        if pending_captures:
            for prey_id, participants in pending_captures.items():
                self._handle_team_capture(prey_id, participants, action_dict)

        # All engagements involve prey (grass only after capture resolution)
        ...
        # Schedule captures for next step based on current adjacency
        self._schedule_pending_captures()
    ...
    def _process_agent_movements(self, action_dict, frozen_prey=None):
        """
        Process movement and grid updates for all agents (non-vectorized, simple loop).
        """
        if frozen_prey is None:
            frozen_prey = set()
        for agent in action_dict.keys():
            ...
            if "prey" in agent and agent in frozen_prey:
                new_position = old_position
                self._last_move_block_reason[agent] = "frozen"
            else:
    ...
    def _is_cooperator_action(self, predator_id, action, prey_pos):
        if action is None:
            return False
        try:
            action = int(action)
            ...
            return False
        if "type_1" in predator_id:
            move_vec = self.action_to_move_tuple_type_1_agents.get(action)
        elif "type_2" in predator_id:
            move_vec = self.action_to_move_tuple_type_2_agents.get(action)
        else:
            return False
        if move_vec is None:
            return False
    ...
    def _handle_team_capture(self, prey_id, participants, action_dict):
        ...
        live_participants = [
            pid for pid in participants if pid in self.agent_positions and not self.terminations.get(pid)
        ]
        ...
        for pid in live_participants:
            if self._is_cooperator_action(pid, action_dict.get(pid), prey_pos):
                cooperators.append(pid)
            else:
                defectors.append(pid)

        prey_energy = float(self.agent_energies[prey_id])
        coop_energy = sum(self.agent_energies[pid] for pid in cooperators)
        success = coop_energy > prey_energy + self.team_capture_margin

        coop_cost = float(self.team_capture_coop_cost)
        if coop_cost:
            for pid in cooperators:
                self.agent_energies[pid] -= coop_cost
                self._per_agent_step_deltas.setdefault(pid, {"decay": 0.0, "move": 0.0, "eat": 0.0, "repro": 0.0})
                self._per_agent_step_deltas[pid]["eat"] -= coop_cost
                    ...
                    evt.setdefault("failed_eating_events", []).append(
                        {
                            ...
                            "bite_size": 0.0,
                            "energy_before": float(self.agent_energies[pid] + (coop_cost if pid in cooperators else 0.0)),
                            "energy_after": float(self.agent_energies[pid]),
                            "team_capture": participant_count > 1,
                            "predator_list": live_participants,
        ...
        self.team_capture_helper_total += participant_count

        energy_share_pid = prey_energy / participant_count
        for pid in live_participants:
        ...
        if participant_count > 1:
            self.team_capture_events.append({"t": int(self.current_step), "position": tuple(prey_pos), "helpers": participant_count})
