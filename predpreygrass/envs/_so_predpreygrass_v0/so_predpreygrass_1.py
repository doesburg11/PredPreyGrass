...
from predpreygrass.envs._so_predpreygrass_v0.so_predpreygrass_base_1 import PredPreyGrass as predpreygrass
...
class raw_env(AECEnv, EzPickle):
    ...
    def step(self, action):
        ...
        #print("step", self.steps, "agent", agent, "reward", self.rewards[agent])
        #print("rewards before", self.rewards)
        ...
        #print(f"rewards after: {self.rewards}")
        self.steps += 1
        self._cumulative_rewards = dict(zip(self.agents, [0 for _ in self.agents]))
        #self._cumulative_rewards[self.agent_selection] = 0 # dict(zip(self.agents, [0 for _ in self.agents]))
        self.agent_selection = self._agent_selector.next()
