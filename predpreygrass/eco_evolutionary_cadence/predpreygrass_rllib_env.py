...
action is only executed when the agent's cooldown reaches zero.  A `move_available`
channel in the observation lets the policy condition on whether this step counts.
...
class PredPreyGrass(MultiAgentEnv):
    ...
    def _initialize_from_config(self):
        ...
        self.include_move_available_in_obs = config.get("include_move_available_in_obs", True)
    ...
    def _n_obs_channels(self) -> int:
        return (
            ...
            + (1 if self.include_speed_in_obs else 0)
            + (1 if self.include_move_available_in_obs else 0)
        )
    ...
    def _get_observation(self, agent):
        ...
        ch = self.num_obs_channels
        if self.include_speed_in_obs:
            genome = self.agent_genomes.get(str(agent))
            if genome is not None:
                obs[ch, :, :] = float(genome.speed)  # already in [0.0, 1.0]
            ch += 1
        if self.include_move_available_in_obs:
            can_move = 1.0 if self.agent_move_cooldowns.get(str(agent), 0) == 0 else 0.0
            obs[ch, :, :] = can_move
        return obs

    def _empty_observation(self, agent):
        obs_range = self.predator_obs_range if "predator" in agent else self.prey_obs_range
        return np.zeros((self._n_obs_channels(), obs_range, obs_range), dtype=np.float32)
