...
if __name__ == "__main__":
    ...
    # Initialize previous positions and traces for all agents
    previous_positions = {agent: position for agent, position in env.agent_positions.items()}
    trace_grids = {agent: np.full((env.x_grid_size, env.y_grid_size), ".", dtype=str) for agent in env.agents}

    # ASCII arrow mapping for movement directions
    direction_arrows = {
        (-1, 0): "^",  # Up
        (1, 0): "v",   # Down
        (0, -1): "<",  # Left
    ...
    # Smooth line trace symbols
    trace_symbols = {
        (-1, 0): "┃",  # Vertical movement up
        (1, 0): "┃",   # Vertical movement down
        (0, -1): "━",  # Horizontal movement left
        (0, 1): "━",   # Horizontal movement right
    }
    ...
    for step in range(num_steps):
        ...
        # Populate the grids with movement arrows and traces
        for agent, position in env.agent_positions.items():
            ...
            # Update trace grid for the agent
            if (dx, dy) in trace_symbols:
                trace_symbol = trace_symbols[(dx, dy)]
                trace_grids[agent][prev_x, prev_y] = trace_symbol

            # Add arrow to the predator or prey grid
            if "predator" in agent:
                predator_grid[x, y] = arrow
            elif "prey" in agent:
                prey_grid[x, y] = arrow

        # Merge traces into the grids (last 5 steps only)
        for agent, trace_grid in trace_grids.items():
            if "predator" in agent:
                predator_grid = np.where(trace_grid != ".", trace_grid, predator_grid)
            elif "prey" in agent:
                prey_grid = np.where(trace_grid != ".", trace_grid, prey_grid)
