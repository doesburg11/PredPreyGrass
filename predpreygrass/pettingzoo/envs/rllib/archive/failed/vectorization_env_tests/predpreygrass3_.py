...
class PredPreyGrass(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        # Step 1: Convert energy dictionaries to NumPy arrays for vectorized updates
        predator_energies_array = np.array(list(self.predator_energies.values()))
        prey_energies_array = np.array(list(self.prey_energies.values()))
        grass_energies_array = np.array(list(self.grass_energies.values()))

        # Step 2: Vectorized Energy depletion for predators and prey
        predator_energies_array -= self.energy_loss_per_step_predator
        prey_energies_array -= self.energy_loss_per_step_prey
        ...
        self.predator_energies = dict(zip(self.predator_energies.keys(), predator_energies_array))
        self.prey_energies = dict(zip(self.prey_energies.keys(), prey_energies_array))
        self.agent_energies = {**self.predator_energies, **self.prey_energies}
        ...
        self.grid_world_state[1:3, :, :] = 0  # Resets only channels 1 (Predator) & 2 (Prey)

        # Update the grid world state with new positions and energies for predators and prey
    ...
    def _print_movement_table(self, action_dict, predator_position_after_action, prey_new_unresolved_positions, resolved_positions, colliding_predator_agents, colliding_prey_agents):
        ...
        print("\nPrey Position Table:")
        print("{:<12} {:<15} {:<15} {:<10} {:<15} {:<15} {:<15} {:<20}".format(
            "Agent", "Tuple", "Energy", "Array", "Action", "Action Array", "Mew", "Resolved"
        ))
        print("-" * 120)
