...
class PredPreyGrass(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        # Step 3: Prepoare agent removals (Prey caught, Energy depleted)
        for agent in self.agents:
            ...
            if self.agent_energies[agent] <= 0:
                ...
                continue
            # Prey with positive energy is caught by predator
            elif "prey" in agent:
                ...
                for predator, predator_position in self.agent_positions.items():
                    if "predator" in predator and np.array_equal(
                        predator_position, prey_position
                    ):
                        # Prey is caught
                        ...
                        if agent in self.agent_positions:
                            # Assign penalty and observation before removal of prey
                            ...
                            truncations[agent] = False
                            self.num_prey -= 1
                            self.grid_world_state[2, *self.agent_positions[agent]] = (
                                0  # Clear the prey from the grid_world_state (channel 2)
                            )
                            del self.agent_positions[agent]
                            del self.agent_energies[agent]
                        ...
                        # Prey survives
                        observations[agent] = self._get_observation(agent)
                        #print(f"---{agent} survived. Reward: {self.reward_prey_step}")
                        rewards[agent] = self.reward_prey_step
                        self.cumulative_rewards[agent] += rewards[agent]
                        terminations[agent] = False
                        truncations[agent] = False  
                        break          #
            # Predator with positive energy did not catch prey
            elif "predator" in agent:
                observations[agent] = self._get_observation(agent)
                if rewards.get(agent) is None:
            ...
            # max_steps reached
                ...
                # TODO break to speed up done 
                observations[agent] = self._get_observation(
                    agent
                )  # Ensure last observation
                rewards[agent] = 0.0
                truncations[agent] = True
                terminations[agent] = False
            else:
                print(f"[DEBUG] Agent {agent} is not predator or prey. RAISE EXCEPTION.")
