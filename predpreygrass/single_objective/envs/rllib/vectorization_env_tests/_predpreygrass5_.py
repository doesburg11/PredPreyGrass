...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        self.verbose_movement: bool = True
        self.verbose_resulution_same_type_collisions: bool = True
        self.verbose_movement_table: bool = True
    ...
    def step(self, action_dict):
        ...
        # Step 1: Apply Energy Loss (Vectorized)
        if len(self.agent_energies) > 0:
            agent_energies = np.array(list(self.agent_energies.values()))

            # Ensure we have at least one agent before creating masks
            if len(agent_energies) > 0:
                is_predator = np.array([agent.startswith("predator") for agent in self.agent_energies.keys()])
                is_prey = np.array([agent.startswith("prey") for agent in self.agent_energies.keys()])

                # Apply energy loss only if there are valid agents
        ...
        # Grass energy regrowth (if grass exists)
        if len(self.grass_energies) > 0:
            grass_energies_array = np.array(list(self.grass_energies.values()))
            grass_energies_array = np.clip(grass_energies_array + self.energy_gain_per_step_grass, None, self.initial_energy_grass)
            self.grass_energies = dict(zip(self.grass_energies.keys(), grass_energies_array))

        # Step 2: Remove Agents with Zero Energy Before Doing Any Work
        agents_to_remove = [agent for agent, energy in self.agent_energies.items() if energy <= 0]

        for agent in agents_to_remove:
            self.terminations[agent] = True  # Mark for removal in RLlib
            self.observations[agent] = self._get_observation(agent)  # Final observation before removal

            if self.verbose_movement:
                print(f"Agent {agent} removed due to energy depletion.")

            # Remove agent from tracking structures
        ...
        # Step 3: **End Simulation Early if No Predators or Prey Remain**
        if self.current_num_predators == 0 and self.current_num_prey == 0:
            print("All predators and prey are gone. Ending simulation.")
            self.truncations = {agent: True for agent in self.agents}
            self.agents = []  # No active agents left
            return self.observations, self.rewards, self.terminations, self.truncations, self.infos

        # Step 4: Compute Position Updates Only If Necessary
        if self.current_num_predators > 0 or self.current_num_prey > 0:
            predator_positions_array = np.array(list(self.predator_positions.values())) if self.current_num_predators > 0 else np.array([])
            prey_positions_array = np.array(list(self.prey_positions.values())) if self.current_num_prey > 0 else np.array([])

            if predator_positions_array.shape[0] > 0:
                predator_actions_array = np.array([self.action_to_move_tuple[action_dict[agent]] for agent in self.predator_positions.keys()])
                predator_new_position = np.clip(predator_positions_array + predator_actions_array, [0, 0], [self.grid_size - 1, self.grid_size - 1])
                predator_position_after_action = predator_new_position.copy()
            else:
                predator_new_position = np.array([])
                predator_position_after_action = np.array([])

            if prey_positions_array.shape[0] > 0:
                prey_actions_array = np.array([self.action_to_move_tuple[action_dict[agent]] for agent in self.prey_positions.keys()])
                prey_new_unresolved_positions = np.clip(prey_positions_array + prey_actions_array, [0, 0], [self.grid_size - 1, self.grid_size - 1])
                prey_position_after_action = prey_new_unresolved_positions.copy()
            else:
                prey_new_unresolved_positions = np.array([])
                prey_position_after_action = np.array([])

            # Step 5: Detect & Resolve Collisions (Same Type)
            predator_agent_ids = np.array(list(self.predator_positions.keys())) if self.current_num_predators > 0 else np.array([])
            prey_agent_ids = np.array(list(self.prey_positions.keys())) if self.current_num_prey > 0 else np.array([])

            colliding_predator_agents, colliding_prey_agents = {}, {}
            ...
            # Resolve collisions
            if colliding_predator_agents:
                self._resolve_agents_of_same_type_collisions(colliding_predator_agents, predator_new_position)
            if colliding_prey_agents:
                self._resolve_agents_of_same_type_collisions(colliding_prey_agents, prey_new_unresolved_positions)

            # Step 6: Finalize Resolved Positions
            resolved_positions = {}
            if self.current_num_predators > 0:
                resolved_positions.update({agent: tuple(predator_new_position[i]) for i, agent in enumerate(self.predator_positions.keys())})
            if self.current_num_prey > 0:
                resolved_positions.update({agent: tuple(prey_new_unresolved_positions[i]) for i, agent in enumerate(self.prey_positions.keys())})

            # Step 7: Print Movement Table (Debugging)
        ...
        # Step 8: Update Active Agents List
