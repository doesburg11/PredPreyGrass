...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        self.reward_predator_step: float = 0.5
        ...
        self.observation_space = gymnasium.spaces.Box(
            low=-1.0, high=100.0, shape=self.obs_space_shape, dtype=np.float64
        )
    ...
    def step(self, action_dict):
            ...
            # Check if prey is caught
                ...
                for predator, predator_position in self.agent_positions.items():
                    if "predator" in predator and np.array_equal(predator_position, prey_position):
                        ...
                        self.episode_rewards[predator] += rewards[predator]
        ...
        # Step 5: Generate updated observations for all active agents
        for agent in self.agents:
            if agent in self.agent_positions:
                observations[agent] = self._get_observation(agent)
            else:
                # Assign zeroed-out observations for removed agents
                observations[agent] = np.zeros((self.num_obs_channels, self.max_obs_range, self.max_obs_range), dtype=np.float32)
    ...
    def _get_observation(self, agent):
        ...
        # Return a NumPy array instead of a dictionary
