...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        # Learning agents
        self.max_num_predators = 30
        self.max_num_prey = 40
        self.num_predators = 30
        self.num_prey = 40
...
if __name__ == "__main__":
    ...
    visualizer = GridVisualizer(grid_size, agents, trace_length=15)

    for step in range(100):
        #print(f"Step {step + 1}")

        # Randomly move agents
        env.step({agent: env.action_spaces[agent].sample() for agent in agents})
        agent_positions = env.agent_positions
        # Update visualization
        visualizer.update(agent_positions)
        sleep(0.0)
