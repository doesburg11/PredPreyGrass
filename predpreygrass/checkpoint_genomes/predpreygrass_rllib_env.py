...
class PredPreyGrass(MultiAgentEnv):
    def __init__(self, config=None):
        ...
        if isinstance(self.initial_energy_predator, dict):
            default_pred_energy = self.initial_energy_predator.get(
                "default", self.initial_energy_predator.get("type_1_predator", 0.0)
            )
            self.initial_energy_predator_by_type = {
                "type_1_predator": self.initial_energy_predator.get("type_1_predator", 0.0),
                "type_2_predator": self.initial_energy_predator.get("type_2_predator", 0.0),
                "default": default_pred_energy,
            }
            self.initial_energy_predator = default_pred_energy
        else:
            self.initial_energy_predator_by_type = {
                "type_1_predator": float(self.initial_energy_predator),
                "type_2_predator": float(self.initial_energy_predator),
                "default": float(self.initial_energy_predator),
            }
        initial_energy_prey = config.get("initial_energy_prey", 0.0)
        if isinstance(initial_energy_prey, dict):
            default_prey_energy = initial_energy_prey.get("default", initial_energy_prey.get("type_1_prey", 0.0))
            self.initial_energy_prey_by_type = {
                "type_1_prey": initial_energy_prey.get("type_1_prey", 0.0),
                "type_2_prey": initial_energy_prey.get("type_2_prey", 0.0),
                "default": default_prey_energy,
            }
            self.initial_energy_prey = default_prey_energy
        else:
            self.initial_energy_prey_by_type = {
                "type_1_prey": initial_energy_prey,
                "type_2_prey": initial_energy_prey,
                "default": initial_energy_prey,
            }
            self.initial_energy_prey = initial_energy_prey
        self.predator_creation_energy_threshold = config.get(
            "energy_treshold_creation_predator",
            config.get("predator_creation_energy_threshold", 0.0),
        )
        energy_treshold_prey = config.get("energy_treshold_creation_prey", None)
        if isinstance(energy_treshold_prey, dict):
            self.prey_creation_energy_thresholds = {
                "type_1_prey": energy_treshold_prey.get("type_1_prey", 0.0),
                "type_2_prey": energy_treshold_prey.get("type_2_prey", 0.0),
                "default": energy_treshold_prey.get("default", 0.0),
            }
        else:
            fallback_prey_threshold = config.get("prey_creation_energy_threshold", 0.0)
            if energy_treshold_prey is not None:
                fallback_prey_threshold = float(energy_treshold_prey)
            self.prey_creation_energy_thresholds = {
                "type_1_prey": fallback_prey_threshold,
                "type_2_prey": fallback_prey_threshold,
                "default": fallback_prey_threshold,
            }
        self.energy_loss_per_step_prey = config.get("energy_loss_per_step_prey", 0.0)
        if isinstance(self.energy_loss_per_step_prey, dict):
            self.energy_loss_per_step_type_1_prey = self.energy_loss_per_step_prey.get("type_1_prey", 0.0)
            self.energy_loss_per_step_type_2_prey = self.energy_loss_per_step_prey.get("type_2_prey", 0.0)
            self.energy_loss_per_step_prey_default = self.energy_loss_per_step_prey.get("default", 0.0)
        else:
        ...
        # Sexual reproduction settings (predators)
        self.predator_sexual_reproduction_enabled = bool(
            config.get("predator_sexual_reproduction_enabled", True)
        )
        self.predator_male_type = int(config.get("predator_male_type", 1))
        self.predator_female_type = int(config.get("predator_female_type", 2))
        self.predator_fertility_age_min = int(config.get("predator_fertility_age_min", 0))
        fertility_age_max = config.get("predator_fertility_age_max", None)
        self.predator_fertility_age_max = (
            None if fertility_age_max is None else int(fertility_age_max)
        )
        self.predator_male_mating_age_min = int(
            config.get("predator_male_mating_age_min", self.predator_fertility_age_min)
        )
        self.predator_mating_radius = int(config.get("predator_mating_radius", 1))
        self.predator_mating_energy_threshold = float(
            config.get("predator_mating_energy_threshold", self.predator_creation_energy_threshold)
        )
        self.predator_male_single_mating_per_step = bool(
            config.get("predator_male_single_mating_per_step", True)
        )
        self.predator_mating_parent_energy_share = float(
            config.get("predator_mating_parent_energy_share", 0.5)
        )
        offspring_prob = config.get("predator_offspring_type_prob", None)
        if isinstance(offspring_prob, dict):
            p1 = float(
                offspring_prob.get("type_1_predator", offspring_prob.get("type_1", 0.5))
            )
        elif offspring_prob is None:
            p1 = 0.5
        else:
            p1 = float(offspring_prob)
        p1 = min(max(p1, 0.0), 1.0)
        self.predator_offspring_type_prob = {1: p1, 2: 1.0 - p1}

        # Provisioning (adjacent energy transfers)
        self.provisioning_enabled = bool(config.get("provisioning_enabled", False))
        self.provisioning_radius = int(config.get("provisioning_radius", 1))
        self.provisioning_amount = float(config.get("provisioning_amount", 0.0))
        self.provisioning_cost_multiplier = float(
            config.get("provisioning_cost_multiplier", 0.0)
        )
        self.provisioning_min_donor_energy = float(
            config.get("provisioning_min_donor_energy", 0.0)
        )
        self.predator_child_age_max = int(config.get("predator_child_age_max", -1))
        self.predator_adult_age = int(
            config.get(
                "predator_adult_age",
                max(self.predator_child_age_max + 1, 0),
            )
        )

        # Sex-based energy economics (predators)
        self.predator_energy_init_by_sex = config.get("predator_energy_init_by_sex", None)
        self.predator_idle_cost_by_sex = config.get("predator_idle_cost_by_sex", None)
        if not isinstance(self.predator_idle_cost_by_sex, dict):
            raise ValueError(
                "predator_idle_cost_by_sex must be provided as a dict with 'male' and 'female' keys."
            )
        if "male" not in self.predator_idle_cost_by_sex or "female" not in self.predator_idle_cost_by_sex:
            raise ValueError(
                "predator_idle_cost_by_sex must include both 'male' and 'female' entries."
            )
        action_cost_by_sex = config.get("predator_action_cost_by_sex", None)
        if isinstance(action_cost_by_sex, dict):
            self.predator_attack_cost_by_sex = action_cost_by_sex.get("attack", {})
            self.predator_move_cost_by_sex = action_cost_by_sex.get("move", {})
        else:
            self.predator_attack_cost_by_sex = {}
            self.predator_move_cost_by_sex = {}
        self.predator_kill_efficiency_by_sex = config.get("predator_kill_efficiency_by_sex", None)
        self.predator_failed_attack_damage_by_sex = config.get("predator_failed_attack_damage_by_sex", None)
        self.predator_reproduction_cost_by_sex = config.get("predator_reproduction_cost_by_sex", None)
        ...
        # Channel layout: 0 walls, 1 type_1_predators, 2 type_2_predators,
        # 3 type_1_prey, 4 type_2_prey, 5 grass
        self.wall_channel = 0
        self.predator_type_1_channel = 1
        self.predator_type_2_channel = 2
        self.type_1_prey_channel = 3
        self.type_2_prey_channel = 4
        self.grass_channel = 5
        ...
        # Team capture is always enabled in stag_hunt
        ...
        def _generate_action_map(range_size: int):
            ...
            return {
                ...
                for i, (dx, dy) in enumerate(
                    (dx, dy) for dx in range(-delta, delta + 1) for dy in range(-delta, delta + 1)
                )
            }
    ...
    def step(self, action_dict):
        ...
        if self.predator_sexual_reproduction_enabled:
            self._handle_predator_sexual_reproduction(predator_snapshot)
        else:
        ...
        # Step 7.5: Optional provisioning transfers after reproduction.
        self._handle_provisioning_transfers()
        ...
        for agent in self.agents:
            ...
            deltas = self._per_agent_step_deltas.get(
                agent,
                {"decay": 0.0, "move": 0.0, "eat": 0.0, "repro": 0.0, "provision": 0.0},
            )
            ...
            step_data[agent] = {
                ...
                "energy_provision": deltas["provision"],
                "age": age,
    ...
    def _apply_time_step_update(self):
        ...
        for agent in list(self.agents):
            ...
            if "predator" in agent:
                energy_decay = self._get_predator_idle_cost(agent)
            else:
    ...
    def _split_action(self, agent, action):
        """Return (move_action, None) for all agents (join/defect removed)."""
        if isinstance(action, dict):
            move = action.get("move", action.get("action", 0))
        elif isinstance(action, (tuple, list)) or (
            isinstance(action, np.ndarray) and action.shape != ()
        ):
    ...
    def _process_agent_movements(self, action_dict):
        ...
        for agent in action_dict.keys():
            ...
            move_action, _ = self._split_action(agent, action)
            ...
            if "predator" in agent:
                pred_channel = self._get_predator_channel(agent)
                self.predator_positions[agent] = tuple(new_position)
                self.grid_world_state[pred_channel, old_position[0], old_position[1]] = 0
                self.grid_world_state[pred_channel, new_position[0], new_position[1]] = self.agent_energies[agent]
                move_cost = self._get_predator_move_cost(agent)
                if move_cost and move_vector != (0, 0):
    ...
    def _get_move(self, agent, action: int, *, move_vector=None):
        ...
        if new_position in self.wall_positions:
            ...
            if (
                self.grid_world_state[self.predator_type_1_channel, *new_position] > 0
                or self.grid_world_state[self.predator_type_2_channel, *new_position] > 0
            ):
                new_position = current_position
    ...
    def _iter_moore_positions(self, center, radius: int):
        """Yield grid positions within Chebyshev distance <= radius (clipped to grid)."""
        cx, cy = center
        r = max(int(radius), 0)
        xlo = max(0, cx - r)
        xhi = min(self.grid_size - 1, cx + r)
        ylo = max(0, cy - r)
        yhi = min(self.grid_size - 1, cy + r)
        for x in range(xlo, xhi + 1):
            for y in range(ylo, yhi + 1):
                yield (x, y)

    def _is_predator_male(self, agent_id: str) -> bool:
        return agent_id.startswith(f"type_{self.predator_male_type}_predator")

    def _is_predator_female(self, agent_id: str) -> bool:
        return agent_id.startswith(f"type_{self.predator_female_type}_predator")

    def _is_fertile_female(self, agent_id: str) -> bool:
        if not self._is_predator_female(agent_id):
            return False
        age = self.agent_ages.get(agent_id, 0)
        if age < self.predator_fertility_age_min:
            return False
        if self.predator_fertility_age_max is not None and age > self.predator_fertility_age_max:
            return False
        return True

    def _is_predator_child(self, agent_id: str) -> bool:
        if self.predator_child_age_max < 0:
            return False
        return self.agent_ages.get(agent_id, 0) <= self.predator_child_age_max

    def _sample_predator_offspring_type(self) -> int:
        p1 = float(self.predator_offspring_type_prob.get(1, 0.5))
        return 1 if self.rng.random() < p1 else 2

    def _get_sex_value(self, mapping, sex: str, default: float = 0.0) -> float:
        if not isinstance(mapping, dict):
            return float(default)
        if sex in mapping:
            return float(mapping[sex])
        if "default" in mapping:
            return float(mapping["default"])
        return float(default)

    def _predator_sex(self, agent_id: str) -> Optional[str]:
        if self._is_predator_male(agent_id):
            return "male"
        if self._is_predator_female(agent_id):
            return "female"
        return None

    def _get_initial_predator_energy(self, agent_id: str) -> float:
        sex = self._predator_sex(agent_id)
        if sex is not None and self.predator_energy_init_by_sex is not None:
            return self._get_sex_value(
                self.predator_energy_init_by_sex, sex, self.initial_energy_predator
            )
        if agent_id.startswith("type_1_predator"):
            return float(self.initial_energy_predator_by_type.get("type_1_predator", self.initial_energy_predator))
        if agent_id.startswith("type_2_predator"):
            return float(self.initial_energy_predator_by_type.get("type_2_predator", self.initial_energy_predator))
        return float(self.initial_energy_predator)

    def _get_predator_idle_cost(self, agent_id: str) -> float:
        sex = self._predator_sex(agent_id)
        if sex is not None and self.predator_idle_cost_by_sex is not None:
            return self._get_sex_value(self.predator_idle_cost_by_sex, sex, 0.0)
        return 0.0

    def _get_predator_move_cost(self, agent_id: str) -> float:
        sex = self._predator_sex(agent_id)
        if sex is not None:
            return self._get_sex_value(self.predator_move_cost_by_sex, sex, 0.0)
        return 0.0

    def _get_predator_attack_cost(self, agent_id: str) -> float:
        sex = self._predator_sex(agent_id)
        if sex is not None:
            return self._get_sex_value(self.predator_attack_cost_by_sex, sex, 0.0)
        return 0.0

    def _get_predator_kill_efficiency(self, agent_id: str) -> float:
        sex = self._predator_sex(agent_id)
        if sex is not None and self.predator_kill_efficiency_by_sex is not None:
            return self._get_sex_value(self.predator_kill_efficiency_by_sex, sex, 1.0)
        return 1.0

    def _get_predator_failed_attack_damage(self, agent_id: str) -> float:
        sex = self._predator_sex(agent_id)
        if sex is not None and self.predator_failed_attack_damage_by_sex is not None:
            return self._get_sex_value(self.predator_failed_attack_damage_by_sex, sex, 0.0)
        return 0.0

    def _get_predator_reproduction_extra_cost(self, agent_id: str) -> float:
        sex = self._predator_sex(agent_id)
        if sex is not None and self.predator_reproduction_cost_by_sex is not None:
            return self._get_sex_value(self.predator_reproduction_cost_by_sex, sex, 0.0)
        return 0.0
    ...
    def _handle_team_capture(self, prey_id):
        ...
        nearby_predators = [
            h for h in self._predators_in_moore_neighborhood(prey_pos) if h not in self.agents_just_ate
        ]
        if not nearby_predators:
            return False

        joiners = list(nearby_predators)
        ...
        for pid in joiners:
            attack_cost = self._get_predator_attack_cost(pid)
            if attack_cost:
                self.agent_energies[pid] -= attack_cost
                self._per_agent_step_deltas.setdefault(
                    pid,
                    {"decay": 0.0, "move": 0.0, "eat": 0.0, "repro": 0.0, "provision": 0.0},
                )
                self._per_agent_step_deltas[pid]["eat"] -= attack_cost
                self.grid_world_state[self._get_predator_channel(pid), *self.agent_positions[pid]] = self.agent_energies[pid]
        if total_pred_energy <= prey_energy + self.team_capture_margin:
            for pid in joiners:
                fail_damage = self._get_predator_failed_attack_damage(pid)
                if fail_damage:
                    self.agent_energies[pid] -= fail_damage
                    self._per_agent_step_deltas.setdefault(
                        pid,
                        {"decay": 0.0, "move": 0.0, "eat": 0.0, "repro": 0.0, "provision": 0.0},
                    )
                    self._per_agent_step_deltas[pid]["eat"] -= fail_damage
                    self.grid_world_state[self._get_predator_channel(pid), *self.agent_positions[pid]] = self.agent_energies[pid]
                    ...
                    evt.setdefault("failed_eating_events", []).append(
                        {
                            ...
                            "attack_cost": float(self._get_predator_attack_cost(pid)),
                            "failed_attack_damage": float(self._get_predator_failed_attack_damage(pid)),
                        }
                    )
        ...
        joiner_energy_pool = prey_energy
        ...
        for pid in joiners:
            ...
            if self.team_capture_equal_split:
                energy_share_pid = joiner_energy_pool / helper_count
            else:
                energy_share_pid = (
                    joiner_energy_pool * (helper_energy_snapshot[pid] / total_helper_energy)
                    if total_helper_energy > 0
                    else 0.0
                )
            efficiency = self._get_predator_kill_efficiency(pid)
            energy_gain = energy_share_pid * efficiency
            self.agent_energies[pid] += energy_gain
            self._per_agent_step_deltas[pid]["eat"] += energy_gain
            self.grid_world_state[self._get_predator_channel(pid), *self.agent_positions[pid]] = self.agent_energies[pid]
            ...
            if pred_record is not None:
                ...
                pred_record["energy_gained"] += energy_gain
            info = self._pending_infos.setdefault(pid, {})
            info["team_capture_helpers"] = helper_count
            info["team_capture_energy_gain"] = energy_gain
            evt = self.agent_event_log.get(pid)
            if evt is not None:
                evt.setdefault("eating_events", []).append(
                    {
                        ...
                        "predator_list": joiners,
                        "attack_cost": float(self._get_predator_attack_cost(pid)),
                        "kill_efficiency": float(efficiency),
                    }
                )
    ...
    def _handle_predator_sexual_reproduction(self, predator_snapshot):
        if not predator_snapshot:
            return
        if self.predator_male_type == self.predator_female_type:
            return

        energies = self.agent_energies
        ages = self.agent_ages
        females = [
            pid
            for pid in predator_snapshot
            if self._is_predator_female(pid) and not self.terminations.get(pid)
        ]
        if not females:
            return
        ...
        pos_to_predator = {
            pos: pid
            for pid, pos in self.predator_positions.items()
            if not self.terminations.get(pid)
        }
        used_males = set()

        if self.predator_energy_init_by_sex is not None:
            max_child_energy = max(
                self._get_sex_value(self.predator_energy_init_by_sex, "male", self.initial_energy_predator),
                self._get_sex_value(self.predator_energy_init_by_sex, "female", self.initial_energy_predator),
            )
        else:
            max_child_energy = max(
                float(self.initial_energy_predator_by_type.get("type_1_predator", self.initial_energy_predator)),
                float(self.initial_energy_predator_by_type.get("type_2_predator", self.initial_energy_predator)),
            )
        parent_cost_base = max(0.0, max_child_energy * self.predator_mating_parent_energy_share)

        for female in females:
            if not self._is_fertile_female(female):
                continue
            female_gate = max(
                self.predator_mating_energy_threshold,
                parent_cost_base + self._get_predator_reproduction_extra_cost(female),
            )
            if energies.get(female, 0.0) < female_gate:
                continue

            female_pos = self.agent_positions.get(female)
            if female_pos is None:
                continue

            candidates = []
            for pos in self._iter_moore_positions(female_pos, self.predator_mating_radius):
                male = pos_to_predator.get(pos)
                if male is None or male == female:
                    continue
                if not self._is_predator_male(male):
                    continue
                if self.predator_male_single_mating_per_step and male in used_males:
                    continue
                if ages.get(male, 0) < self.predator_male_mating_age_min:
                    continue
                male_gate = max(
                    self.predator_mating_energy_threshold,
                    parent_cost_base + self._get_predator_reproduction_extra_cost(male),
                )
                if energies.get(male, 0.0) < male_gate:
                    continue
            ...
            male = max(candidates, key=lambda pid: energies.get(pid, 0.0))
            if self._spawn_sexual_predator_offspring(female, male):
                if self.predator_male_single_mating_per_step:
                    used_males.add(male)

    def _spawn_sexual_predator_offspring(self, female_id: str, male_id: str) -> bool:
        child_type = self._sample_predator_offspring_type()
        new_agent = self._alloc_new_id("predator", child_type)
        if not new_agent:
            alt_type = 1 if child_type == 2 else 2
            new_agent = self._alloc_new_id("predator", alt_type)
            if not new_agent:
                cap = (
                    self.n_possible_type_1_predators
                    if child_type == 1
                    else self.n_possible_type_2_predators
                )
                self.reproduction_blocked_due_to_capacity_predator += 1
                key = f"predator_{child_type}"
                if key not in self._printed_capacity_exhaustion:
                    msg = (
                        f"[PredPreyGrass] Predator ID pool exhausted for type {child_type} "
        ...
        # Register child + parent bookkeeping
        self.agent_last_reproduction[female_id] = self.current_step
        self.agent_last_reproduction[male_id] = self.current_step
        self._register_new_agent(new_agent, parent_agent_id=female_id)

        for parent_id in (female_id, male_id):
            self.agent_live_offspring_ids[parent_id].append(new_agent)
            self.agent_offspring_counts[parent_id] += 1
            parent_record = self.agent_stats_live.get(parent_id)
            if parent_record is not None:
                parent_record["offspring_count"] += 1
                evt = self.agent_event_log.get(parent_id)
                if evt is not None:
                    evt.setdefault("reproduction_events", []).append(
                        {
                            "t": int(self.current_step),
                            "child_id": new_agent,
                            "mate_id": male_id if parent_id == female_id else female_id,
                        }
                    )
        ...
        new_position = self._find_available_spawn_position(
            self.agent_positions[female_id], occupied_positions
        )
        ...
        child_energy = self._get_initial_predator_energy(new_agent)
        parent_cost = max(0.0, child_energy * self.predator_mating_parent_energy_share)
        female_extra = self._get_predator_reproduction_extra_cost(female_id)
        male_extra = self._get_predator_reproduction_extra_cost(male_id)
        female_total_cost = parent_cost + female_extra
        male_total_cost = parent_cost + male_extra

        if (
            self.agent_energies.get(female_id, 0.0) < female_total_cost
            or self.agent_energies.get(male_id, 0.0) < male_total_cost
        ):
            return False

        # Energy transfers
        self.agent_energies[new_agent] = child_energy
        self.agent_energies[female_id] -= female_total_cost
        self.agent_energies[male_id] -= male_total_cost
        ...
        self._per_agent_step_deltas[female_id]["repro"] -= female_total_cost
        self._per_agent_step_deltas[male_id]["repro"] -= male_total_cost

        # Write energies into the grid
        self.grid_world_state[self._get_predator_channel(new_agent), *new_position] = child_energy
        self.grid_world_state[self._get_predator_channel(female_id), *self.agent_positions[female_id]] = self.agent_energies[
            female_id
        ]
        self.grid_world_state[self._get_predator_channel(male_id), *self.agent_positions[male_id]] = self.agent_energies[
            male_id
        ]

        self.active_num_predators += 1

        # Rewards for both parents
        self.rewards[new_agent] = 0
        female_reward = self._get_type_specific("reproduction_reward_predator", female_id)
        male_reward = self._get_type_specific("reproduction_reward_predator", male_id)
        self.rewards[female_id] = female_reward
        self.rewards[male_id] = male_reward
        self.debug_predator_total_reward += float(female_reward) + float(male_reward)
        self.debug_predator_repro_events += 1

        for parent_id, reward in ((female_id, female_reward), (male_id, male_reward)):
            parent_record = self.agent_stats_live.get(parent_id)
            if parent_record is not None:
                parent_record["cumulative_reward"] += reward
                evt = self.agent_event_log.get(parent_id)
                if evt is not None:
                    evt.setdefault("reward_events", []).append(
                        {
                            "t": int(self.current_step),
                            "reproduction_reward": float(reward),
    ...
    def _handle_predator_reproduction(self, agent):
        ...
        if self.agent_energies[agent] >= self.predator_creation_energy_threshold:
            ...
            child_energy = self._get_initial_predator_energy(new_agent)
            self.agent_energies[new_agent] = child_energy
            self.agent_energies[agent] -= child_energy
            ...
            self.grid_world_state[self._get_predator_channel(new_agent), *new_position] = child_energy
            self.grid_world_state[self._get_predator_channel(agent), *self.agent_positions[agent]] = self.agent_energies[agent]
    ...
    def _handle_prey_reproduction(self, agent):
        ...
        if self.agent_energies[agent] >= self._get_prey_creation_threshold(agent):
            ...
            child_energy = self._get_initial_prey_energy(new_agent)
            self.agent_energies[new_agent] = child_energy
            self.agent_energies[agent] -= child_energy
            self._per_agent_step_deltas[agent]["repro"] = -child_energy
    ...
    def _handle_provisioning_transfers(self):
        if not self.provisioning_enabled:
            return
        if self.provisioning_amount <= 0.0:
            return

        donor_cost = self.provisioning_amount * (1.0 + self.provisioning_cost_multiplier)
        if donor_cost <= 0.0:
            return
        ...
        for donor in tuple(self.predator_positions.keys()):
            if self.terminations.get(donor):
                continue
            if self.agent_ages.get(donor, 0) < self.predator_adult_age:
                continue

            donor_energy = self.agent_energies.get(donor, 0.0)
            if donor_energy < max(self.provisioning_min_donor_energy, donor_cost):
                continue

            donor_pos = self.agent_positions.get(donor)
            if donor_pos is None:
                continue

            best_recipient = None
            best_energy = None
            donor_offspring = self.agent_live_offspring_ids.get(donor, [])
            if not donor_offspring:
                continue

            for pos in self._iter_moore_positions(donor_pos, self.provisioning_radius):
                recipient = pos_to_predator.get(pos)
                if recipient is None or recipient == donor:
                    continue
                if self.terminations.get(recipient):
                    continue

                if recipient not in donor_offspring:
                    continue
                is_child = self._is_predator_child(recipient)
                if not is_child:
                    continue

                recipient_energy = self.agent_energies.get(recipient, 0.0)
                if donor_energy <= recipient_energy:
                    continue

                if best_recipient is None or recipient_energy < best_energy:
                    best_recipient = recipient
                    best_energy = recipient_energy

            if best_recipient is None:
                continue

            # Apply the transfer
            self.agent_energies[donor] -= donor_cost
            self.agent_energies[best_recipient] += self.provisioning_amount
            ...
            self._per_agent_step_deltas[donor]["provision"] -= donor_cost
            self._per_agent_step_deltas[best_recipient]["provision"] += self.provisioning_amount

            self.grid_world_state[self._get_predator_channel(donor), *self.agent_positions[donor]] = self.agent_energies[
                donor
            ]
            self.grid_world_state[
                self._get_predator_channel(best_recipient), *self.agent_positions[best_recipient]
            ] = self.agent_energies[best_recipient]
    ...
    def _build_action_space(self, agent_id):
        ...
        if "type_1" in agent_id:
            size = max(1, self.type_1_act_range**2)
        elif "type_2" in agent_id:
            size = max(1, self.type_2_act_range**2)
        else:
            raise ValueError(f"Unknown agent type in ID: {agent_id}")

        if "predator" in agent_id:
            return gymnasium.spaces.Discrete(size)
    ...
    def _get_prey_creation_threshold(self, agent_id: str) -> float:
        if agent_id.startswith("type_1_prey"):
            return self.prey_creation_energy_thresholds["type_1_prey"]
        if agent_id.startswith("type_2_prey"):
            return self.prey_creation_energy_thresholds["type_2_prey"]
        return self.prey_creation_energy_thresholds["default"]

    def _get_initial_prey_energy(self, agent_id: str) -> float:
        if agent_id.startswith("type_1_prey"):
            return self.initial_energy_prey_by_type["type_1_prey"]
        if agent_id.startswith("type_2_prey"):
    ...
    def _get_predator_channel(self, agent_id: str) -> int:
        if agent_id.startswith("type_1_predator"):
            return self.predator_type_1_channel
        if agent_id.startswith("type_2_predator"):
            return self.predator_type_2_channel
        return self.predator_type_1_channel

    def _get_agent_channel(self, agent_id: str) -> int:
        if "predator" in agent_id:
            return self._get_predator_channel(agent_id)
    ...
    def _place_predators(self, predator_list, predator_positions):
        ...
        for i, agent in enumerate(predator_list):
            ...
            init_energy = self._get_initial_predator_energy(agent)
            self.agent_energies[agent] = init_energy
            self.grid_world_state[self._get_predator_channel(agent), *pos] = init_energy
