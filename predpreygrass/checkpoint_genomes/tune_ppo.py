...
from predpreygrass.checkpoint_genomes.predpreygrass_rllib_env import PredPreyGrass
from predpreygrass.checkpoint_genomes.config.config_env_checkpoint_genomes import config_env
from predpreygrass.checkpoint_genomes.utils.episode_return_callback import EpisodeReturn
from predpreygrass.checkpoint_genomes.utils.networks import build_multi_module_spec

import ray
from ray.rllib.algorithms.ppo import PPOConfig
from ray.rllib.algorithms.ppo.torch.ppo_torch_learner import PPOTorchLearner
from ray.tune.registry import register_env
from ray.tune import Tuner, RunConfig, CheckpointConfig, Trainable
...
def get_config_ppo():
    ...
    if num_cpus == 32:
        from predpreygrass.checkpoint_genomes.config.config_ppo_gpu_checkpoint_genomes import config_ppo
    elif num_cpus == 8:
        from predpreygrass.checkpoint_genomes.config.config_ppo_cpu_checkpoint_genomes import config_ppo
    else:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train PPO for checkpoint_genomes.")
    parser.add_argument(
        "--restore-from",
        default=None,
        help="Checkpoint directory to restore from (overrides RESTORE_CHECKPOINT env var).",
    )
    return parser.parse_args()


def resolve_restore_path(arg_value: Optional[str]) -> Optional[Path]:
    if not arg_value:
        return None
    restore_path = Path(arg_value).expanduser()
    if not restore_path.exists():
        raise FileNotFoundError(f"restore-from checkpoint not found: {restore_path}")
    return restore_path
...
if __name__ == "__main__":
    ...
    restore_env = os.getenv("RESTORE_CHECKPOINT")
    restore_path = resolve_restore_path(args.restore_from or restore_env)
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    version = "SEXUAL_REPRODUCTION"
    if restore_path:
        experiment_name = f"{version}_RESTORE_{timestamp}"
    else:
    ...
    config_metadata = {
        ...
        "restore_from": str(restore_path) if restore_path else None,
    }
    ...
    if restore_path:
        trainable = PPOCheckpointTrainable
        param_space = {
            "algo_config": ppo_config,
            "restore_from_path": str(restore_path),
        }
    else:
        trainable = ppo_config.algo_class
        param_space = ppo_config

    # Use iterations since restore so each generation runs a fixed number of steps,
    # regardless of the absolute training_iteration embedded in the checkpoint.
    tuner = Tuner(
        trainable,
        ...
        run_config=RunConfig(
            ...
            stop={"iterations_since_restore": max_iters},
