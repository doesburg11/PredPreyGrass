**Quickstart**
1. Edit `predpreygrass/checkpoint_genomes/genome_config.json`.
2. Run:
```bash
python predpreygrass/checkpoint_genomes/generation_loop.py
```
...
This is implemented in `predpreygrass/checkpoint_genomes/make_child_checkpoint.py` and used by `tune_ppo.py` when restoring.
...
Mutation is applied after crossover in `predpreygrass/checkpoint_genomes/make_child_checkpoint.py`:
...
**Key Files**
- `predpreygrass/checkpoint_genomes/tune_ppo.py`
- `predpreygrass/checkpoint_genomes/run_tune_ppo.py`
- `predpreygrass/checkpoint_genomes/tune_ppo_config.json`
- `predpreygrass/checkpoint_genomes/make_child_checkpoint.py`
- `predpreygrass/checkpoint_genomes/run_make_child_checkpoint.py`
- `predpreygrass/checkpoint_genomes/make_child_checkpoint_config.json`
- `predpreygrass/checkpoint_genomes/generation_loop.py`
- `predpreygrass/checkpoint_genomes/genome_config.json`
- `predpreygrass/checkpoint_genomes/evaluate_ppo_from_checkpoint_multi_runs.py`

**Quick Start (No Arguments)**
1. Edit `predpreygrass/checkpoint_genomes/genome_config.json` to set the seed checkpoint and output folder.
2. Run:
```bash
python predpreygrass/checkpoint_genomes/generation_loop.py
```
...
```bash
python predpreygrass/checkpoint_genomes/run_make_child_checkpoint.py
```
2. Train from a checkpoint:
```bash
python predpreygrass/checkpoint_genomes/run_tune_ppo.py
```
...
**Where Outputs Go**
- Generation outputs: `predpreygrass/checkpoint_genomes/genomes`
...
- If you edit `tune_ppo.py` to change its default ray_results path, do it in `predpreygrass/checkpoint_genomes/tune_ppo.py`.
...
- If imports ever point to the old module, ensure you are running files under `predpreygrass/checkpoint_genomes`.
